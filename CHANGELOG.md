...
- `@fluojs/http`, `@fluojs/throttler`: default request identity resolution for rate limiting now trusts only the raw socket identity. Migration note: configure `trustProxy` as the primary recommendation with only the proxy addresses your deployment observes (for example, `trustProxy: ['192.0.2.10/32']` for one known reverse proxy). `trustProxyHeaders: true` is broad legacy compatibility only and is not recommended for new deployments; provide an explicit `keyResolver`/`keyGenerator` when no stable socket identity is available.
