...
- **Parallel Waves Stay Parallel**: Independent children (for example the active `review-head` axes) are dispatched in one wave of background spawns, never serialized behind one another's results. Preflight selects the axes; the lead checks the actual diff before dispatch and adds any required axes.
- **Reviewers Get the Reviewed Head**: A reviewer that must read a specific commit is handed a checkout actually at that commit. `main` is usually not that commit. The lead creates a detached review worktree (`git worktree add --detach .worktrees/review-<issue>-<head-prefix> <head-sha>`), passes that absolute path in the prompt, and states explicitly that the changed files must be read from there. A PR need not exist. A reviewer left to read `main` reviews base-state code and fails closed on tooling access rather than on the change.
...
- **Workflow Entry Points**: Native entrypoints are `$search-issue`, `$create-lane`, `$execute-lane`, and `$docs-sync-guardian`. `$execute-lane` is the sole execution orchestrator, using `issue-preflight`, `issue-implement`, `verify-local`, `review-head`, and `sync-pr` stage skills. It resolves to the v4 engine (`.agents/skills/execute-lane/`): a pure decision engine plus fresh git/GitHub observation, with no session-bound run identity. Hand it a `$create-lane` ledger with `init --from-lane-v2 .omo/lanes/<lane-id>.json`. Superseded DAG workflows are historical only and must not be invoked for new lanes.
- **Runtime Truth**: Planning ledgers live under `.omo/lanes/`; execution intent and contract/head-bound facts live under `.omo/lanes-v4/`. Fresh git/GitHub observations establish branch, PR, CI, and merge state. Goal, todo, task, and DAG state are projections and never replace workflow evidence.
