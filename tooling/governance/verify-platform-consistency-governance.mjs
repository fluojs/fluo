...
const cloudflareWorkersLifecycleDocRequirements = [
  ...
  ['docs/reference/package-surface.md', ['executionContext.waitUntil(...)', 'underlying drain', 'bootstrap a fresh application', 'Cloudflare Worker close ownership', 'host-invoked shutdown callback', 'successful host close is restartable']],
  ['docs/reference/package-surface.ko.md', ['executionContext.waitUntil(...)', 'underlying drain', '새 application을 bootstrap', 'Cloudflare Worker close 소유권', 'host가 호출하는 shutdown callback', '성공한 host close는 재시작 가능']],
  ['book/intermediate/ch24-cloudflare.md', ['CloudflareWorkersWebSocketModule.forRoot()', 'ctx.waitUntil()', 'underlying drain', 'does not invoke `worker.close()`', 'successful host close is restartable', ...cloudflareWorkerFetchEnvMarkers.english, 'Partial<WorkerEnv>', 'export class WorkerBindingsModule', 'export class DatabaseModule']],
  ['book/intermediate/ch24-cloudflare.ko.md', ['CloudflareWorkersWebSocketModule.forRoot()', 'ctx.waitUntil()', 'underlying drain', 'host는 exported `fetch` handler에 대해 `worker.close()`를 호출하지 않으며', '성공한 host close는 terminal이 아니라 재시작 가능', ...cloudflareWorkerFetchEnvMarkers.korean, 'Partial<WorkerEnv>', 'export class WorkerBindingsModule', 'export class DatabaseModule']],
...
export function enforceCloudflareWorkersLifecycleDocsSync(
  ...
  for (const [relativePath, requiredMarkers] of cloudflareWorkersLifecycleDocRequirements) {
    ...
    const missingMarkers = requiredMarkers.filter(
      (marker) => !normalizedContent.includes(marker.replace(/\s+/gu, ' ')),
    );
...
export function enforceCanonicalRuntimeMatrixReferences(readText = read) {
  ...
  assert(
    packageSurface.includes('CacheModule.forRoot(options)') && packageSurface.includes('CacheModule.forRootAsync') &&
      docsContext.includes('CacheModule.forRoot(options)') && docsContext.includes('CacheModule.forRootAsync') &&
      cacheManagerReadme.includes('CacheModule.forRoot(options)') && cacheManagerReadme.includes('CacheModule.forRootAsync'),
    'cache-manager package-surface, docs/CONTEXT.md, and README.md must keep canonical synchronous and async registration discoverable together.',
  );
  assert(
    packageSurfaceKo.includes('CacheModule.forRoot(options)') && packageSurfaceKo.includes('CacheModule.forRootAsync') &&
      docsContextKo.includes('CacheModule.forRoot(options)') && docsContextKo.includes('CacheModule.forRootAsync') &&
      cacheManagerReadmeKo.includes('CacheModule.forRoot(options)') && cacheManagerReadmeKo.includes('CacheModule.forRootAsync'),
