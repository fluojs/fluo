...
import {
  hasSynchronousOnlyClaim,
  readGraphqlMigrationTableRow,
} from './graphql-async-registration-doc-claims.mjs';
...
const migrationDocumentationPaths = new Set([
  'docs/getting-started/migrate-from-nestjs.md',
  'docs/getting-started/migrate-from-nestjs.ko.md',
]);
...
export function enforceGraphqlAsyncRegistrationContract(readText = read) {
  ...
  const factoryDependencies = typesSource.statements.find((statement) =>
    ts.isTypeAliasDeclaration(statement) && statement.name.text === 'GraphqlAsyncFactoryDependencies');
  ...
  assert(
    asyncOptions.typeParameters?.[0]?.name.text === 'TTokens'
      && asyncOptions.typeParameters[0].constraint?.getText() === 'readonly InjectionToken[]',
    typesPath,
    'must type async injection tokens as a public tuple generic',
  );
  assert(
    forRootAsync.parameters[0]?.name.getText() === 'options'
      && forRootAsync.parameters[0]?.type?.getText() === 'GraphqlAsyncModuleOptions<TTokens>'
      && forRootAsync.typeParameters?.[0]?.name.text === 'TTokens',
    modulePath,
    'must preserve the public token tuple on GraphqlModule.forRootAsync',
  );
  ...
  assert(
    injectOption?.[1] === 'TTokens'
      && factoryOption?.[1]?.includes('GraphqlAsyncFactoryDependencies<TTokens>')
      && factoryOption[1].includes('MaybePromise<GraphqlModuleOptions>')
      && factoryDependencies
      && ts.isTypeAliasDeclaration(factoryDependencies)
      && factoryDependencies.getText().includes('OptionalInjectToken<infer TDependency>')
      && factoryDependencies.getText().includes('TDependency | undefined'),
    typesPath,
    'must map injected tokens to matching factory values, including undefined for optional tokens',
  );
  assert(
  ...
  for (const section of localizedAsyncRegistrationSections) {
    ...
    assert(
      !hasSynchronousOnlyClaim(sectionText),
      section.path,
      'contains a contradictory async registration claim',
    );
    if (migrationDocumentationPaths.has(section.path)) {
      assert(
        !hasSynchronousOnlyClaim(readGraphqlMigrationTableRow(documentation, section.path, assert)),
        section.path,
        'contains a contradictory async registration claim',
      );
    }
