...
describe('Terminus runtime health contract governance', () => {
  const contractSentinel =
    'fluo-terminus-contract: registration=application-owned-TerminusModule.forRoot;health=aggregated-diagnostics;ready=binary-status;default-liveness=absent;unhealthy-status=503;route-protection=path-scoped-external-boundary';
  const governedSurfaceFacts = [
    ['packages/terminus/README.md', '`/health` returns the aggregated `HealthCheckReport` diagnostics'],
    ['packages/terminus/README.ko.md', '`/health`는 집계된 `HealthCheckReport` 진단을 반환'],
    ['docs/getting-started/migrate-from-nestjs.md', '`/health` returns aggregated diagnostics'],
    ['docs/getting-started/migrate-from-nestjs.ko.md', '`/health`는 집계 진단을 반환'],
    ['docs/CONTEXT.md', contractSentinel],
    ['docs/CONTEXT.ko.md', contractSentinel],
    ['book/beginner/ch18-health.md', contractSentinel],
    ['book/beginner/ch18-health.ko.md', contractSentinel],
  ] as const;

  type TerminusContractGuard = {
    enforceTerminusRuntimeHealthContract: (readText?: (path: string) => string) => void;
    enforceTerminusRuntimeHealthContractCompanions: (changedFiles: readonly string[]) => void;
  };

  async function loadTerminusContractGuard(): Promise<TerminusContractGuard> {
    return (await import('./verify-platform-consistency-governance.mjs')) as unknown as TerminusContractGuard;
  }

  it('governs every Terminus contract surface and invokes the central guard', async () => {
    const { enforceTerminusRuntimeHealthContract } = await loadTerminusContractGuard();
  ...
  it.each(governedSurfaceFacts)('rejects a removed governed fact in %s', async (path, fact) => {
    const { enforceTerminusRuntimeHealthContract } = await loadTerminusContractGuard();
    const readText = (relativePath: string): string => {
      const content = readFileSync(join(repoRoot, relativePath), 'utf8');

      return relativePath === path ? content.replace(fact, '') : content;
    };
  ...
  ])('rejects changed Terminus runtime %s', async (source, replacement, expectedMessage) => {
    const { enforceTerminusRuntimeHealthContract } = await loadTerminusContractGuard();
    const readText = (relativePath: string): string => {
      const content = readFileSync(join(repoRoot, relativePath), 'utf8');
  ...
  it('requires all governed documents and both guard files as Terminus companions', async () => {
    const { enforceTerminusRuntimeHealthContractCompanions } = await loadTerminusContractGuard();
    const completeCompanions = [
      ...governedSurfaceFacts.map(([path]) => path),
      'tooling/governance/verify-platform-consistency-governance.mjs',
      'tooling/governance/verify-platform-consistency-governance.test.ts',
    ];
