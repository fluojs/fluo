/**
 * Deno native composition fixture app.
 *
 * Runs the managed Deno adapter path end to end on a real `Deno.serve`:
...
} from '../../../../../../packages/platform-deno/dist/index.js';
import {
  FluoFactory,
  defineModule,
...
if (!sentinelPath) {
  throw new Error('Usage: deno run deno-server-app.ts <sentinel-path>');
}
...
const adapter = DenoHttpApplicationAdapter.create({ hostname: '127.0.0.1', port: 0 });
const app = await FluoFactory.create(AppModule, {
  adapter,
  shutdownRegistration: createDenoShutdownSignalRegistration(['SIGTERM']),
});
