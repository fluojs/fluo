...
- **Shutdown ownership**: `close()` stops new HTTP and websocket-upgrade ingress with a `503` shutdown response, waits for Bun server termination and every accepted request from realtime binding evaluation through HTTP response or upgrade completion, clears adapter state only after both settle, and removes signal listeners registered by `runBunApplication()`.
