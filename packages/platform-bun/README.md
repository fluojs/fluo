...
- `BunWebSocketBinding` and `BunWebSocketUpgradeHost`: platform-owned binding contracts used by `@fluojs/websockets/bun` before normal HTTP dispatch. Bindings receive only an upgrade-capable host, not the adapter-owned Bun server lifecycle or raw fetch handler.
...
- **Adapter instance helpers**: `BunHttpApplicationAdapter` exposes `getServer()`, `getListenTarget()`, `getRealtimeCapability()`, `listen()`, and `close()`. Realtime modules install through the version-1 capability during module registration; direct configure/setter APIs are removed.
...
The broader repository suite also exercises Bun through `WebRuntimeHttpAdapterPortabilityHarness.create(...)` alongside Deno and Cloudflare Workers in `packages/testing/src/portability/web-runtime-adapter-portability.test.ts`, keeping the shared web-runtime portability baseline aligned across fetch-style platforms.
