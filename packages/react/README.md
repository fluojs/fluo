...
`--watch` performs startup generation, then serializes coalesced changes under the application module
directory while preserving the last valid artifact after generation failures. Every run closes its
bootstrapped application after reading `app.dispatcher.describeRoutes()`. Import the generated
artifact from application code:
