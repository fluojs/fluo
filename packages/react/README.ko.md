...
run은 파일을 다시 쓰지 않습니다. `--check`는 write 없이 exact generated byte를 비교합니다. `--watch`는
startup generation 뒤 application module directory 아래의 coalesced change를 serialize하고 generation
failure 뒤에도 마지막 valid artifact를 보존합니다. 모든 run은 `app.dispatcher.describeRoutes()`를 읽은 뒤
bootstrap한 application을 닫습니다. 생성 artifact는 application code에서 import합니다.
