...
JWT 오류가 패키지 경계를 넘으면 안정적인 `code`와 함께 `isJwtError(...)`를 사용하세요. 이 함수는 버전이 있는 JWT 소유자 규약을 검증하고 브랜드가 없는 유사 객체나 호환되지 않는 버전을 거부합니다. Passport 내장 전략도 이 경로로 호환되는 중복 사본의 만료/유효하지 않음 분류를 보존합니다.
