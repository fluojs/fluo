...
- `@fluojs/cli`는 mandatory `@fluojs/runtime` dependency 때문에 Node.js `>=20.19.3 <21 || >=22.2.0 <27`을 요구합니다. 업데이트 전에 기존 Node 20 설치는 `>=20.19.3`으로, Node 22 설치는 `>=22.2.0`으로 올리세요. Node 21과 Node 27 이상은 지원하지 않으므로 CLI process를 지원되는 Node 20 또는 Node 22 line으로 옮겨야 합니다. 생성된 Bun, Deno, Cloudflare Workers starter가 비 Node runtime을 대상으로 할 수는 있지만 CLI process 자체는 Node.js에서 실행됩니다. 생성된 Node HTTP, mixed, microservice-only 프로젝트도 listener-level RFC `QUERY`가 framework dispatch에 도달하도록 같은 범위를 사용합니다.
