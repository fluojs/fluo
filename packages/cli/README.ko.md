...
기존 테스트에서는 제거된 `createTestApp`, `createTestingModule` import를 `import { Test } from '@fluojs/testing'`으로 교체하고 `Test.createApp(...)`, `Test.createTestingModule(...)`을 호출하세요. 옵션, builder chain, request assertion, public test type은 그대로 유지됩니다. 컴파일 이후 module assertion은 `try`에, `await testingModule.container.dispose()`는 `finally`에 두세요. 앱 테스트도 `finally`에서 `await app.close()`를 호출하세요. CLI 업그레이드는 기존 파일을 다시 쓰지 않으므로 템플릿을 재생성하거나 직접 이관해야 합니다.
