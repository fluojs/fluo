...
describe('scaffoldBootstrapApp', () => {
  ...
  it('describes only the wired application starter path in generated HTTP starter output', async () => {
    ...
    expect(mainFile).toContain("import { createConsoleApplicationLogger, createNodeShutdownSignalRegistration } from '@fluojs/platform-nodejs';");
  });
  ...
  it('generates a runnable Kafka microservice starter scaffold', async () => {
    ...
    expect(envFile).not.toContain('KAFKA_RESPONSE_TOPIC=');
    expect(appFile).toContain("import { Kafka, logLevel, type Consumer, type Producer } from 'kafkajs';");
    expect(appFile).toContain('const responseTopic = process.env.KAFKA_RESPONSE_TOPIC;');
    ...
    expect(appFile).toContain('readonly ownsResources = true;');
    expect(appFile).toContain("outboundClients: 'framework'");
    expect(appFile).toContain("server: 'framework'");
    expect(appFile).toContain('setLogger(');
  ...
  it('generates a runnable RabbitMQ microservice starter scaffold', async () => {
    ...
    expect(envFile).not.toContain('RABBITMQ_RESPONSE_QUEUE=');
    expect(appFile).toContain("import { connect } from 'amqplib';");
    expect(appFile).toContain('const responseQueue = process.env.RABBITMQ_RESPONSE_QUEUE;');
