...
- Package manager: install/run commands can use ${options.packageManager}; ${createRunCommand(options.packageManager, 'dev')} delegates to \`fluo dev\`, which owns the generated runtime lifecycle and defaults \`NODE_ENV\` to \`development\` when unset
...
- Package manager: install/run commands can use ${options.packageManager}; ${createRunCommand(options.packageManager, 'dev')} delegates to \`fluo dev\`, which owns the generated runtime lifecycle and defaults \`NODE_ENV\` to \`development\` when unset; ${starter.packageManagerNote}
...
- Runtime contract: \`src/app.ts\` owns one shared module graph, and \`src/main.ts\` bootstraps the HTTP API before calling \`connectMicroservice()\` and \`startAllMicroservices()\`; ${createRunCommand(options.packageManager, 'dev')} delegates to \`fluo dev\`, which defaults \`NODE_ENV\` to \`development\` when unset
