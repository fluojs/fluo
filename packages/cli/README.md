...
- `inspect` resolves `@fluojs/runtime` only when inspection runs. Install a runtime version compatible with your active Node.js version before using `fluo inspect`; other CLI commands remain available across the CLI's full Node.js `>=20.0.0` range.
