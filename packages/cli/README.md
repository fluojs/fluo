...
- `@fluojs/cli` requires Node.js `>=20.19.3 <21 || >=22.2.0 <27` because it has a mandatory `@fluojs/runtime` dependency. Upgrade existing Node 20 installations to `>=20.19.3` or Node 22 installations to `>=22.2.0` before updating; Node 21 and Node 27+ are unsupported, so move the CLI process to a supported Node 20 or Node 22 line. Generated Bun, Deno, and Cloudflare Workers starters may target non-Node runtimes, but the CLI process itself runs on Node.js. Generated Node HTTP and mixed projects use the same range so listener-level RFC `QUERY` reaches framework dispatch. Node microservice-only projects retain their independent `>=20.0.0` floor.
