...
Generated standard non-Deno HTTP starters use a TDD-first Vitest layout: fast greeting unit tests and `greeting.slice.test.ts` stay colocated under `src/greeting/`, app dispatch tests stay in `src/app.test.ts`, and the default e2e-style request-pipeline tests live in `test/app.e2e.test.ts` with `Test.createApp({ rootModule })` plus `app.request(...).send()`. The React starter instead includes focused streamed SSR, DOM hydration, and production Playwright hydration tests. Its `test:browser` script starts the built Fastify server and fails on missing assets, hydration warnings, or navigation that bypasses the server-owned route.
...
The Cloudflare Workers template calls
`CloudflareWorkerApplicationHost.create(AppModule)` and exports
`export default { fetch: worker.fetch }`. The host owns first-request bootstrap,
retry, generation replacement, and drain recovery. Each generation internally
creates its adapter, calls `FluoFactory.create(AppModule, { adapter })`, and awaits
no-socket `app.listen()`. Use the host's `{ fromEnv }` overload when the first
Worker environment must select bootstrap configuration.
...
Existing tests must replace the removed `createTestApp` and `createTestingModule` imports with `import { Test } from '@fluojs/testing'`, then call `Test.createApp(...)` and `Test.createTestingModule(...)`. Options, builder chains, request assertions, and public test types remain unchanged. After compilation, put module assertions in `try` and `await testingModule.container.dispose()` in `finally`; app tests likewise use `await app.close()` in `finally`. Regenerate templates or migrate existing files explicitly; upgrading the CLI does not rewrite them.
...
```bash
# Store a versioned report as the default inspect artifact recipe
