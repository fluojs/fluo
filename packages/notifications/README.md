...
- Queue adapters must resolve every `enqueue()` result and every `enqueueMany()` result entry with a non-empty string identifier. The service rejects malformed values with `NotificationQueueResultIntegrityError`; it never fabricates a queued delivery id from the notification envelope.
