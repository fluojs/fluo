...
- Queue adapter는 모든 `enqueue()` 결과와 모든 `enqueueMany()` 결과 항목을 non-empty string identifier로 resolve해야 합니다. 서비스는 malformed value를 `NotificationQueueResultIntegrityError`로 거부하며 notification envelope에서 queued delivery id를 fabricate하지 않습니다.
