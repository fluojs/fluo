...
Handler failure isolation is narrower than publish completion. If a matching local listener throws or returns a rejected promise, the event bus logs and isolates that failure, continues the other matching listeners, and does not reject `publish(...)` because of that listener failure alone. Inbound transport handlers follow the same logging and isolation rule, and inbound callback completion does not surface isolated listener failures. A completed publisher promise therefore does not prove that every listener succeeded. Do not interpret this listener-only rule as swallowing timeout, cancellation, transport publication, bootstrap, or other unrelated publisher failures; those keep their separate documented completion and failure behavior.
