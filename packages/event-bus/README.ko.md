...
Handler failure isolation은 publish completion보다 좁은 계약입니다. 일치하는 local listener가 throw하거나 rejected promise를 반환하면 event bus는 해당 실패를 log하고 격리하며, 다른 matching listener를 계속 실행하고 그 listener 실패만으로 `publish(...)`를 reject하지 않습니다. Inbound transport handler에도 같은 logging 및 isolation 규칙이 적용되며, inbound callback completion은 격리된 listener 실패를 외부로 드러내지 않습니다. 따라서 publisher promise가 완료되었다고 해서 모든 listener가 성공했다는 뜻은 아닙니다. 이 listener-only 규칙을 timeout, cancellation, transport publication, bootstrap 또는 그 밖의 무관한 publisher 실패까지 삼키는 것으로 해석하면 안 됩니다. 이러한 실패에는 각각 별도로 문서화된 completion 및 failure 동작이 그대로 적용됩니다.
