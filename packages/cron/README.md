...
Dynamic registry task identity now comes exclusively from the first `name` argument: remove `options.name` from `addCron`, `addInterval`, and `addTimeout` calls. Use `distributed: { enabled: true }` to enable Redis locking and omit `distributed` to keep it disabled; boolean `distributed: true` and `distributed: false` are no longer accepted. Integration code that reads decorator metadata must use `getSchedulingTaskMetadata(...)`, `getSchedulingTaskMetadataEntries(...)`, and `schedulingMetadataSymbol`; cron-only metadata aliases and metadata writers are no longer public. `normalizeCronModuleOptions(...)` and `NormalizedCronModuleOptions` are runtime internals, not application configuration APIs.
...
The registry exposes `addCron`, `addInterval`, `addTimeout`, `remove`, `enable`, `disable`, `get`, `getAll`, `updateCronExpression`, and `updateIntervalMs`. The first `name` argument is the sole registry key, scheduler metadata name, and default distributed lock identity for dynamic tasks. Dynamic options configure task behavior only; they do not rename a task. Registry and decorator task names must be non-empty strings. `get` and `getAll` return immutable `SchedulingTaskDescriptor` snapshots, not live `CronJob` handles or mutable registry state. Timeout tasks run once, then disable themselves while remaining in the registry so they can be re-enabled deliberately.

Dynamic cron registration is atomic with scheduler startup: if the scheduler rejects a new cron job, the registry does not retain a half-registered task. Updating a running cron expression or interval cadence is also rollback-safe. A provisional replacement remains tick-ineligible until the previous scheduled handle stops successfully and the registry commits the new handle token; callbacks already queued by a retired handle are ignored. If replacement scheduling fails, fluo restores the previous expression or interval milliseconds and handle before rethrowing the failure. If the previous handle cannot be stopped, fluo stops the provisional replacement and restores the previous state; when that rollback stop also fails, fluo retains the provisional handle for bounded later cleanup without committing its token, so it remains tick-ineligible. Disabling or removing an active task clears its scheduler handle only after `stop()` succeeds. A stop failure is logged, the handle remains registered for a safe retry, and the operation returns `false`. A failed `disable()` leaves the task descriptor disabled so later ticks stay gated while another disable or shutdown retries cleanup; a failed `remove()` keeps the task registered until a later removal succeeds. If the first shutdown cleanup attempt also fails, the next shutdown lifecycle hook retries each retained handle once and clears it only after `stop()` succeeds. Cron tasks use both scheduler-level no-overlap protection and fluo's in-process running guard, so the same task instance will not run overlapping ticks.
...
- Public scheduling types: `SchedulingTaskKind`, `SchedulingTaskCallback`, `SchedulingTaskOptions`, `CronTaskOptions`, `IntervalTaskOptions`, `TimeoutTaskOptions`, `DynamicCronTaskOptions`, `DynamicIntervalTaskOptions`, `DynamicTimeoutTaskOptions`, `CronTaskMetadata`, `IntervalTaskMetadata`, `TimeoutTaskMetadata`, `SchedulingTaskMetadata`, `CronTaskDescriptor`, `SchedulingTaskDescriptor`, and `SchedulingRegistry`.
- Public module and scheduler types: `CronModuleOptions`, `CronDistributedOptions`, `CronShutdownOptions`, `CronScheduleOptions`, `CronScheduler`, and `CronScheduledJob`.
- Public status types: `CronLifecycleState`, `CronStatusAdapterInput`, and `CronPlatformStatusSnapshot`.
- Metadata integration APIs: `getSchedulingTaskMetadata`, `getSchedulingTaskMetadataEntries`, and `schedulingMetadataSymbol`.
