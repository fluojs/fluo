...
Package manifest의 `engines.node >=20.19.3 <21 || >=22.2.0 <27` 선언은 mandatory `@fluojs/runtime`에 맞춘 published package와 기본 Node.js entrypoint 기준입니다. Node 20 host는 `>=20.19.3`으로, Node 22 host는 `>=22.2.0`으로 올리세요. Node 21과 Node 27 이상은 지원하지 않습니다. Bun, Deno, Cloudflare Workers 지원은 아래 전용 fetch-style subpath를 통해 노출되며, 해당 subpath는 request/handler type을 web-standard로 유지하고 application code가 root Node.js lifecycle-service alias에 의존하지 않게 합니다.
