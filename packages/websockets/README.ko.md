...
`@fluojs/websockets/node` registration path는 패키지가 직접 소유한 `ws` dependency를 사용합니다. 애플리케이션 코드에서 `ws`를 직접 사용하지 않는 한 별도 설치가 필요하지 않습니다.
...
Gateway decorator와 shared contract는 runtime-neutral root에서 import합니다. 기본 Node.js 기반 websocket runtime을 선택하려면 Node subpath의 `NodeWebSocketModule.forRoot()`를 사용합니다.
...
`NodeWebSocketModule.forRoot(...)`를 사용하면 핸드셰이크 전에 익명 업그레이드를 거절하고, 공통 연결/페이로드 제한을 조정할 수 있습니다.
...
`@fluojs/websockets/node` guard는 Node의 `IncomingMessage`를 받습니다. Fetch-style subpath는 Web standard `Request`를 받으므로, 재사용 가능한 옵션 객체를 작성할 때는 subpath별 `WebSocketModuleOptions` 타입을 선택하세요. Guard는 `true`, `undefined`, 또는 return 없음으로 upgrade를 허용하고, `false` 또는 `{ status, body? }` 형태의 `WebSocketUpgradeRejection`으로 거절하거나 `UnauthorizedException` 같은 `HttpException` 계열 오류를 throw할 수 있습니다. Throw된 HTTP exception은 socket이 accept되기 전에 동일한 pre-handshake rejection response로 변환됩니다.

### Room
`WebSocketRoomService`를 사용하면 gateway 또는 application service가 adapter 내부에 접근하지 않고도 가벼운 room membership state를 유지할 수 있습니다. Runtime lifecycle service는 `joinRoom(socketId, room)`, `leaveRoom(socketId, room)`, `broadcastToRoom(room, event, data)`, `getRooms(socketId)`를 구현합니다. `joinRoom(...)`은 `socketId`가 선택한 runtime lifecycle service에 등록된 현재 열린 socket을 가리킬 때만 membership을 추가합니다. 알 수 없거나 이미 닫힌 socket identifier는 무시하며 room index를 생성하지 않습니다. `broadcastToRoom(...)`은 현재 room에 있는 열린 socket에 `{ event, data }` 형태의 JSON frame을 보냅니다. Node.js 기반 adapter는 전송 전에 설정된 `backpressure` policy를 적용하지만, fetch-style runtime(`@fluojs/websockets/bun`, `@fluojs/websockets/deno`, `@fluojs/websockets/cloudflare-workers`)은 room broadcast에 backpressure policy를 적용하지 않습니다.

`WebSocketRoomService`는 runtime lifecycle service가 구현하는 type-only contract입니다. `@Inject(...)`로 runtime lifecycle token을 주입하고 constructor parameter를 `WebSocketRoomService`로 type 지정하세요. `NodeWebSocketGatewayLifecycleService`는 `@fluojs/websockets/node`에서 import하며 다른 runtime subpath는 해당 runtime lifecycle token을 export합니다.
...
- `@fluojs/websockets/node`의 `NodeWebSocketModule`: Node WebSocket registration module입니다.
- `NodeWebSocketModule.forRoot({ upgrade, limits, backpressure, buffer, heartbeat, replies, shutdown })`: Node pre-upgrade guard, bounded runtime default, optional event-envelope reply를 구성합니다.
- `@fluojs/websockets/node`의 `NodeWebSocketGatewayLifecycleService`: 보존되는 Node.js lifecycle service DI token입니다. 직접 instantiate하지 말고 application container에서 resolve하세요.
...
- Typed runtime seam: `WebSocketUpgradeContext`, `WebSocketUpgradeGuard`, `WebSocketUpgradeRejection`, `WebSocketGatewayDescriptor`, `WebSocketGatewayHandlerDescriptor`, runtime subpath의 handler/guard projection, platform package의 native host/socket/binding type.
...
Registration과 runtime projection에는 runtime subpath를 사용하세요. Root `@fluojs/websockets` entrypoint는 runtime-neutral이며 gateway decorator, metadata helper, descriptor, shared contract를 여기서 import합니다. `NodeWebSocketModule`과 `NodeWebSocketGatewayLifecycleService`는 Node subpath에서 import하고 Bun, Deno, Workers module과 native projection은 해당 subpath와 platform package에서 import합니다. 제거된 root Node registration과 lifecycle alias에는 compatibility export가 없습니다.

Published package와 Node.js runtime subpath의 package-owned 지원 계약은 manifest의 `engines.node >=24.0.0 <27` 선언입니다. Node 20 및 Node 22 host를 Node.js `>=24.0.0 <27`로 올리세요. Node 24 미만과 Node 27 이상은 지원하지 않습니다. Bun, Deno, Cloudflare Workers 지원은 아래 전용 fetch-style subpath를 통해 노출되며, 해당 subpath는 request/handler type을 web-standard로 유지하고 자신의 lifecycle token을 사용합니다. Root에는 Node.js lifecycle-service alias가 없습니다.

Root는 runtime-neutral authoring primitive와 shared contract인 `WebSocketGateway`, `OnConnect`, `OnMessage`, `OnDisconnect`, `defineWebSocketGatewayMetadata`, `getWebSocketGatewayMetadata`, `defineWebSocketHandlerMetadata`, `getWebSocketHandlerMetadata`, `getWebSocketHandlerMetadataEntries`, `webSocketGatewayMetadataSymbol`, `webSocketHandlerMetadataSymbol`을 제공합니다. 각 runtime subpath는 module registration entrypoint, lifecycle token, 진짜 runtime-specific projection만 export합니다. Native host, socket, binding type은 각 platform package가 소유하며 Bun upgrade projection은 `@fluojs/platform-bun`이 소유합니다.
