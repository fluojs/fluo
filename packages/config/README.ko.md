...
패키지는 Node.js 20.0.0 이상을 지원합니다. Env-file loading과 watch mode는 Node filesystem, path, crypto builtin을 host runtime boundary를 통해 lazy하게 해석하며, 가능하면 `process.getBuiltinModule(...)`을 사용하고 그렇지 않으면 published ESM과 호환되는 `node:module` fallback을 사용합니다. `ConfigService`와 `loadConfig({ defaults, processEnv, runtimeOverrides })`의 in-memory 사용은 env-file access를 요구하지 않습니다.
