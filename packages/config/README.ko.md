...
Env-file loading과 watch mode는 Node builtin을 host runtime boundary를 통해 lazy하게 해석하기 위해 Node.js 20.16.0 이상이 필요합니다. `ConfigService`와 `loadConfig({ defaults, processEnv, runtimeOverrides })`의 in-memory 사용은 env-file access를 요구하지 않습니다.
