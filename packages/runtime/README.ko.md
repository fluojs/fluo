...
- `RuntimePlatformShell.start()`와 `stop()`은 lifecycle transition을 직렬화합니다. 같은 작업에 대한 겹치는 호출은 진행 중인 작업을 공유하며, startup 중 요청된 stop은 startup이 settle된 뒤 reverse dependency cleanup을 실행합니다. 실패한 transition 이후에도 명시적으로 재시도할 수 있고 dependency ordering과 startup rollback은 그대로 유지됩니다.
