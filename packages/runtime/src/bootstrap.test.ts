...
import { HTTP_APPLICATION_ADAPTER, PLATFORM_SHELL, RUNTIME_CLEANUP_REGISTRATION } from './tokens.js';
import type { MicroserviceRuntime, OnModuleInit, RuntimeCleanupRegistration } from './types.js';
...
describe('FluoFactory.createApplicationContext', () => {
  ...
  it('runs internally registered runtime cleanup callbacks after bootstrap hook failure', async () => {
    const events: string[] = [];
    ...
    class FailingBootstrapHook {
      onApplicationBootstrap() {
        events.push('bootstrap:fail');
        throw new Error('bootstrap hook failed');
      }
    }
    ...
    await expect(FluoFactory.createApplicationContext(AppModule, {
    })).rejects.toThrow('bootstrap hook failed');
...
describe('FluoFactory.createMicroservice', () => {
  ...
  it('does not let a delayed microservice listen transition back to ready after close starts', async () => {
    const events: string[] = [];
    const listenCanFinish = createDeferred<void>();
    const MICROSERVICE_TOKEN = Symbol.for('fluo.microservices.service');
    ...
    const closePromise = microservice.close('SIGTERM');
    ...
    expect(microservice.state).toBe('closed');
    expect(events).toEqual([
      'runtime:listen:start',
      'runtime:listen:end',
      'runtime:close:SIGTERM',
