...
Advanced handlers can instead take response ownership by calling `RequestContext.response.send(...)`, `redirect(...)`, or a manual streaming helper. Once that response is committed, the interceptor chain preserves the handler-owned return value unchanged and the runtime dispatcher skips its normal success-response write. Direct response code must therefore produce the final safe payload before committing; a serializer cannot reshape it afterward.
...
- HTTP response writing is single-owner: framework-managed handler results may be transformed by interceptors before the runtime commits them, but once a handler or response helper commits `RequestContext.response`, the runtime preserves that handler-owned response, skips a second success-response write, and `SerializerInterceptor` bypasses serialization.
