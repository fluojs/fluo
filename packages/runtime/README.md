...
- `RuntimePlatformShell.start()` and `stop()` serialize lifecycle transitions. Overlapping calls for the same operation share the in-flight work, and a stop requested during startup waits for startup to settle before reverse dependency cleanup. Failed transitions do not prevent an explicit retry, while dependency ordering and startup rollback remain unchanged.
