...
- `Application.listen()` and microservice `listen()` are serialized with shutdown: overlapping startup calls share the same in-flight startup, shutdown waits for in-flight startup to settle, and a startup that races with shutdown cannot transition the shell back to `ready` after close begins. Starting application close sets the public state to terminal `closed` immediately, so a failed close attempt cannot make disposed resources reachable through a later `listen()` or `get()`. A later `close()` skips completed teardown phases and retries the failed teardown stage. Once microservice close starts, a terminal ingress gate rejects new `send()` and `emit()` calls before runtime or transport handoff, including while `listen()` is still pending and after a failed close attempt.
