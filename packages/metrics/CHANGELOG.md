...
- [#3779](https://github.com/fluojs/fluo/pull/3779) [`1f8912c`](https://github.com/fluojs/fluo/commit/1f8912c12ac57f031d13dbfec9ce88291aa55e87) Thanks [@ayden94](https://github.com/ayden94)! - Move low-level Prometheus integration APIs, including `Registry`, to `@fluojs/metrics/integration` and remove the `MetricsModule.forRoot({ registry })` input. Import `Registry` from the integration subpath, then share a registry only by providing `METRICS_REGISTRY` to `FluoFactory.create(..., { providers })`; migrate each independent application with its own bootstrap provider.
