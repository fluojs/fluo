...
export class NodemailerEmailTransport implements EmailTransport {
  ...
  /**
   * Wraps an existing Nodemailer transporter without transferring ownership.
   *
   * @param options Existing transporter wiring owned and closed by the caller.
   * @returns A transport suitable for direct use with {@link EmailModule.forRoot}.
   */
  static create(options: NodemailerEmailTransportOptions): NodemailerEmailTransport {
    return new NodemailerEmailTransport(options.transporter);
  }

  /**
   * Creates a factory-owned SMTP transport for {@link EmailModule.forRoot}.
   *
   * @param options SMTP connection settings and an optional diagnostic label.
   * @returns A lazy transport factory whose resources are owned by the email module.
   */
  static createFactory(options: NodemailerEmailTransportFactoryOptions): EmailTransportFactory {
    const kind = options.kind?.trim() || 'nodemailer-smtp';

    return {
      create: async () => NodemailerEmailTransport.create({ transporter: nodemailer.createTransport(options.smtp) }),
      kind,
      ownsResources: true,
    };
  }
   ...
   * @example
   * ```ts
   * const transport = NodemailerEmailTransport.create({ transporter });
