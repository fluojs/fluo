...
- Bootstrap failures after GraphQL schema resolution remove only the failed service's cross-realm GraphQL object allowlist before rethrowing. Each actual `graphql/jsutils/instanceOf` module object tracks its own installed patch and active application allowlists.
