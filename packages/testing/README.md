...
Use `assertSupportsConditionalRequests()` to prove generated ETags, bodyless conditional GET and HEAD behavior, and unsafe precondition ordering through the adapter's real listener or fetch dispatch seam.
...
- **Harness subpaths**: `platform-conformance`, `http-adapter-portability`, `web-runtime-adapter-portability`, `fetch-style-websocket-conformance`. The HTTP portability harnesses expose `assertSupportsCustomHttpRouteMethods()`, `assertSupportsConditionalRequests()`, `assertSupportsHttpErrorRepresentations()`, `assertDoesNotCommitAbortedHttpErrorRepresentations()`, `createErrorRepresentationBootstrapOptions`, `NetworkHttpErrorRepresentationBootstrapOptions`, and `WebHttpErrorRepresentationBootstrapOptions` for adapter-owned bootstrap typing.
