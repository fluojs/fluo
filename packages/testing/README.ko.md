...
`vitest`는 mock 헬퍼와 `@fluojs/testing/vitest` 엔트리포인트가 요구하는 peer dependency입니다. `@babel/core`는 optional peer dependency이며 `@fluojs/testing/vitest` 엔트리포인트를 사용할 때만 필요합니다.
