...
`@fluojs/testing`은 fluo 애플리케이션 테스트를 위한 공식적인 기준(Baseline)을 제공합니다. 격리된 테스트 환경을 구축하고, 의존성을 가짜(Fake)나 목(Mock)으로 교체하며, 모듈 그래프에서 직접 컴포넌트를 resolve하거나 `Test.createApp(...).request(...).send()`로 가상 HTTP 요청을 실행하여 e2e 스타일 테스트를 수행할 수 있게 합니다.
...
`Test`는 app과 testing module 생성을 위한 유일한 공개 entrypoint입니다. `@fluojs/testing` 또는 non-mock `@fluojs/testing/module` subpath에서 import하세요.

**Breaking migration:** `createTestApp` / `createTestingModule` free-function import를 `Test`로 바꾸고 `Test.createApp(options)` / `Test.createTestingModule(options)`를 호출하세요. Free function과 `@fluojs/testing/app` subpath는 제거되며 compatibility alias로 남지 않습니다. 명시적 `rootModule`, bootstrap option, override chain, unconditional app/container cleanup은 그대로 유지하세요.
...
애플리케이션 route, guard, interceptor, DTO validation, request body, query parameter, header, synthetic principal, request-scoped provider isolation, serialized response를 검증하는 기본 HTTP/e2e 스타일 경로로는 `Test.createApp({ rootModule })`을 사용하세요. 하나의 slice 안에서 module wiring, provider visibility, provider/guard/interceptor override가 계약일 때는 `Test.createTestingModule(...)`을 사용합니다.

## 주요 패턴

`Test.createApp(...)`는 `FluoFactory.create`로 HTTP shell을 만들며 request-context
...
### `Test.createApp()` 기반 request-level 테스트
...
**Breaking migration:** `createMock` / `createDeepMock` 대신 `ShallowMock` / `PrototypeMock`을 import하고 `ShallowMock.create(...)` / `PrototypeMock.create(...)`를 호출하며, `DeepMocked<T>` import는 `ShallowMocked<T>`로 바꾸세요. 기존 이름은 제거되며 두 헬퍼 모두 재귀 mocking을 하지 않습니다.

`ShallowMock.create(partial, options)`는 제공한 값을 보존하고 누락된 각 속성에 같은 `vi.fn()`을 지연 생성합니다. Data property는 명시적으로 제공하세요. 중첩 객체와 반환값은 mock하지 않습니다. `PrototypeMock.create(Type)`은 constructor 실행이나 accessor 평가 없이 symbol key를 포함한 자체 및 상속 prototype method의 spy를 만듭니다. 결과는 class instance가 아닌 일반 test double이므로 instance field와 arrow-function member는 직접 제공하세요.
...
**Breaking migration:** 각 `create*Harness` free-factory import를 같은 subpath의 harness class로 바꾸고 static `create(options)` 메서드를 호출하세요. Option, assertion, cleanup 계약은 그대로 유지되며 free-factory alias는 남지 않습니다.
...
`PlatformShellLifecycleConformanceHarness.create({ createShell })`는 active `start()` / `stop()` overlap 전체가 `PlatformLifecycleConflictError`로 reject되는지, callback reentry가 synchronous 시점과 임의의 await 이후에도 conflict-safe한지, 실패한 transition이 settle된 뒤 호출자가 retry할 수 있는지 검증합니다. 컴포넌트 수준 검사는 `PlatformConformanceHarness.create(...).assertAll()`에 유지하세요. PlatformShell lifecycle 계약은 의도적으로 별도 harness입니다.
...
2. Direct page return, missing-renderer diagnostic, DTO validation, request-scope identity, response ownership,
   guard, interceptor, native mutation route는 `Test.createApp({ rootModule })`로 검증하고 `finally`에서 app을
   닫습니다.
