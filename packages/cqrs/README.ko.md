...
CQRS handler, event handler, saga는 singleton provider에서만 discovery됩니다. Non-singleton registration은 경고와 함께 건너뜁니다. 같은 메시지에 대한 Command/Query handler 중복은 서로 다른 DI token을 사용할 때만 오류이며, 같은 token이 반복 discovery되면 하나의 handler로 병합됩니다. Event handler와 saga도 같은 token 병합 규칙을 따르지만, 같은 Event에 대해 서로 다른 token을 가진 event handler들은 유효하며 discovery 순서대로 각각 한 번씩 호출됩니다.
...
- `DuplicateCommandHandlerError`, `DuplicateQueryHandlerError`: 서로 다른 DI token을 가진 singleton provider가 같은 command 또는 query type을 claim할 때 발생합니다. 같은 token이 반복 discovery되면 하나의 handler로 병합됩니다.
- `DuplicateEventHandlerError`: 충돌하는 event-handler discovery 실패를 위해 export됩니다. 일반적으로 같은 event에 여러 `@EventHandler(...)` provider를 등록하는 것은 유효하며 discovery 순서대로 fan-out되지만, 완전히 동일한 DI provider(동일한 token)가 중복 발견되면 불필요한 중복 실행을 막기 위해 단일 구독으로 병합(collapse)됩니다.
