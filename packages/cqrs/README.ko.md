...
유효한 lifecycle state는 `created`, `discovering`, `ready`, `stopping`, `stopped`, `failed`입니다. Readiness는 event pipeline과 saga runtime이 모두 `ready`일 때만 `ready`이고, 둘 중 하나가 `discovering`이면 `degraded`이며, `created`, `stopping`, `stopped`, `failed` 중 하나이면 `not-ready`입니다. Health는 transition이나 failure state 밖에서는 `healthy`, 둘 중 하나가 `discovering` 또는 `stopping`이면 `degraded`, `stopped` 또는 `failed`이면 `unhealthy`입니다. 0이 아닌 drain-timeout counter는 이 판단보다 우선해 degraded health를 보고합니다. Command와 Query lifecycle field는 diagnostic 전용이며 기존 event/saga readiness 또는 health rule을 바꾸지 않습니다.
