...
CQRS handlers, event handlers, and sagas are discovered only on singleton providers. Non-singleton registrations are skipped with warnings. Duplicate command or query handlers for the same message are errors only when the duplicate uses a different DI token; repeated discovery of the same token collapses into one handler. Event handlers and sagas follow the same same-token collapse rule, while distinct event-handler tokens for the same event remain valid and are each invoked once in discovery order.
...
- `DuplicateCommandHandlerError`, `DuplicateQueryHandlerError`: Raised when different singleton providers with different DI tokens claim the same command or query type. Repeated discovery of the same token is collapsed into one handler.
