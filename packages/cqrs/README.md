...
CQRS handlers, event handlers, and sagas are discovered only on singleton providers. Non-singleton registrations are skipped with warnings. Duplicate command or query handlers are detected by provider registration, so two singleton providers using the same handler class under different DI tokens are still treated as conflicting command/query ownership. Event handlers keep fan-out semantics: two singleton providers using the same event-handler class under different DI tokens both run in discovery order.
