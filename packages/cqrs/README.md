...
CQRS handlers, event handlers, and sagas are discovered only on singleton providers. Non-singleton registrations are skipped with warnings. Command and query handlers remain point-to-point across provider aliases as well as classes: two singleton providers that use the same handler class for the same command or query still fail discovery. Event handlers keep fan-out semantics, so multiple singleton providers for the same event, including aliases of the same handler class, are invoked in discovery order.
