...
`runWithRequestContext(...)`는 호스트가 `globalThis.AsyncLocalStorage` 또는 Node의 `process.getBuiltinModule('node:async_hooks')`로 `AsyncLocalStorage`를 제공할 때만 활성 컨텍스트를 `await` 이후까지 보존합니다. 비동기 컨텍스트 primitive가 없는 호스트에서는 동기 stack fallback을 사용하며, 겹치는 비동기 요청이 서로의 컨텍스트를 관찰하지 않도록 awaited continuation이 재개되기 전에 컨텍스트를 비웁니다.
