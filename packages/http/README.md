...
`runWithRequestContext(...)` preserves the active context across awaited work only when the host provides `AsyncLocalStorage` through `globalThis.AsyncLocalStorage` or Node's `process.getBuiltinModule('node:async_hooks')`. Hosts without an async-context primitive use a synchronous stack fallback that clears the context before awaited continuations resume, avoiding cross-request leaks instead of pretending to isolate overlapping async work.
