...
`HttpApplicationAdapter.getRealtimeCapability()` reports whether a platform is server-backed, fetch-style, or unsupported for realtime protocol integration. Fetch-style capability version 1 describes request-upgrade support only. Version 2 additionally exposes `bindingInstallation`, a versioned protocol-neutral seam that lets first-party realtime packages install their binding before adapter `listen()` starts.

`createFetchStyleHttpAdapterRealtimeCapability(reason, options)` returns version 1 when `bindingInstallation` is omitted and version 2 when an installer is supplied. The installer accepts a protocol-owned binding or `undefined` for cleanup; the platform adapter remains responsible for parsing that boundary into its host-specific binding type and rejecting installation after startup. Application code should normally register `@fluojs/websockets` or `@fluojs/socket.io` modules rather than call this low-level adapter capability directly.
