...
`runWithRequestContext(...)` preserves the active context across awaited work when the host provides `AsyncLocalStorage` through `globalThis.AsyncLocalStorage` or Node's built-in `node:async_hooks` module. The root `@fluojs/http` import does not probe or instantiate async-context storage; helpers resolve storage lazily on first use and guard `process.getBuiltinModule(...)` failures. During that first lazy resolution, declared async callbacks wait for request-local storage before they run. Non-async callbacks keep synchronous return and throw behavior by using the synchronous fallback; if they return a promise, ambient context is intentionally unavailable after the callback's synchronous frame until storage has resolved. The helpers never replace `Promise.prototype.then`, so unrelated promise continuations cannot capture a pending request. Non-Node hosts without an async-context primitive use the same synchronous-only fallback.
