...
First-party package integrations may use the internal metadata registry to share explicitly versioned metadata stores across compatible copies in one JavaScript realm. The registry carries metadata only: it does not make application, provider, request, or container state process-global, and its entries remain weakly owned by their original metadata targets.
