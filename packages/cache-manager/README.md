...
On that supported HTTP path, eviction is deferred until `response.send(...)` settles successfully and the response reports that it committed. If send rejects, settles without a confirmed commit, or the request aborts before commit because of disconnect or shutdown, the deferred eviction is cancelled so the previous cached read result remains available. The interceptor does not infer commit from elapsed time: if a custom adapter or outer pipeline never invokes `response.send(...)`, automatic eviction does not run. Such paths must complete the standard response lifecycle or invalidate explicitly with `CacheService.del(...)`. Deferred eviction failures stay contained inside the interceptor, so cache-key factories or cache-store deletes cannot surface as post-response unhandled promise rejections.
