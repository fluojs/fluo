...
**Failures:** Import `CacheUpdateError` and `isCacheUpdateError` from `@fluojs/cache-manager`; use the predicate before branching on `error.code`. It validates the versioned cache owner contract across compatible duplicate copies and rejects plain lookalikes. Codes are: `unsupported` (no capability), `invalidated` (deletion/reset/expiry invalidates the snapshot), `closed` (service shutdown), `cancelled` (caller abort), or `conflict` (attempt budget exhausted). An invalid attempt limit also rejects with `RangeError`. Reducer, store, and serialization failures propagate unchanged; they are not retried as conflicts or wrapped in `CacheUpdateError`.
