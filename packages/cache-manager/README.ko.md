...
`@fluojs/cache-manager`는 Node.js `>=20.19.3 <21 || >=22.2.0 <27`을 지원하며 `engines.node`로 정확히 이 범위를 선언합니다. 이 package-owned 지원 계약에 따라 Node 21, Node 22.2.0 미만, 검증되지 않은 Node 27 이상은 제외됩니다. 이전 1.x 릴리스는 `engines.node >=20.0.0`을 광고했지만, 이는 실제 dependency floor와 일치한 적이 없습니다.
