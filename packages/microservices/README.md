...
- When `readerClient.xautoclaim` is available, Redis Streams reclaims pending request and event entries after `pendingReclaimIdleMs` of inactivity (default: `60_000`). Set that option to zero or a negative value to disable reclaiming; the adapter retains the `XAUTOCLAIM` cursor per consumer group until the next `close()`.
