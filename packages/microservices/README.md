...
- Platform status snapshots report transport resource ownership: TCP and gRPC report framework-owned listener/client resources, MQTT reports framework ownership only when it creates the client, and caller-owned broker collaborator transports remain externally managed.
