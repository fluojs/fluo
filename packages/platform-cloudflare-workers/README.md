...
During application shutdown, the adapter stops accepting new ingress immediately and gives active HTTP handlers a bounded 10-second drain window before `close()` fails with a timeout instead of hanging indefinitely. While that drain is still in progress, a concurrent `listen()` call rejects with `Cloudflare Workers adapter cannot listen while shutdown is still draining.` instead of reopening the Worker. Once closed, follow-up HTTP and WebSocket upgrade requests receive the same JSON `503` shutdown response until the adapter is explicitly listened again. Lazy hosts keep returning shutdown responses while a timed-out close is still draining, but they clear that temporary gate once the underlying close eventually settles so a later request can bootstrap a fresh Worker application.

## Quick Start

### Direct Application Creation
Create the concrete adapter first, then let `FluoFactory.create` own application
creation and call `app.listen()` to bind its dispatcher without opening a socket.
...
### Host-Owned Lazy Application
Use a `CloudflareWorkerApplicationHost` only when this isolate must own lazy
bootstrap, generation replacement, retry, and close/drain recovery. It is a
different capability from direct application creation, not a second Factory.
...
A successful `worker.close()` is intentionally restartable. It releases the
current host generation; a later `worker.fetch(...)` creates a fresh generation
in the isolate, rerunning bootstrap lifecycle hooks and reconstructing application
singleton providers. For an env-aware host, that generation uses the cached
configuration from the first environment without calling its factory again. Do
not treat `close()` as a terminal Worker shutdown signal. If an application needs
terminal behavior, it must own and enforce that state explicitly.
...
`worker.ready(env)` requires an explicit `env` for the same reason. The first
supplied environment determines the singleton bootstrap configuration; later
request environments still attach to `request.cloudflare.env`, but do not
reconfigure it. A successful `worker.close()` releases only the current
application generation: the next `ready(env)` or `fetch(...)` creates a fresh
application from the retained first-environment module and options without
calling the factory again. Use `CloudflareWorkerApplicationHost.create(AppModule,
options)` when bootstrap configuration is already available before the first
Worker request.

For the fixed-module host's request-bound `env` path, fetch-time bindings cannot
supply `ConfigModule.forRoot(...)` or singleton bootstrap providers. Read,
...
`FluoFactory.create(...)` and optimized for the Cloudflare environment.
`CloudflareWorkerHttpApplicationAdapter.create(...)` accepts adapter-owned parsing
and websocket-pair options; pass routing and middleware options to
`FluoFactory.create(...)` directly, or to the fixed-module host options when the
host-owned lazy capability is required.
...
- `CloudflareWorkerHttpApplicationAdapter.create(options)`: Creates the concrete Worker HTTP adapter.
...
`bodyParser` policy. `CloudflareWorkerHttpApplicationAdapter.create(...)` and
host configuration accept `'text'`, `'default'`, or a custom callback. Omission
retains MIME-based parsing and existing limits.
