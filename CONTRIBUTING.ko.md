...
1. **사전 요구 사항**: private development workspace용 Node.js `>=20.19.3 <21 || >=22.12.0 <27` 및 `pnpm`.
2. **의존성 설치**:
   ```sh
   pnpm install
   ```
...
Changeset은 PR이 공개 `@fluojs/*` 패키지의 public behavior, API surface, package contents, release metadata에 영향을 줄 때 필요합니다. Docs-only 수정, test-only coverage, 배포 동작을 바꾸지 않는 examples, 소비자에게 보이는 영향이 없는 내부 refactor에는 보통 필요하지 않습니다. 애매하다면 PR에 release impact를 설명하면 maintainer가 적절한 semver intent를 고르는 데 도움을 줄 수 있습니다.
...
- 논의가 필요한 문제라면 PR 전에 이슈를 여는 것을 권장하지만, 범위가 명확한 수정, 문서 업데이트, 작은 개선에는 이슈가 필수는 아닙니다.
- Codex나 Claude 같은 도구를 활용한 AI-assisted contribution도 환영합니다. 변경의 배경이 되는 이슈, discussion, 또는 짧은 문제 설명을 PR에 연결해 원래 컨텍스트를 유지해 주세요.
- 큰 기능 제안은 구현이 깊어지기 전에 이슈나 discussion을 열어 API 형태, behavioral contract, release impact를 maintainer와 먼저 맞추세요.
