...
- `JwtService.decode(token)`는 서명이나 클레임을 검증하지 않고 payload를 읽습니다. 반환된 객체는 검증되지 않은 입력(unverified input)이며 권한 결정에 사용해서는 안 됩니다. `JwtService.verify(token, policy?)`는 정규화된 `JwtPrincipal`을 반환합니다. claims-only caller는 `(await jwt.verify(token)).claims`를 사용하고, 호출 단위 `algorithms`, `audience`, `issuer`, `clockSkewSeconds`, `maxAge`, `requireExp`는 optional policy로 전달하세요. `decode()`는 진단 및 비권위적 검사에만 사용됩니다.
