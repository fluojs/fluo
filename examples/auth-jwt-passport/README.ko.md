...
- `JwtService.decode(token)`는 서명이나 클레임을 검증하지 않고 payload를 읽습니다. 반환된 객체는 검증되지 않은 입력(unverified input)이며 권한 결정에 사용해서는 안 됩니다. `JwtService.verify(token, options)`는 검증된 클레임을 반환하며 `JwtPrincipal`을 반환하지는 않습니다. 정규화된 `JwtPrincipal` 신원이 필요하면 `DefaultJwtVerifier.verifyAccessToken(token)`을 사용하세요. `decode()`는 진단 및 비권위적 검사에만 사용됩니다.
