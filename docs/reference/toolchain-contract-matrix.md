...
| **Diagnostics (JSON)** | `fluo inspect <module-path> --json` | Exports runtime-produced graph, readiness, health, diagnostics, and compiled route inspection data in exactly one JSON document on stdout. Runtime diagnostics are written to stderr. Route entries contain effective path/version and parameter names, with `kind: 'react-page'` for React pages and `kind: 'http'` for ordinary handlers. JSON is also the default output mode when no output mode is selected. `--timing` may be used with or without an explicit `--json` flag to include bootstrap timing diagnostics next to the snapshot. |
