...
| **React page type generation** | `fluo typegen <module-path> --output <path>` | Bootstraps the selected module with runtime logs suppressed, projects authoritative compiled `HandlerDescriptor` values through `createReactPageCatalog(...)`, and atomically publishes a versioned artifact. Reports `CREATE`, `UPDATE`, or `UNCHANGED`; identical output is not rewritten. |
