...
| **Vitest** | `v4.1+` | Root, package-local workspace, and generated non-Deno toolchains declare `vitest` `^4.1.11`. Generated projects pair it with `@vitest/coverage-v8` `^4.1.11`, `fluoDecoratorsPlugin({ sourceMaps: true, transformBoundary: 'test' })`, and `@fluojs/core/metadata-preload`. |
| **Node.js** | Root workspace and Node-bound public packages: `>=24.0.0 <27` | Exact `24.0.0`, latest `24.x`, and latest `26.x` run a frozen install followed by the same full build, typecheck, lint, and test coverage as local `pnpm verify`, distributed across jobs, plus generated starter checks. Latest `24.x` runs `pnpm verify:docs` once and owns release automation; latest `26.x` verifies forward compatibility only. Node 22 is absent from the new-major matrix. The eight portable engine omissions and Bun/Deno/Workers runtime lanes remain independent. See [support and migration](./node-support.md). |
...
| **Project Creation (React SSR + Vite)** | `fluo new my-app --starter react-vite-ssr` | Generates the fixed Node.js + Fastify HTTP React starter with explicit client/server entries, Vite manifest loading at `src/main.ts`, decorator-bearing declarations in `src/app.ts`, JSX rendering in `.tsx` modules, application-owned page rendering, direct JSX, hydration, real anchors, full-document navigation, and focused Vitest/Playwright checks. RSC, Server Functions, file routing, client route tables, prefetch, and data caches are excluded. |
...
| **Resource Generation** | `fluo g <type>` | Produces consistent naming suffixes (`.service.ts`, `.controller.ts`). Request DTOs may target an explicit feature directory with `fluo g req users CreateUser`. `fluo g module User --with-slice-test` emits `src/users/user.slice.test.ts`, `fluo g resource User --with-slice-test` emits resource-level provider override coverage in `src/users/user.slice.test.ts`, and `fluo g e2e users` emits `test/users.e2e.test.ts` with `Test.createApp({ rootModule })`. |
| **Diagnostics (JSON)** | `fluo inspect <module-path> --format json` or `--json` | Exports runtime-produced graph, readiness, health, diagnostics, and compiled route inspection data in exactly one JSON document on stdout. Runtime diagnostics are written to stderr. `--format json` is explicitly equivalent to `--json`; other `--format` values are rejected at the CLI boundary. Route entries contain effective path/version and parameter names, with `kind: 'react-page'` for React pages and `kind: 'http'` for ordinary handlers. JSON is also the default output mode when no output mode is selected. `--timing` may be used with or without an explicit JSON option to include bootstrap timing diagnostics next to the snapshot. |
...
| **Slice test** | `.slice.test.ts` | `users/user.slice.test.ts` from `fluo g module User --with-slice-test` or `fluo g resource User --with-slice-test` |
...
| **Vite app transform** | `@fluojs/vite` | Generated `vite.config.ts` applies `fluoDecoratorsPlugin()` to application `.ts` files, skips tests/declarations/dependencies/non-TypeScript files, runs `@babel/plugin-proposal-decorators` plus `@babel/preset-typescript`, and reports missing Babel peers from the transform hook. The React SSR + Vite starter therefore keeps decorator-bearing declarations in `src/app.ts` while JSX remains in `.tsx` modules. Importing `@fluojs/vite` or creating the plugin does not load `@babel/core`. The workspace Vite 8.2.2/Rolldown integration gate verifies the pre-stage ordering and executes the bundled field-decorator metadata result. |
| **Bundle** | Vite | Bundles generated applications for the selected runtime. |
| **Next.js app transform and bundle** | [`@fluojs/platform-nextjs/next-config`](../../packages/platform-nextjs/README.md#decorator-compiler-wiring) + Turbopack | For existing Next.js 16.x applications (peer `>=16.0.0 <17`) on Node.js `>=24.0.0 <27`, `withFluoNextBackend()` appends the packaged `decorators-loader` to server-side application `*.ts` rules, excluding browser and dependency files. It preserves existing config and rules, runs Babel decorators `{ version: '2023-11' }` plus the TypeScript preset, and returns JavaScript to Turbopack. Decorated declarations stay in `.ts`, not `.tsx`. This packaged integration supports Turbopack only, not webpack or the Next.js Edge Runtime; it is not a Vite plugin or a CLI starter. |
| **Validate** | `@fluojs/vite` + Vitest | Generated `vitest.config.ts` applies `fluoDecoratorsPlugin({ sourceMaps: true, transformBoundary: 'test' })` and `@fluojs/core/metadata-preload`, so test/spec and application modules share the canonical decorator transform while declarations and dependencies remain excluded. |
