...
| **@fluojs/vite** | `v1.0+`; Node.js `>=20.0.0` | Generated non-Deno `vite.config.ts` files import `fluoDecoratorsPlugin()` from `@fluojs/vite`; the plugin owns Vite application-file decorator transforms, requires Vite `>=6.2.0`, and keeps Babel peer loading lazy until an eligible transform runs. |
