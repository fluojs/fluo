...
- **`@fluojs/discord`**: standalone으로도 동작하고 공식 알림 채널로도 등록할 수 있는 webhook-first Discord 전달 코어입니다. `DiscordModule.forRoot(...)` / `forRootAsync(...)`, 직접 `DiscordService` 전달, `DISCORD_CHANNEL` notifications integration, lifecycle-gated send, transport kind와 resource ownership diagnostics, optional `verifyOnModuleInit` bootstrap verification, `DiscordTemplateRenderer` notification template rendering, `DiscordService.createPlatformStatusSnapshot()`과 `createDiscordPlatformStatusSnapshot(...)` status snapshot, 직접 `process.env`를 읽지 않는 transport-agnostic boundary를 문서화합니다.
