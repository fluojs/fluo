...
- **`@fluojs/drizzle`**: Node의 `node:async_hooks`를 사용하는 Node.js 20+ Drizzle ORM lifecycle 및 ALS 기반 transaction context이며, package manifest는 `engines.node >=20.0.0`을 선언합니다. `@Transaction()`, `DrizzleDatabase<TDatabase>`, `DrizzleDatabaseFacade<TDatabase>`, status snapshot helper, 호환성 전용 provider wiring helper인 `DrizzleDatabase.createFacade(...)`를 노출하며, 애플리케이션 등록은 `DrizzleModule.forRoot(...)` / `forRootAsync(...)`에 남겨 둡니다. Drizzle의 Bun SQL, Cloudflare D1 등 비 Node driver는 비 Node transaction-context adapter가 문서화되기 전까지 이 wrapper 범위 밖입니다.
