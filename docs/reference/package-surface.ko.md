...
- **`@fluojs/terminus`**: `TerminusModule.forRoot(...)` 기반 aggregated health/readiness diagnostics, indicator/provider registration, 느린 indicator를 위한 execution timeout, runtime platform health/readiness contributor, `@fluojs/terminus/node`의 Node memory/disk indicator, 그리고 root package가 optional Redis peer에 의존하지 않도록 분리된 `@fluojs/terminus/redis`의 Redis lifecycle-aware `PING` diagnostics를 담당합니다.
