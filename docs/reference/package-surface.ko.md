...
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` fetch-style 어댑터 seam을 재사용해 fetch-style 런타임 패리티를 제공하고, 이후 Bun 전용 작업을 위한 contract-only raw websocket 확장 seam도 노출하는 HTTP 어댑터.
- **`@konekti/platform-deno`**: 공유 `@konekti/runtime/web` fetch-style 어댑터 seam과 이후 Deno 전용 작업을 위한 contract-only raw websocket 확장 seam 위에 구축된 Deno `Deno.serve(...)` 어댑터.
