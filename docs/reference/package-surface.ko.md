...
- **`@fluojs/metrics`**: `MetricsModule.forRoot(...)`를 통한 Prometheus scrape endpoint 등록, optional HTTP request collector, runtime platform telemetry Gauge, 기본적으로 격리되고 명시적으로 공유할 수 있는 `prom-client` Registry ownership, custom application counter/gauge/histogram을 위한 `MetricsService`, package integration용 low-level `METER_PROVIDER` / `PrometheusMeterProvider` bridge를 담당합니다.
