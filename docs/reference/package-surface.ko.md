...
- **`@fluojs/mongoose`**: Mongoose 라이프사이클 통합과 `@Transaction()`을 통한 ALS/session 인지형 transaction boundary, 명시적 `requestTransaction(...)` 요청 경계, 모델 작업을 위한 `currentSession()` escape-hatch 접근, 사용 가능한 경우 `connection.transaction(...)`을 통한 ambient-session 위임, 활성 요청/session drain 상태를 보고하는 shutdown snapshot을 담당합니다.
