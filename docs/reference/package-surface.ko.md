...
- **`@konekti/platform-bun`**: 공용 `@konekti/runtime/web` fetch-style 어댑터 seam을 재사용해 fetch-style 런타임 패리티를 제공하고, Bun 네이티브 request-upgrade 호스팅을 위한 supported raw websocket 확장 seam을 노출하는 HTTP 어댑터입니다. 공식 raw websocket 호스팅은 전용 `@konekti/websockets/bun` 바인딩이 제공하고, Bun 전용 Socket.IO 호스팅은 이제 `@konekti/socket.io`와 공식 `@socket.io/bun-engine` 경로를 통해 제공되며, `@konekti/websockets/node`는 계속 Node upgrade-listener 전용으로 유지됩니다.
...
- **`@konekti/socket.io`**: 공용 Konekti 런타임과 websocket 데코레이터 위에 구축된 Socket.IO v4 게이트웨이 어댑터이며, raw Node 서버 직접 소유를 가정하지 않고 플랫폼이 선택한 realtime capability를 소비합니다. 현재 정직한 지원 주장은 문서화·테스트된 server-backed 어댑터인 `@konekti/platform-nodejs`, `@konekti/platform-fastify`, `@konekti/platform-express`와 공식 `@socket.io/bun-engine` 호스트 경로를 사용하는 `@konekti/platform-bun`으로 제한됩니다.
