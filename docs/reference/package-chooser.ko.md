...
| Express host 호환이 필요함 | `@fluojs/platform-express` | `engines.node >=20.19.3`을 선언하는 Node.js 20.19.3+ 패키지이며 first-class `fluo new` 애플리케이션 스타터로도 제공됩니다. Express host 교체는 NestJS legacy decorator 또는 reflection metadata 동작을 유지하지 않으므로 TC39 표준 데코레이터와 명시적 DI/module wiring으로 먼저 마이그레이션하세요. Application pipeline에는 fluo `Middleware`를 사용하고, migration 전용 Express/Connect handler는 adapter의 pre-router `nativeMiddleware` 옵션에 등록하거나 portable fluo 계약 뒤에 감싸세요. |
