...
| portable 정적 에셋이 필요함 | `@fluojs/http` + `@fluojs/platform-nodejs` | `@fluojs/http`는 `createStaticAssetsMiddleware(...)`를 소유하고, Node subpath만 `createNodeFileSystemAssetSource(...)`를 소유합니다. Web, Bun, Deno, Workers 애플리케이션은 자체 `StaticAssetSource`를 제공합니다. |
...
| Express host 호환이 필요함 | `@fluojs/platform-express` | 정확한 `engines.node >=20.19.3 <21 || >=22.2.0 <27` 범위를 선언하는 Node.js 패키지이며 first-class `fluo new` 애플리케이션 스타터로도 제공됩니다. Adapter가 자체 Express application을 생성하므로 기존 Express application을 채택하거나 재사용할 수 없습니다. Express host 교체는 NestJS legacy decorator 또는 reflection metadata 동작을 유지하지 않으므로 TC39 표준 데코레이터와 명시적 DI/module wiring으로 먼저 마이그레이션하세요. Application pipeline에는 fluo `Middleware`를 사용하고 이식 가능한 동작은 해당 계약으로 재작성하는 방식을 우선하세요. Migration 전용 Express/Connect handler는 adapter의 pre-router `nativeMiddleware` 옵션으로 construction-time에 제공해야 하며 post-bootstrap `use(...)` mutation은 지원하지 않습니다. |
| Node.js HTTP를 직접 제어해야 함 | `@fluojs/platform-nodejs` | 정확한 `engines.node >=20.19.3 <21 || >=22.2.0 <27` 범위를 사용하는 first-class `fluo new` 애플리케이션 스타터로도 제공됩니다. |
...
| Route-local layout 또는 SSR Suspense fallback composition이 필요함 | `@fluojs/react` | Application `renderPage` 하나를 등록한 뒤 React router class나 `@Path(...)` method에 `@PageLayout(...)` 또는 `@SuspenseFallback(...)` component reference를 선언합니다. Renderer는 HTTP matching 이후 inherited class/method policy와 활성 request context를 받습니다. Segment-tree loading/error/not-found semantic, handler-await pending UI, client navigation pending UI, URL-prefix layout ancestry를 추가하지 않습니다. |
| Stable SSR용 hydration asset이 필요함 | `@fluojs/react` 또는 `@fluojs/react/vite` | `createReactServerEntry(...)`에 명시적인 `bootstrapScripts`, `bootstrapModules`, 신뢰된 `bootstrapScriptContent`, `nonce`, `identifierPrefix`, 신뢰된 `assetMap` snapshot을 직접 전달하거나, `@fluojs/react/vite`로 이미 로드한 Vite manifest를 같은 hydration contract용 deterministic CSS, JavaScript bootstrap asset, asset map, trusted bootstrap data, diagnostics로 파싱합니다. 이 stable path는 RSC나 Server Functions를 암시하지 않습니다. |
...
| Application-owned renderer/build adapter로 RSC를 prototype해야 함 | `@fluojs/react/experimental/rsc` + `@fluojs/http` | React, React DOM, Flight renderer를 정확히 `19.2.6`으로 고정하고, 명시적인 client-reference 및 server-to-client module map을 제공한 뒤, application-encoded Flight payload를 일반 fluo HTTP handler에서 반환합니다. 이 subpath는 불안정하고 built-in renderer/plugin이 없으며 별도 router를 만들지 않습니다. |
| Mutation 지향 experimental Server Functions가 필요함 | `@fluojs/react/experimental/rsc` + `@fluojs/http` | Signed action registry를 만들고 `invoke(context)`를 명시적인 guarded `@Post(...)` endpoint에 mount하며, exact allowed origin, application-owned Web Crypto provider, 32 byte 이상의 secret을 전달하고 platform pre-parse body limit을 설정합니다. Argument는 계속 신뢰할 수 없는 값이며 authorization은 action 내부 또는 주변 guard에 속합니다. Query loader, data cache, generic mutation convention, HTTP-dispatch bypass가 아닙니다. |
| Stable RSC import가 필요함 | unavailable | `@fluojs/react/rsc`는 blocked 상태입니다. Prototype에는 experimental path만 사용하고 [React RSC graduation policy](../contracts/react-rsc-graduation.ko.md)를 따르세요. Stable subpath를 만들기 전에 renderer/version, manifest/transport, hydration/data-safety, route/navigation, dual-import, docs, release evidence가 필요합니다. |
...
| Node.js에서 Drizzle 기반 관계형 접근이 필요함 | `@fluojs/drizzle` | Node.js `>=20.19.3 <21 || >=22.2.0 <27`용 Drizzle ORM 통합에 사용합니다. Root wrapper는 Node의 `node:async_hooks` transaction context를 사용하며 패키지 자체의 지원 범위를 요구합니다. Node 21과 Node 27 이상은 지원하지 않습니다. Bun SQL, Cloudflare D1 같은 비 Node Drizzle driver는 비 Node context adapter가 문서화되기 전까지 현재 fluo wrapper 범위 밖입니다. 그런 런타임에서는 이 wrapper를 import하지 말고 raw Drizzle driver handle을 런타임별 fluo provider(`useFactory` 또는 `useValue`) 뒤에 등록하세요. |
| Node.js에서 도큐먼트 데이터베이스 접근이 필요함 | `@fluojs/mongoose` | Node.js `>=20.19.3 <21 || >=22.2.0 <27`용 Mongoose 통합에 사용합니다. Root wrapper는 Node의 `node:async_hooks` transaction context를 사용하며 패키지 자체의 지원 범위를 요구합니다. Node 21과 Node 27 이상은 지원하지 않습니다. 비 Node 런타임에서는 runtime-specific transaction-context adapter가 문서화되기 전까지 raw Mongoose-compatible handle을 애플리케이션 소유 provider 뒤에 등록하세요. |
...
| Socket.IO 시맨틱이 필요함 | `@fluojs/socket.io` + `@fluojs/websockets` | Node.js `>=20.19.3 <21 || >=22.2.0 <27` server-backed adapter 또는 공식 Bun engine path의 Socket.IO 호환 통합에 사용합니다. Socket.IO gateway authoring은 `@fluojs/websockets`의 `@WebSocketGateway`, `@OnMessage`, lifecycle decorator를 재사용하므로 companion 패키지를 함께 설치하세요. Native Socket.IO emit 또는 ACK 중심 migration seam에는 `SOCKETIO_SERVER`를 주입합니다. Deno와 Workers는 지원하지 않고 Bun은 static CORS shape를 요구하며 Socket.IO는 모든 runtime에서 `@WebSocketGateway({ serverBacked })`를 거부합니다. |
| optional cross-process fan-out이 있는 in-process domain event가 필요함 | `@fluojs/event-bus` + optional `@fluojs/event-bus/redis`, `@fluojs/redis`, `ioredis` peer | 반응이 process boundary를 넘어야 할 때만 Redis Pub/Sub을 사용합니다. `ioredis`를 설치하고 transport 전용 `publishClient`와 `subscribeClient`를 서로 다른 instance로 생성하며, event-bus teardown 후 client lifecycle owner가 두 client를 닫습니다. |
