...
| **Node.js** | 루트 워크스페이스 및 Node HTTP listener는 `>=20.19.3` | RFC `QUERY`가 fluo dispatch에 도달하는 데 필요한 최소 listener 기준선입니다. 생성된 Node HTTP 및 mixed starter도 같은 하한을 사용하며, Node microservice-only starter와 CLI process는 독립적으로 문서화된 tooling floor를 유지합니다. Bun, Deno, Cloudflare Workers 어댑터는 패키지 메타데이터가 비-Node 런타임 계약과 일치하도록 `engines.node`를 의도적으로 생략합니다. |
