...
| **Node.js** | Private root workspace: `>=20.19.3 <21 || >=22.12.0 <27` | Private development workspace는 Node 20.19.3 floor를 유지하고, workspace에 고정된 Vite 8.2.2가 `^20.19.0 || >=22.12.0`을 요구하므로 Node 22 line에서는 Node 22.12.0 이상을 요구합니다. 이 root-only toolchain floor는 public package 계약을 변경하지 않습니다. Node HTTP listener package와 생성된 Node HTTP/mixed starter는 `>=20.19.3 <21 || >=22.2.0 <27`을 유지하고, 다른 package와 starter도 각각 독립적으로 문서화된 runtime floor를 유지합니다. |
