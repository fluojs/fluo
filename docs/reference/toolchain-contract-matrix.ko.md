...
| **CLI self-update prompt** | interactive TTY에서 `fluo <command>` 실행 | 캐시된 npm `latest` dist-tag로 `@fluojs/cli` 버전을 확인합니다. 새 버전이 있으면 현재 global install을 소유한 것으로 보이는 package-manager 명령(`npm install -g`, `pnpm add -g`, `bun add -g`, `yarn global add`, npm fallback 포함) 실행 여부를 먼저 묻고, 승인된 경우 같은 argv로 업데이트된 `fluo` 바이너리를 다시 실행합니다. `fluo new`와 `fluo create`는 스캐폴딩 전에 fresh cache를 우회해 첫 프로젝트 생성이 새로 배포된 CLI 동작을 더 빨리 반영하게 하며, 다른 non-help 명령은 일반 cache TTL을 유지합니다. CI, non-TTY, npm-script, `FLUO_NO_UPDATE_CHECK=1`, `--no-update-check`, 순수 help/version 경로, registry 실패에서는 prompt를 건너뜁니다. |
| **개발 watch restart** | 생성된 애플리케이션 프로젝트에서 `fluo dev` 실행 | Node 스타터는 기본적으로 fluo가 소유한 restart runner를 사용합니다. 이 runner는 filesystem burst를 debounce하고, restart 전에 content hash를 비교하며, noisy output/cache/editor 경로를 무시하고, 앱 로그 전용 출력·색상 보존·restart clear/header 동작을 일관되게 유지합니다. Primary 또는 fallback watcher에서 terminal error가 발생하거나, 필수 source target이 watcher 등록 전에 없거나 읽을 수 없게 되는 경우, fallback 획득이 전부 실패하는 경우, sibling watcher가 성공한 뒤 일부 필수 watcher 획득이 실패하는 경우, runtime에서 새 directory를 발견한 뒤 필수 watcher 획득이 실패하는 경우, 또는 traversal이 source coverage를 하나도 확보하지 못하는 경우에는 모든 watcher를 닫고, pending restart 작업을 비우며, signal handler를 제거하고, 기존 제한 시간 안에 앱 child를 종료한 뒤 exit code `1`로 종료합니다. 이 동작은 true HMR이 아니라 restart-on-watch입니다. Node 디버깅에는 `fluo dev --raw-watch` 또는 `FLUO_DEV_RAW_WATCH=1`로 runtime-native `node --watch` 명령을 복원할 수 있습니다. Bun/Deno/Workers는 `fluo dev` 추상성을 유지하되 runtime-owned watch/reload 동작(`bun --watch src/main.ts`, `deno run --watch --allow-env --allow-net src/main.ts`, `wrangler dev --show-interactive-dev-session=false`)을 기본값으로 사용합니다. Deno는 generated `AppModule`이 `Deno.env.toObject()`로 application-owned value를 snapshot하므로 broad env access를 유지합니다. fluo 소유 restart runner가 필요하면 `fluo dev --runner fluo` 또는 `FLUO_DEV_RUNNER=fluo`를 사용합니다. |
| **HTTP starter testing layout** | 생성된 non-Deno HTTP 애플리케이션 starter | 빠른 unit test는 `src/greeting/` 아래에 생성하고, slice/module graph test는 `src/greeting/greeting.slice.test.ts`에 두며, app dispatch test는 `src/app.test.ts`, 기본 e2e 스타일 suite는 `Test.createApp({ rootModule })`와 `app.request(...).send()`를 사용하는 `test/app.e2e.test.ts`에 생성합니다. 생성된 Vitest config는 `src/**/*.test.ts`와 `test/**/*.test.ts`를 모두 포함하고, 지원되는 script는 `test`, `test:watch`, `test:cov`, `test:e2e`를 포함합니다. 기존 `src/app.e2e.test.ts` 사용자는 request helper를 바꾸지 않고 해당 test를 `test/app.e2e.test.ts`로 이동할 수 있습니다. |
...
| **비-Node production lifecycle** | 생성된 Bun, Deno, Cloudflare Workers package script | Bun 생성 프로젝트는 `dev: fluo dev`를 유지한 뒤 `bun build ./src/main.ts --outdir ./dist --target bun`으로 빌드하고 `bun dist/main.js`로 시작합니다. Deno 생성 프로젝트는 `dev: fluo dev`를 유지한 뒤 `deno compile --allow-env --allow-net --output dist/app src/main.ts`로 빌드하고 `./dist/app`을 실행합니다. Generated `AppModule`이 complete application-owned environment snapshot을 읽으므로 broad env access를 binary에 포함합니다. Signal listener에는 별도의 Deno permission이 필요하지 않습니다. Cloudflare Workers 생성 프로젝트는 `dev: fluo dev`를 유지하고, `wrangler deploy --dry-run`으로 빌드 검증하며, `preview: wrangler dev --remote --show-interactive-dev-session=false`, `deploy: wrangler deploy`를 노출하고, Wrangler native publish flow를 사용하도록 의도적으로 `start`를 생략합니다. |
...
| **진단 (JSON)** | `fluo inspect <module-path> --format json` 또는 `--json` | 런타임이 생산한 graph, readiness, health, diagnostics, compiled route inspection data를 stdout에 정확히 하나의 JSON document로 내보냅니다. 런타임 diagnostics는 stderr에 씁니다. `--format json`은 명시적으로 `--json`과 동등하며 다른 `--format` 값은 CLI boundary에서 거부합니다. Route entry는 effective path/version과 parameter name을 포함하며 React page는 `kind: 'react-page'`, 일반 handler는 `kind: 'http'`를 사용합니다. 출력 모드를 고르지 않으면 JSON이 기본 출력 모드입니다. `--timing`은 명시적인 JSON option 유무와 관계없이 snapshot 옆에 bootstrap timing diagnostics를 포함할 수 있습니다. |
...
| --- | --- | --- |
| `--json` | 런타임 platform shell과 authoritative compiled HTTP descriptor로 조합한 `RuntimeInspectionSnapshot` JSON. | Studio, script, support triage가 사용할 수 있는 안정적인 machine-readable snapshot입니다. `routes[]`에는 route identity와 parameter name만 포함되고 request value, body, cookie, header, query value는 제외됩니다. `--timing`을 함께 쓰면 payload는 `{ snapshot, timing }`이 되며, `timing`은 versioned bootstrap timing diagnostics입니다. |
...
| **Slice test** | `.slice.test.ts` | `fluo g module User --with-slice-test` 또는 `fluo g resource User --with-slice-test`가 생성하는 `users/user.slice.test.ts` |
