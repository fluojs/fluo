...
- **`@konekti/platform-bun`**: Bun-based HTTP adapter that reuses the shared `@konekti/runtime/web` fetch-style adapter seam for fetch-style runtime parity and now exposes the shared contract-only raw websocket expansion seam for future Bun-specific work.
- **`@konekti/platform-deno`**: Deno `Deno.serve(...)` adapter built on the shared `@konekti/runtime/web` fetch-style adapter seam and the shared contract-only raw websocket expansion seam for future Deno-specific work.
