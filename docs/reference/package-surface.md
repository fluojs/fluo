...
- **`@fluojs/mongoose`**: Mongoose lifecycle integration with ALS/session-aware transaction boundaries via `@Transaction()`, explicit `requestTransaction(...)` request boundaries, explicit `currentSession()` access for model operations, ambient-session delegation through `connection.transaction(...)` when available, and shutdown snapshots that report active request/session drain state.
