...
- **`@fluojs/config`**: Environment-aware configuration loading and typed access. The root import stays safe for in-memory consumers by resolving Node filesystem/path/crypto builtins lazily only when env-file loading or watch mode runs; those env-file paths remain compatible with the package-level Node.js 20.0.0 engine range by falling back through `node:module` when `process.getBuiltinModule(...)` is unavailable.
