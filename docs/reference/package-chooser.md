...
| Need to prototype RSC with an application-owned renderer/build adapter | `@fluojs/react/experimental/rsc` + `@fluojs/http` | Pin React, React DOM, and the Flight renderer to exactly `19.2.6`; provide explicit client-reference and server-to-client module maps; then return application-encoded Flight payloads through ordinary fluo HTTP handlers. The subpath is unstable, has no Server Functions or built-in renderer/plugin, and does not create a separate router. |
