...
| Need portable static assets | `@fluojs/http` + `@fluojs/platform-nodejs` | `@fluojs/http` owns `createStaticAssetsMiddleware(...)`; only the Node subpath owns `createNodeFileSystemAssetSource(...)`. Web, Bun, Deno, and Workers applications provide their own `StaticAssetSource`. |
| Need GraphQL endpoints | `@fluojs/graphql` | Add on top of the HTTP stack. |
| Need the default Node.js adapter | `@fluojs/platform-fastify` | Recommended starter path for projects on Node.js `>=24.0.0 <27`; the package declares that exact `engines.node` range so listener-level RFC `QUERY` reaches fluo dispatch. |
...
| Need Express host compatibility | `@fluojs/platform-express` | Node.js `>=24.0.0 <27` package with that exact `engines.node` range, also available as a first-class `fluo new` application starter. The adapter constructs its Express application and cannot adopt an existing Express application. An Express host swap does not retain NestJS legacy decorator or reflection-metadata behavior, so migrate to TC39 standard decorators and explicit DI/module wiring first. Use fluo `Middleware` for the application pipeline and prefer rewriting portable behavior to that contract. Supply migration-only Express/Connect handlers through the adapter's pre-router `nativeMiddleware` option at construction time; post-bootstrap `use(...)` mutation is unsupported. |
| Need direct Node.js HTTP control | `@fluojs/platform-nodejs` | Also available as a first-class `fluo new` application starter on Node.js `>=24.0.0 <27` with that exact `engines.node` range. |
...
| Need route-local layout or SSR Suspense fallback composition | `@fluojs/react` | Register one application `renderPage`, then declare component references with `@PageLayout(...)` or `@SuspenseFallback(...)` on React router classes or `@Path(...)` methods. The renderer receives inherited class/method policies and the active request context after HTTP matching. This does not add segment-tree loading/error/not-found semantics, handler-await pending UI, client navigation pending UI, or URL-prefix layout ancestry. |
...
| condition | package choice | notes |
| --- | --- | --- |
| Need Vite to build TypeScript with TC39 standard decorators | `@fluojs/vite` | Use `fluoDecoratorsPlugin()` from `@fluojs/vite` in both `vite.config.ts` and `vitest.config.ts`; use `{ sourceMaps: true, transformBoundary: 'test' }` in the Vitest config. The plugin applies Babel's `@babel/plugin-proposal-decorators` transform with `{ version: '2023-11' }` plus `@babel/preset-typescript`, and every transformed module with decorators preloads `@fluojs/core/metadata-preload` before its declarations evaluate. It preserves fluo's Vite file-boundary skips for declarations, dependencies, and non-TypeScript files, lazily loads Babel only for eligible transforms, and reports missing Babel peers from the transform hook. |
...
| Need Drizzle-based relational access on Node.js | `@fluojs/drizzle` | Use for Node.js `>=24.0.0 <27` Drizzle ORM integration. The root wrapper uses Node's `node:async_hooks` transaction context and uses a package-owned Node support contract; Node versions below 24 and Node 27+ are unsupported. Drizzle's Bun SQL, Cloudflare D1, and other non-Node drivers are outside the current fluo wrapper until a non-Node context adapter is documented. For those runtimes, register the raw Drizzle driver handle behind runtime-specific fluo providers (`useFactory` or `useValue`) instead of importing this wrapper. |
...
| Need Socket.IO semantics | `@fluojs/socket.io` + `@fluojs/websockets` | Use for Socket.IO-compatible integrations on Node.js `>=24.0.0 <27` server-backed adapters or the official Bun engine path. Install the websockets companion because Socket.IO gateway authoring reuses `@WebSocketGateway`, `@OnMessage`, and lifecycle decorators from `@fluojs/websockets`; inject `SOCKETIO_SERVER` for native Socket.IO emits/ACK-oriented migration seams. Deno and Workers are unsupported, Bun requires static CORS shapes, and Socket.IO rejects `@WebSocketGateway({ serverBacked })` on every runtime. |
| Need in-process domain events with optional cross-process fan-out | `@fluojs/event-bus` + optional `@fluojs/event-bus/redis`, `@fluojs/redis`, and `ioredis` peer | Use Redis Pub/Sub only when reactions must cross process boundaries. Install `ioredis`, then create dedicated, separate `publishClient` and `subscribeClient` instances; the client lifecycle owner closes both after event-bus teardown. |
...
| Need Node.js memory or disk health indicators | `@fluojs/terminus/node` | Dedicated Node-specific memory/disk indicators. |
