...
| Need Express host compatibility | `@fluojs/platform-express` | Also available as a first-class `fluo new` application starter on Node.js. Use fluo `Middleware` for the application pipeline; register migration-only Express/Connect handlers through the adapter's pre-router `nativeMiddleware` option or wrap them behind the portable fluo contract. |
