...
| **React SSR + Vite 스타터** | **스캐폴딩됨** | `--starter react-vite-ssr`로 생성되는 Node.js + Fastify HTTP. 명시적인 client/server entry, 분리된 Vite config, manifest-fed hydration, HTTP-owned page router, 집중된 SSR/hydration/browser test를 생성합니다. | 검증된 HTTP-first React 경로에는 이 named starter를 사용합니다. Manifest loading은 `src/main.ts`에 유지하고 real anchor와 full-document navigation을 생성하며 RSC, Server Functions, file routing, client route table, prefetch, data cache는 제외합니다. |
