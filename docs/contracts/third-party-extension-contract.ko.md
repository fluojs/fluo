...
| HTTP listener adapters | `packages/http/src/adapter.ts` | 어댑터는 `listen(dispatcher)`와 `close(signal?)`를 반드시 구현해야 합니다. `getServer?()`와 `getRealtimeCapability?()`는 선택 사항이지만, 노출할 경우 문서화된 capability shape를 보존해야 합니다. Fetch-style capability version 2 host는 `listen()` 전에 versioned `bindingInstallation` seam으로 protocol binding을 설치해야 합니다. |
