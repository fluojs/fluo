...
`main` is the stable release lane. Pull requests targeting `main` and the release workflow both run the changeset lane gate; `.github/workflows/release.yml` runs `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=${{ github.event.before }}` before the Changesets action can create or publish a Version Packages PR. The stable gate accepts `patch`, `minor`, and `major` changesets plus matching generated `packages/*/package.json` version deltas, while still rejecting malformed release metadata. Semver intent remains authoritative: do not downgrade a changeset to satisfy a lane check. Major changesets require explicit maintainer approval and consumer-facing migration notes before merge.
...
- `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=<ref>` verifies pending changesets and generated package version deltas for the stable `main` lane without downgrading valid semver intent.
