...
`main` is the stable patch lane. Pull requests targeting `main` and the release workflow both run the changeset lane gate; `.github/workflows/release.yml` runs `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=${{ github.event.before }}` before the Changesets action can create or publish a Version Packages PR. The gate rejects pending `.changeset/*.md` files that declare `minor` or `major` bumps, and it also rejects generated `packages/*/package.json` version deltas that would publish a minor or major bump from an already-created Version Packages PR. Minor and major changesets must be held on a dedicated release branch or prerelease branch until maintainers intentionally promote that lane back to stable release flow. Patch changesets may continue through `main` and publish under `latest`.
...
- `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=<ref>` enforces the `main` patch-only lane in PR CI and again before the Changesets action can create or publish a stable Version Packages PR, including generated package version deltas.
