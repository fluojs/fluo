...
Release reviewers must compare `.changeset/*.md`, generated package `CHANGELOG.md` sections, affected package `README.md` contracts, and changed package `engines.node` ranges against the previous published manifest tag before accepting semver metadata. If an Official package narrows `engines.node`, the changeset must be `major` and include either a non-empty `Migration:` paragraph or a structured `Migration Guide` / `Upgrade Guidance` section with consumer-facing replacement-runtime guidance; `verify:changeset-release-lane` enforces that requirement. After Changesets consumes pending metadata, generated major version deltas remain valid release output without a pending intent. If a `@fluojs/cli` entry says it adds, exposes, supports, introduces, enables, or provides a public CLI, starter, inspect, or programmatic surface documented in the README, the entry must be `minor`; patch metadata must be corrected before the Version Packages PR is merged.
