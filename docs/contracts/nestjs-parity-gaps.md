...
| `class-validator`, class-transformer `@Type(...)`, and reflection-first DTO contracts as the default validation model | Intentionally replaced by explicit fluo validation contracts. Nested DTO constructors are passed to `@ValidateNested(...)`; mapped helpers are imported from `@fluojs/validation` and follow fluo's documented class-level rule semantics. | `docs/getting-started/migrate-from-nestjs.md`, `packages/validation/README.md` |
