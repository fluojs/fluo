...
| `ValidationPipe` whitelist/forbid behavior and class-validator `groups` / `always` execution | Not implemented. Ordinary validators skip `null` / `undefined`, `@IsDefined()` owns requiredness, safe own enumerable extra properties are retained, and validation calls do not select groups. | `packages/validation/README.md`, `docs/getting-started/migrate-from-nestjs.md` |
