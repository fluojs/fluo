...
| `ValidationPipe` whitelist/forbid 동작과 class-validator `groups` / `always` 실행 | 아직 구현되지 않았습니다. 일반 validator는 `null` / `undefined`를 건너뛰고, `@IsDefined()`가 필수 여부를 담당하며, plain 객체 materialization은 안전한 own enumerable 추가 속성을 유지하고 validation 호출은 group을 선택하지 않습니다. | `packages/validation/README.md`, `docs/getting-started/migrate-from-nestjs.md` |
