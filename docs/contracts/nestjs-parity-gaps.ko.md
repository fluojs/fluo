...
| NestJS Swagger controller scanning, reflection-driven response schema, implicit documentation UI setup | 명시적 `OpenApiModule` 등록으로 의도적으로 대체합니다. Application은 `sources` 및/또는 `descriptors`를 나열하고, `ui: true`로 `/docs`를 opt-in하며, `@ApiResponse({ schema })` 또는 `@ApiResponse({ type })`으로 response content를 선언합니다. Handler 반환값에서는 response shape를 추론하지 않습니다. | `packages/openapi/README.md`, `docs/architecture/openapi.md`, `docs/getting-started/migrate-from-nestjs.md` |
