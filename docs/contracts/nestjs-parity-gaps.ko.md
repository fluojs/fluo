...
| `class-validator`, class-transformer `@Type(...)`, reflection-first DTO contracts as the default validation model | 명시적인 fluo validation contract로 의도적으로 대체합니다. 중첩 DTO constructor는 `@ValidateNested(...)`에 전달하고, mapped helper는 `@fluojs/validation`에서 import하여 fluo가 문서화한 class-level rule 의미를 따릅니다. | `docs/getting-started/migrate-from-nestjs.md`, `packages/validation/README.md` |
