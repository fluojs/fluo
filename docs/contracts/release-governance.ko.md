...
`main`은 stable release lane입니다. `main` 대상 pull request와 release workflow는 모두 changeset lane gate를 실행합니다. `.github/workflows/release.yml`은 Changesets action이 Version Packages PR을 만들거나 publish하기 전에 `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=${{ github.event.before }}`를 실행합니다. Stable gate는 `patch`, `minor`, `major` changeset과 이에 대응하는 generated `packages/*/package.json` version delta를 허용하면서, malformed release metadata는 계속 거부합니다. Semver intent가 authoritative하므로 lane check를 통과시키려고 changeset을 downgrade하지 마세요. Major changeset은 merge 전에 명시적인 maintainer approval과 consumer-facing migration note가 필요합니다.
...
- `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=<ref>`는 stable `main` lane의 pending changeset과 generated package version delta를 검증하되, 유효한 semver intent를 downgrade하지 않습니다.
