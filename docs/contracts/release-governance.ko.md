...
`main`은 stable patch lane입니다. `main` 대상 pull request와 release workflow는 모두 changeset lane gate를 실행합니다. `.github/workflows/release.yml`은 Changesets action이 Version Packages PR을 만들거나 publish하기 전에 `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=${{ github.event.before }}`를 실행합니다. 이 gate는 pending `.changeset/*.md` 파일 중 `minor` 또는 `major` bump를 선언한 항목을 거부하고, 이미 생성된 Version Packages PR이 `packages/*/package.json`에 만든 minor 또는 major version delta도 거부합니다. Minor와 major changeset은 메인테이너가 해당 lane을 안정 릴리스 흐름으로 승격하기 전까지 dedicated release branch 또는 prerelease branch에 보관해야 합니다. Patch changeset은 계속 `main`을 통해 진행하고 `latest`로 publish할 수 있습니다.
...
- `pnpm verify:changeset-release-lane -- --lane=stable --base-ref=<ref>`는 PR CI와 Changesets action이 stable Version Packages PR을 만들거나 publish하기 전 단계에서 generated package version delta까지 포함해 `main` patch-only lane을 강제합니다.
