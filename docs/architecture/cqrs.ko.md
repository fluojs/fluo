...
| --- | --- | --- | --- |
| Command | `CommandBusLifecycleService.execute(...)` 또는 `COMMAND_BUS.execute(...)` | 하나의 Command 타입에 하나의 singleton 핸들러 | Command는 생성자 식별자로 해석됩니다. 핸들러가 없으면 `CommandHandlerNotFoundException`이 발생합니다. 같은 Command에 대해 서로 다른 DI token을 가진 singleton 핸들러가 중복되면 탐색 단계에서 `DuplicateCommandHandlerError`가 발생하며, 같은 token으로 중복 발견된 항목은 하나의 핸들러로 병합됩니다. |
| Query | `QueryBusLifecycleService.execute(...)` 또는 `QUERY_BUS.execute(...)` | 하나의 Query 타입에 하나의 singleton 핸들러 | Query는 생성자 식별자로 해석됩니다. 핸들러가 없으면 `QueryHandlerNotFoundException`이 발생합니다. 같은 Query에 대해 서로 다른 DI token을 가진 singleton 핸들러가 중복되면 탐색 단계에서 `DuplicateQueryHandlerError`가 발생하며, 같은 token으로 중복 발견된 항목은 하나의 핸들러로 병합됩니다. |
| Event | `CqrsEventBusService.publish(...)` 또는 `EVENT_BUS.publish(...)` | 하나의 Event 타입에 0개 이상의 singleton 핸들러 | Event 핸들러는 게시된 이벤트 타입에 대해 `instanceof`로 매칭됩니다. 같은 Event에 대한 중복 `@EventHandler(...)` provider는 유효하며 discovery 순서대로 fan-out됩니다. 같은 token으로 중복 발견된 동일 DI provider는 단일 구독으로 병합됩니다. 로컬 핸들러가 먼저 실행되고, 그다음 saga 디스패치가 수행되며, 마지막으로 `@fluojs/event-bus`를 통한 위임 게시가 수행됩니다. |
...
| 중복 등록 | 중복 singleton Command/Query 핸들러는 해당 bus가 point-to-point이므로 서로 다른 DI token을 사용할 때만 discovery error입니다. Command, Query, Event handler, Saga가 같은 DI token으로 반복 discovery되면 해당 메시지 타입에 대해 한 번만 호출되도록 병합됩니다. 서로 다른 DI token을 가진 중복 singleton Event 핸들러는 충돌이 아니며, 일치하는 모든 distinct `@EventHandler(...)` provider가 유지되고 discovery 순서대로 한 번씩 호출됩니다. | `packages/cqrs/src/buses/command-bus.ts`, `packages/cqrs/src/buses/query-bus.ts`, `packages/cqrs/src/buses/event-bus.ts`, `packages/cqrs/src/buses/saga-bus.ts`, `packages/cqrs/src/errors.ts` |
...
| 종료 동작 | CQRS 이벤트 버스는 진행 중인 `publish(...)` pipeline과 `publishAll(...)` sequence가 settle될 때까지 기다린 뒤 stopped 상태로 전환합니다. saga 런타임도 종료 시 진행 중인 디스패치를 기다린 뒤 descriptor와 캐시된 핸들러 인스턴스를 정리합니다. 종료가 시작되었거나 완료된 뒤 새 `publish(...)`, `publishAll(...)`, saga `dispatch(...)` 호출은 로그를 남기고 no-op으로 처리되며, CQRS handler, saga 또는 위임 `@fluojs/event-bus` 발행을 실행하지 않습니다. | `packages/cqrs/src/buses/event-bus.ts`, `packages/cqrs/src/buses/saga-bus.ts` |
...
- CQRS event bus 또는 saga runtime이 `stopping` 또는 `stopped` 상태에 들어간 뒤에는 teardown 중 handler 재탐색이나 fan-out 재시작을 하지 않도록 새 publication과 saga dispatch를 의도적으로 무시합니다.
