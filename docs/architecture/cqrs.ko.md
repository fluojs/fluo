...
| saga 런타임 | saga token별 실행을 직렬화하고 이미 활성 상태인 token이 소유한 어떤 route로든 context-preserving nested entry를 거부하므로 singleton saga instance가 동시에 여러 execution owner를 갖지 않습니다. Node.js async-local API 없이 opaque, frozen fieldless `CqrsDispatchContext`를 nested CQRS 호출로 전달하며, 진단용 런타임 스냅샷 데이터를 제공합니다. 신뢰하는 topology state는 writable context field로 노출하지 않고 immutable private state로 유지합니다. | `packages/cqrs/src/dispatch-context.ts`, `packages/cqrs/src/buses/saga-topology.ts`, `packages/cqrs/src/buses/saga-bus.ts` |
...
- Saga 오케스트레이션은 안전하지 않은 재진입을 방지합니다. 하나의 saga provider token이 활성 상태인 동안 현재 event route 또는 그 token이 소유한 다른 event route로 nested entry하면 `SagaTopologyError`가 발생하며, 중첩 깊이 제한 `32`를 초과해도 실패합니다. 비순환 nested stage는 서로 다른 singleton provider token을 사용해야 합니다.
