...
| Duplicate registrations | Duplicate singleton command and query handlers are discovery errors because those buses are point-to-point. The duplicate check is provider-registration based: the same handler class registered under two different DI tokens still conflicts for commands and queries. Duplicate singleton event handlers are not a conflict; every matching `@EventHandler(...)` provider is retained and invoked once in discovery order, including same-class providers registered under different DI tokens. | `packages/cqrs/src/buses/command-bus.ts`, `packages/cqrs/src/buses/query-bus.ts`, `packages/cqrs/src/buses/event-bus.ts`, `packages/cqrs/src/errors.ts` |
