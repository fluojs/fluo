...
| Saga runtime | Serializes execution per saga token and rejects context-preserving nested entry into any route owned by an already active token, so a singleton saga instance never has concurrent execution owners. It threads an opaque, frozen fieldless `CqrsDispatchContext` through nested CQRS calls without Node.js async-local APIs and reports runtime snapshot data for diagnostics. Trusted topology state is immutable and retained privately rather than exposed as writable context fields. | `packages/cqrs/src/dispatch-context.ts`, `packages/cqrs/src/buses/saga-topology.ts`, `packages/cqrs/src/buses/saga-bus.ts` |
...
- Saga orchestration is guarded against unsafe re-entry. While one saga provider token is active, nested entry into either its current event route or another event route owned by that token fails with `SagaTopologyError`; exceeding the nested depth limit of `32` also fails. Acyclic nested stages must use distinct singleton provider tokens.
