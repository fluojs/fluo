...
| Configuration file changes with config reload enabled | `ConfigModule.forRoot({ watch: true, ... })` owns the one injectable `CONFIG_RELOADER` and activates its watcher during `onApplicationBootstrap()`. `createConfigReloader(...)` remains the separate standalone loader. The watcher skips reload when final ordered env-file content matches the last committed watch baseline. | The existing `ConfigService` snapshot is replaced in process after content changes and validation succeeds. | `packages/config/src/load.ts`, `packages/config/src/module.ts` |
