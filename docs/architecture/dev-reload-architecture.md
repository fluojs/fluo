...
| Watch scope for config reload | `startReloaderWatcher(...)` watches the normalized env file path, or its parent directory when the env file is missing at startup, and returns no watcher when `watch` is disabled or no watch target exists. | `packages/config/src/load.ts:661-703` |
...
| Activation point | `ConfigReloadManager` creates the reloader lazily and only during `onApplicationBootstrap()` when `options.watch` is true. | `packages/config/src/reload-module.ts:84-102` |
