...
- Handler-level throttling in `@fluojs/throttler` uses the resolved route signature, a deterministic SHA-256 fingerprint of the compiled module/controller/handler sources, and client identity as the storage key. Matching display names therefore remain isolated across distinct compiled handlers, while identical build artifacts share distributed buckets.
