...
- Handler-level throttling in `@fluojs/throttler` uses the resolved route signature, the deterministic source/method position assigned to each `HandlerDescriptor` by the HTTP route compiler, and client identity as the storage key. Distinct same-source handlers therefore remain isolated, while identical artifact layouts share distributed buckets.
