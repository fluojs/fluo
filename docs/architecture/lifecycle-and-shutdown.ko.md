...
| 3 | 라이프사이클 인스턴스 해석 | 공개 라이프사이클 계약을 구현한 런타임 공급자와 모듈 공급자를 라이프사이클 실행 전에 해석합니다. 모든 적격 singleton `multi: true` contribution은 contribution 순서에 따른 별도 lifecycle instance로 유지됩니다. | `packages/runtime/src/bootstrap.ts` |
