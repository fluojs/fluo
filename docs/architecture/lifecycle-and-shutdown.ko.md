...
| Cron scheduler ownership | `@fluojs/cron`은 decorator로 발견한 cron task를 application bootstrap 중 시작하고, 이미 시작된 registry에 dynamic cron task가 추가되면 즉시 시작하며, shutdown 중 active task execution을 설정된 timeout 안에서 drain합니다. Distributed lock release는 task execution 뒤에 실행됩니다. Release 실패 시 local ownership은 shutdown retry/reporting을 위해 유지되고, bounded shutdown timeout이 만료되면 아직 실행 중인 lock은 보존됩니다. | `packages/cron/src/service.ts`, `packages/cron/src/distributed-lock-manager.ts` |
