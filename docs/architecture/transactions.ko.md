...
| Deprecated 인터셉터 호환성 | `PrismaTransactionInterceptor`, `DrizzleTransactionInterceptor`는 기존 1.x import를 위해 유지되며 각 패키지의 `requestTransaction(...)` API에 위임합니다. Mongoose request boundary는 애플리케이션이 명시적으로 소유합니다. |

NestJS controller 또는 interceptor transaction 패턴을 마이그레이션할 때 일반적인 비즈니스 원자성은 서비스 `@Transaction()` 메서드에 두세요. 기존 Prisma 또는 Drizzle 애플리케이션은 migration 동안 deprecated 호환성 interceptor를 유지할 수 있습니다. Mongoose는 같은 chain 위치에 애플리케이션 소유 interceptor를 두고 `MongooseConnection.requestTransaction(() => next.handle(), context.requestContext.request.signal)`을 호출하세요. 가능한 경우 request `AbortSignal`을 전달하세요.
