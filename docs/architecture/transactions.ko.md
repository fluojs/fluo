...
NestJS controller 또는 interceptor transaction 패턴을 마이그레이션할 때 일반적인 비즈니스 원자성은 서비스 `@Transaction()` 메서드에 두세요. 기존 Prisma 애플리케이션은 migration 동안 deprecated 호환성 interceptor를 유지할 수 있습니다. Drizzle과 Mongoose 애플리케이션은 같은 chain 위치에 애플리케이션 소유 interceptor를 두고 각 wrapper의 명시적 `requestTransaction(...)`에 request `AbortSignal`을 전달하세요.
