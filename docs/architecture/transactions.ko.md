...
| Deprecated 인터셉터 호환성 | `DrizzleTransactionInterceptor`, `MongooseTransactionInterceptor`는 deprecated 1.x bridge로 남습니다. `PrismaTransactionInterceptor`는 제거되었으므로 등록을 request `AbortSignal`과 함께 `PrismaService.requestTransaction(...)`을 호출하는 애플리케이션 소유 interceptor 또는 controller 경계로 마이그레이션하세요. 새 코드에는 서비스 `@Transaction()`과 명시적 request boundary를 우선 사용하세요. |

NestJS controller 또는 interceptor transaction 패턴을 마이그레이션할 때 일반적인 비즈니스 원자성은 서비스 `@Transaction()` 메서드에 두세요. 기존 Drizzle 또는 Mongoose 애플리케이션은 migration 동안 deprecated 호환성 interceptor를 유지할 수 있습니다. Prisma 애플리케이션은 애플리케이션 소유 `requestTransaction(...)` 경계로 옮겨야 합니다. 가능한 경우 request `AbortSignal`을 전달하세요.

제거된 facade와 interceptor의 마이그레이션 단계는 [Prisma 등록 마이그레이션](../getting-started/migrate-prisma-registration.ko.md)을 참고하세요.
