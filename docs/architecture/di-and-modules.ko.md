...
- 요청 범위 컨테이너는 범위 체인에 이미 보이는 토큰을 해당 요청에 한정된 교체 항목으로 `override()`할 수 있습니다. 해당 `override()`를 소유한 컨테이너가 토큰을 직접 해석할 때는 교체 인스턴스를 루트 싱글톤 캐시가 아닌 자신의 요청 캐시에 저장합니다. 이 캐시 분리 보장은 중첩된 요청 범위 자식에서 시작한 해석으로 확장되지 않습니다.
