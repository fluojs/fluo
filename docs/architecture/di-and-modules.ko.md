...
- `@Module({ global: true })` 또는 `global: true`로 표시된 모듈은 자신의 exported 토큰을 직접 import 없이 보이게 만듭니다. export되지 않은 공급자는 계속 비공개입니다.

`@Module()`과 `@Module(undefined)`는 `{}`를 사용합니다. Module을 등록하고, decorator
순서와 관계없이 기존 부분 metadata와 `@Module({ global: true })`을 보존하며 module metadata version을
...
- 필수 생성자 매개변수가 있는 클래스나 컨트롤러는 공급자 객체가 `inject`를 명시적으로 제공하지 않는 한, 이에 대응하는 클래스 수준 `@Inject(...)` 메타데이터를 제공해야 합니다.
...
- 생성자 토큰은 클래스, 문자열, 심볼, `ForwardRef.create(...)` 래퍼, `Optional.create(...)` 래퍼가 될 수 있습니다.
- `ForwardRef.create(...)`는 선언 순서 때문에 데코레이션 시점에 값이 정의되지 않은 토큰 하나를 감쌉니다. 실제 생성자 순환을 해석 가능하게 만들지는 않습니다.
- `Optional.create(token)`은 의존성 토큰 하나를 optional로 감쌉니다. 선택적 토큰이 없으면 예외 대신 `undefined`로 해석됩니다.
...
| --- | --- | --- |
| `singleton` | 클래스, 값, 별칭, 팩토리 공급자의 기본 범위이며 명시적으로 재정의하지 않으면 유지됩니다. | 루트 컨테이너 캐시에서 하나의 인스턴스를 공유합니다. 기존 토큰의 `override()`를 소유한 요청 컨테이너가 그 토큰을 직접 해석하면 교체 인스턴스를 루트 싱글톤 캐시 대신 자신의 요청 캐시에 저장합니다. |
...
- 요청 범위 컨테이너는 `register()`로 싱글톤 공급자를 도입해서는 안 되며, 범위 체인에 보이지 않는 새 토큰에 `override()`를 사용하여 싱글톤 공급자를 도입해서도 안 됩니다. 루트 수준 싱글톤 등록은 요청 범위를 만들기 전에 수행됩니다.
- 요청 범위 컨테이너는 범위 체인에 이미 보이는 토큰을 해당 요청에 한정된 교체 항목으로 `override()`할 수 있습니다. 교체 항목이 기본 범위의 비별칭 provider인 경우, 교체 인스턴스는 `override()`를 소유한 가장 가까운 요청 범위 조상의 요청 캐시에 저장되며, 해당 소유자가 직접 시작한 해석과 중첩된 요청 범위 자식에서 시작한 해석이 이를 공유합니다. 루트 싱글톤 캐시는 변경되지 않습니다. 요청 범위, transient, alias 교체 항목은 각각의 일반 해석 의미를 유지합니다.
- `createRequestScope()`는 상속된 싱글톤 공급자를 위해 루트 싱글톤 캐시를 공유하고, 해당 자식이 소유한 요청 범위 인스턴스를 분리하는 컨테이너를 생성합니다.
...
- 공급자 순환 의존성 체인은 `CircularDependencyError`로 해석 실패합니다. `ForwardRef.create(...)`는 선언 시점의 토큰 조회만 지연하며 실제 생성자 순환을 해석 가능하게 만들지 않으므로, 그런 순환은 리팩터링으로 제거해야 합니다.
