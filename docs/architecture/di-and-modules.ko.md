...
| --- | --- | --- |
| `singleton` | 클래스, 값, 별칭, 팩토리 공급자의 기본 범위이며 명시적으로 재정의하지 않으면 유지됩니다. | 루트 컨테이너 캐시에서 하나의 인스턴스를 공유합니다. 단, 요청 컨테이너의 범위 체인에 이미 보이는 토큰을 요청 로컬로 `override()`하면 해당 요청 컨테이너가 교체 인스턴스를 캐시합니다. |
...
- 요청 범위 컨테이너는 범위 체인에 이미 보이는 토큰을 해당 요청에 한정된 교체 항목으로 `override()`할 수 있습니다. 교체 인스턴스의 캐시는 요청 컨테이너가 소유하며 루트 싱글톤 캐시를 오염시켜서는 안 됩니다. 루트 컨테이너와 형제 요청 범위는 계속 루트 등록을 해석합니다.
- `createRequestScope()`는 상속된 싱글톤 공급자를 위해 루트 싱글톤 캐시를 공유하고, 요청 범위 인스턴스와 요청 로컬 override 캐시는 분리하는 자식 컨테이너를 생성합니다.
