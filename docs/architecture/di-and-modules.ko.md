...
- 요청 범위 컨테이너는 범위 체인에 이미 보이는 토큰을 해당 요청에 한정된 교체 항목으로 `override()`할 수 있습니다. 교체 인스턴스는 `override()`를 소유한 가장 가까운 요청 범위 조상의 요청 캐시에 저장되며, 해당 소유자가 직접 시작한 해석과 중첩된 요청 범위 자식에서 시작한 해석이 이를 공유합니다. 루트 싱글톤 캐시는 변경되지 않습니다.
