...
- `forwardRef(...)`는 선언 순서 때문에 데코레이션 시점에 값이 정의되지 않은 토큰 하나를 감쌉니다. 실제 생성자 순환을 해석 가능하게 만들지는 않습니다.
- `optional(token)`은 의존성 토큰 하나를 optional로 감쌉니다. 선택적 토큰이 없으면 예외 대신 `undefined`로 해석됩니다.
