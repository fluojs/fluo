...
| Bearer credential extraction | `BearerJwtStrategy`는 `Authorization` header를 읽어 `Bearer <token>` credential을 검증하며, header가 array-valued이면 첫 번째 entry만 읽습니다. 없거나 빈 header는 `AuthenticationRequiredError`를 발생시킵니다. 잘못된 scheme이거나 malformed인 header(`Bearer` 뒤에 credential이 없거나 segment가 추가되거나 control character가 포함된 경우 포함)는 `AuthenticationFailedError`를 발생시킵니다. Scheme 비교는 대소문자를 구분하지 않습니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
| Bearer preset error causality | 만료된 bearer credential은 `AuthenticationExpiredError`, `JwtInvalidTokenError` credential은 `AuthenticationFailedError`를 발생시킵니다. 둘 다 원본 JWT verifier error를 `cause`로 보존합니다. `JwtConfigurationError`와 verifier-provider infrastructure failure는 `401 Unauthorized`로 바꾸지 않고 변경 없이 전파하며, preset은 정규화된 `JwtPrincipal`을 변경 없이 반환합니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
