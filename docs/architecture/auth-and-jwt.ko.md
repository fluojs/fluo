...
| Bearer preset error causality | 만료된 bearer credential은 `AuthenticationExpiredError`, 그 외 검증 실패는 `AuthenticationFailedError`를 발생시킵니다. 둘 다 원본 JWT verifier error를 `cause`로 보존하며, preset은 정규화된 `JwtPrincipal`을 변경 없이 반환합니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
