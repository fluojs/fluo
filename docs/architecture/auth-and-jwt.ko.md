...
| Refresh ownership | 하나의 구성된 `JwtModule`이 하나의 `RefreshTokenService`, refresh crypto 설정, 구성된 store를 소유합니다. `RefreshTokenModule.forRoot()`는 Passport HTTP 교환에 이 service를 alias하며, 두 번째 service·store·refresh 설정을 만들면 안 됩니다. | `packages/jwt/src/module.ts`, `packages/passport/src/refresh/refresh-token.ts` |
| Refresh HTTP exchange | `RefreshTokenStrategy`가 기본 endpoint recipe입니다. `body.refreshToken`을 `Authorization: Bearer <token>`보다 우선하며 body 값이 malformed이면 fallback하지 않습니다. Custom `RefreshTokenServicePort`도 strategy가 subject를 설정하기 전에 globally visible `JwtModule` verifier가 필요하고 그 verifier가 수락하는 access token을 반환해야 합니다. 누락된 credential은 `AuthenticationRequiredError`, malformed/reused/invalid token은 `AuthenticationFailedError`, 만료된 token은 `AuthenticationExpiredError`로 변환합니다. | `packages/passport/src/refresh/refresh-token.ts` |
...
| Rotation prerequisite | `refreshToken.rotation`이 활성화되면 구성된 리프레시 토큰 저장소는 원자적 `rotate(...)` 또는 원자적 `consume(...)`를 구현해야 합니다. `rotate(...)`는 현재 토큰 소비와 대체 토큰 저장을 저장소 소유의 단일 작업으로 처리하므로 durable replacement 경로입니다. `consume(...)`만 있으면 service가 consume 이후 대체 토큰을 별도로 저장합니다. 원자적 지원이 없으면 구성 오류입니다. | `packages/jwt/src/refresh/refresh-token.ts` |
| Rotation failure handling | 이미 소비된 refresh token의 재사용은 optional `revokeByFamily(family)`를 호출하고 `JwtInvalidTokenError`를 발생시킵니다. 이 capability가 없는 store는 보수적인 `revokeBySubject(subject)` fallback으로 호환성을 유지하며, 이 경우 subject의 독립적인 family도 함께 revoke됩니다. 대체 토큰 서명은 현재 토큰 소비 전에 완료되고, `rotate(...)` store는 consume과 replacement 저장을 원자적으로 수행합니다. | `packages/jwt/src/refresh/refresh-token.ts`, `packages/jwt/src/refresh/refresh-token.test.ts` |
...
| Bearer credential extraction | `BearerJwtStrategy`는 `Authorization` header를 읽어 `Bearer <token>` credential을 검증하며, header가 array-valued이면 첫 번째 entry만 읽습니다. Credential은 RFC 6750 `b64token`을 따라 ASCII 문자, 숫자, `-`, `.`, `_`, `~`, `+`, `/`가 하나 이상 있고 뒤에만 optional `=` padding이 올 수 있습니다. `?`, 내부 `=`, Unicode, whitespace, control character는 검증 전에 거부합니다. 없거나 빈 header는 `AuthenticationRequiredError`, 잘못된 scheme이거나 malformed인 header는 `AuthenticationFailedError`를 발생시킵니다. Scheme 비교는 대소문자를 구분하지 않고 ASCII space를 하나 이상 요구합니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
| Bearer preset error causality | 누락되거나 malformed인 credential은 `WWW-Authenticate: Bearer`를 직렬화합니다. 만료된 bearer credential은 `AuthenticationExpiredError`, `JwtInvalidTokenError` credential은 `AuthenticationFailedError`를 발생시킵니다. 둘 다 원본 JWT verifier error를 `cause`로 보존하고 `WWW-Authenticate: Bearer error="invalid_token"`을 직렬화합니다. `JwtConfigurationError`와 verifier-provider infrastructure failure는 `401 Unauthorized`로 바꾸지 않고 변경 없이 전파하며, preset은 정규화된 `JwtPrincipal`을 변경 없이 반환합니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
| Bearer preset registration | `BEARER_JWT_STRATEGY_NAME`은 안정적인 전략 이름 `'jwt'`이고, `createBearerJwtStrategyRegistration()`은 `PassportModule.forRoot(...)`용 registry entry를 반환하며, `BearerJwtStrategy`는 애플리케이션 provider로 등록됩니다. 결합된 JWT/Passport module facade는 도입되지 않습니다. | `packages/passport/src/bearer/bearer-jwt.ts` |
