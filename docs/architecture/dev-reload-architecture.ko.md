...
| 활성화 시점 | 수동 `ConfigReloadManager.reload()`는 처음 사용할 때 reloader를 지연 생성합니다. `onApplicationBootstrap()`은 `options.watch`가 참이고 manager가 종료되지 않았을 때만 reloader를 즉시 생성합니다. | `packages/config/src/reload-module.ts:65-105` |
...
| 종료 시 정리 | `ConfigReloadManager.onModuleDestroy()`는 종료 과정에서 watcher를 닫고 listener를 비웁니다. | `packages/config/src/reload-module.ts:83-109` |
| 종료의 최종성 | manager 종료는 최종 상태입니다. `close()` 또는 `onModuleDestroy()` 이후 `reload()`, `subscribe()`, `subscribeError()`는 `InvariantError`를 던지고, `onApplicationBootstrap()`은 no-op이 되며, 대체 reloader나 watcher는 생성되지 않습니다. | `packages/config/src/reload-module.ts:111-115`, `packages/config/src/reload-module.test.ts:326-360` |
