...
| config reload가 활성화된 설정 파일 변경 | `ConfigModule.forRoot({ watch: true, ... })`가 하나의 injectable `CONFIG_RELOADER`를 소유하고 `onApplicationBootstrap()`에서 watcher를 활성화합니다. `createConfigReloader(...)`는 별도의 standalone loader로 유지됩니다. watcher는 최종 순서형 env-file content가 마지막으로 commit된 watch baseline과 같으면 reload를 건너뜁니다. | content가 바뀌고 검증이 성공하면 기존 `ConfigService` 스냅샷이 프로세스 내부에서 교체됩니다. | `packages/config/src/load.ts`, `packages/config/src/module.ts` |
