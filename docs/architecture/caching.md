...
| Built-in strategies | Supported strategy values are `'route'`, `'route+query'`, or a custom function. The default `'route+query'` uses the path plus a sorted query string; set `'route'` only for query-insensitive responses. | `packages/cache-manager/src/module.ts`, `packages/cache-manager/src/types.ts`, `packages/cache-manager/src/interceptor.ts` |
...
| Store teardown | During application shutdown, `CacheService` calls a custom store `close()` hook; an existing `dispose()`-only store remains compatible when `close()` is absent. New resource-owning stores should implement `close()` to release sockets, pools, timers, or other external handles. Concurrent and repeated service or lifecycle close calls share the first teardown promise, including its failure, so teardown runs once behind one authoritative completion boundary. | `packages/cache-manager/src/types.ts`, `packages/cache-manager/src/service.ts` |
