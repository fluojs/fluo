...
| Eviction timing | For non-GET handlers, eviction runs only after the downstream handler succeeds, `response.send(...)` settles successfully, and the response reports a committed result. A rejected send, a resolved send without confirmed commit, or request abort before commit cancels deferred eviction; shutdown and disconnect therefore preserve the previous cached read. No elapsed-time fallback infers commit. Adapter or outer-pipeline paths that never invoke `response.send(...)` must complete the standard lifecycle or invalidate explicitly through `CacheService.del(...)`. | `packages/cache-manager/src/interceptor.ts`, `packages/cache-manager/src/deferred-eviction.ts` |
