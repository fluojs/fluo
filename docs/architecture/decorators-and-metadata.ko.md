...
| DI 연결 | 생성자 의존성은 클래스 수준 `@Inject(...)` 또는 provider 수준 `inject`로 명시합니다. |
...
- `@Inject(...)`는 생성자 토큰을 명시적으로 정의하는 표준 클래스 데코레이터입니다. 토큰 순서는 생성자 매개변수 순서와 대응합니다.
...
| DI 의존성 | 생성자 연결은 명시적 클래스 수준 `@Inject(...)` 토큰 또는 provider `inject` 배열을 사용합니다. |
