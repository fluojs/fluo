...
| HTTP paths | Use each platform concrete `AdapterClass.create(options)`, `FluoFactory.create(...)` from `@fluojs/runtime`, then `app.listen()`. Custom adapters implement `HttpApplicationAdapter` from `@fluojs/http/portable`. `src/` paths are implementation evidence, not consumer imports. |
...
| `AdapterClass.create(options)` | Configures transport without listening or registering signals. Compose the application and host callback through the [Factory recipe](../getting-started/bootstrap-paths.md). |
| Node/Fastify shutdown options | Explicit `createNodeShutdownSignalRegistration()` selects SIGINT/SIGTERM by default; omit the callback for host-owned signals. `forceExitTimeoutMs = 30_000` bounds signal completion separately from adapter `shutdownTimeoutMs = 10_000`. Fastify validates a non-negative safe integer shutdown limit during setup. |
...
The runtime exposes shutdown hooks as explicit contracts only. Signal registration is owned by the surrounding host through its shutdown registration callback, not by the universal runtime surface.
