...
| Close path order | Application and context teardown runs readiness reset first, then runtime cleanup callbacks, shutdown hooks, `adapter.close(signal)`, and container disposal. A retry skips completed runtime phases and re-enters an incomplete adapter or lifecycle-hook stage according to that stage's retry contract. Container disposal is terminal best-effort: it attempts every materialized container-managed `onDestroy()` hook once, and an individual failed hook is not retried by a later application or context close. | `packages/runtime/src/bootstrap.ts`, `packages/runtime/src/retryable-shutdown.ts`, `packages/runtime/src/bootstrap.test.ts` (`does not retry container-managed onDestroy hooks on a second application context close`) |
