...
| Microservice ingress | Starting microservice close establishes a terminal ingress gate synchronously. New `send()` and `emit()` calls reject before runtime or transport handoff while an overlapping `listen()` settles, and a failed close attempt does not reopen ingress. | `packages/runtime/src/bootstrap.ts`, `packages/microservices/src/service.ts` |
