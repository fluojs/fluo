...
| Closed transition | `Application.close()` sets the application state to terminal `closed` before awaiting teardown. The state remains closed after a failed attempt, while a later `close()` skips completed teardown phases and retries the failed stage. | `packages/runtime/src/bootstrap.ts` |
...
| Close path order | Application and context teardown runs readiness reset first, then runtime cleanup callbacks, shutdown hooks, `adapter.close(signal)`, and container disposal. A retry skips completed phases, retries a failed stage as a unit, and preserves that ordering for the remaining work. | `packages/runtime/src/bootstrap.ts`, `packages/runtime/src/retryable-shutdown.ts` |
| Idempotent close entry | `Application.close()` and `ApplicationContext.close()` reuse the in-flight closing promise and return immediately after the first successful close. If teardown fails, a later close skips completed phases and retries the failed stage; application ingress and context resolution remain terminal throughout. | `packages/runtime/src/bootstrap.ts`, `packages/runtime/src/retryable-shutdown.ts` |
