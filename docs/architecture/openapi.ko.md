...
| 노출 경로 | 런타임 모듈은 항상 `GET /openapi.json`을 제공합니다. Swagger UI는 opt-in입니다. `GET /docs`는 `ui: true`일 때만 페이지를 제공하고, 그 외에는 `NotFoundException`을 던집니다. | `packages/openapi/src/openapi-module.ts` |
...
| Swagger UI | `ui: true`일 때 `GET /docs`는 런타임 JSON 경로를 가리키는 HTML을 렌더링합니다. 기본 asset은 고정된 `swagger-ui-dist` 버전 `5.32.2`를 사용하며, self-hosted 또는 CSP-controlled deployment에서는 `swaggerUiAssets.cssUrl`과 `swaggerUiAssets.jsBundleUrl`로 URL을 교체할 수 있습니다. | `packages/openapi/src/openapi-module.ts` |
