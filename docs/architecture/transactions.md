...
## Result-Based Rollback

This section owns the opt-in `shouldRollback` and shared transaction owner's rollback-only contract across the three wrappers. The [Prisma](../../packages/prisma/README.md#choosing-rollback-from-a-result), [Drizzle](../../packages/drizzle/README.md#choosing-rollback-from-a-result), and [Mongoose](../../packages/mongoose/README.md#choosing-rollback-from-a-result) READMEs own public imports, exact argument positions, and typed consumer examples.

| Contract field | Behavior |
| --- | --- |
| Scope and prerequisites | Applies to native transactions directly owned by the same Fluo wrapper on supported Node.js `>=24.0.0 <27`. Requires Prisma interactive `$transaction`, Drizzle `database.transaction`, or a Mongoose delegated transaction or rollback-capable native-session path. |
| Inputs and defaults | `readonly shouldRollback?: (value: T) => boolean` in `TransactionBoundaryOptions<T = unknown>` synchronously evaluates that callback's normally returned value. Omission preserves existing exception-based behavior. Fluo introduces no global `Result` type or convention for `ok`, `success`, or `error` fields. Consumers own the result shape and failure predicate. |
| Capability preflight | Supplying `shouldRollback` on a fallback or legacy decorator target that cannot own native rollback rejects with `TransactionRollbackCapabilityError` **before** the user callback. Even with `strictTransactions: false`, the opt-in cannot silently fall through to direct execution. |
| Root return | If the root predicate returns `true`, native rollback runs and the **same root value** is returned after successful rollback and required cleanup. Objects are not copied or exposed as internal control errors. If the predicate returns `false` and the owner is not rollback-only, the existing commit and hook drain precede the return. |
| Nested return and owner | Nested boundaries on the same wrapper share the native transaction, hook queue, and rollback-only state. A nested predicate returning `true` returns that nested call's original value while marking the owner irrevocably rollback-only and retaining the **first nested failure value**. Later success values or caught exceptions cannot clear it. This is not a separate savepoint or partial rollback. |
| Final root decision | An owner marked rollback-only by a nested failure rolls back when the root finishes. If the root predicate also rejects its own result, the same **root failure value** is returned. Otherwise, including when the root predicate is omitted, `TransactionRollbackOnlyError` is thrown after rollback. Its `readonly result: unknown` is the first nested failure value; consumers narrow it before use. |
| Error distinction | Commit, rollback, and cleanup errors reported by the native runner propagate rather than being hidden by a domain result or `TransactionRollbackOnlyError`. Callback or predicate exceptions escaping to the outer native boundary follow the exception-based rollback path. `TransactionRollbackOnlyError` differs from `AfterCommitError` or Mongoose's `AfterCommitCleanupError`, which report failures after committed work. |
| Hooks and retries | Rollback discards all owner hooks. When the native runner retries the callback, each attempt gets a fresh owner and queue: discarded attempts cannot leak failure values, rollback-only state, or hooks into the next attempt. Fluo adds no retry policy, and commit-only retries do not rerun the callback. |
| Limitations and alternatives | Transactions opened externally through raw clients, other wrappers/connections, and Redis `MULTI/EXEC` are unsupported. This adds no savepoints, DB+Redis atomicity, durability, durable outbox, crash recovery, or network exactly-once guarantees. Durable delivery requires a separate application design such as an Outbox. |

**Strict native confirmation:** Result rollback requires `rollbackObserver` in Fluo module/service runtime options, not native options or `TransactionBoundaryOptions<T>`. Prisma 7.5 hides rollback rejections, so construct the client with the adapter returned by `createPrismaRollbackObserver(new PrismaPg(...))` and register the paired observer. This helper wraps the public adapter factory and confirms **both** SQL `ROLLBACK` execution and adapter cleanup for each attempt. It cannot retrofit an opaque preconstructed client. `createDrizzleRollbackObserver(poolOrClient)` provides the proxy used to construct node-postgres Drizzle and confirms SQL rollback plus pooled-client release. Other drivers are unsupported without a separately verified public observation capability. `createMongooseRollbackObserver(connection.getClient())` requires `monitorCommands: true` at connection creation and correlates complete command attempts by session, transaction number, request, and connection/service IDs. It observes abort errors hidden by MongoDB 7.2; a success event containing `writeConcernError`, a missing event, a pending request, or local `TRANSACTION_ABORTED` state is not confirmation. Missing or mismatched capability rejects before application callbacks with `TransactionRollbackCapabilityError`. Observed errors propagate; absent positive confirmation throws `TransactionRollbackUnconfirmedError`, never the original failure Result or a successful value. Cleanup alone without successful SQL rollback is insufficient. A Mongo boundary with no actual transaction command may also be unconfirmed: an absent abort event is not treated as success. Manual Mongoose callback plus abort/cleanup failures preserve original errors in `AggregateError.errors`, with the preceding failure as `cause`. Default exception-based calls continue to work without an observer. Package READMEs own helper types and complete registration examples; the [native fixture](../../packages/prisma/fixtures/after-commit/README.md#acceptance-assertions) verifies real SQL/cleanup and Mongo command observations. No private driver methods are patched.

**Difference from existing exceptions:** unlike a nested failure selected by `shouldRollback`, an ordinary nested callback exception caught by the outer callback does not by itself mark rollback-only. If an owner without an opted-in failure eventually commits, that nested call's writes and registered hooks retain their existing behavior. Once a nested predicate has marked a failure, however, ignoring its value and returning success cannot permit commit.

Verification must compare this contract with each wrapper's implementation, package regressions, and the [native fixture](../../packages/prisma/fixtures/after-commit/README.md#run). The #3718 documentation or target list is not evidence of passing native execution; record actual commands, exit codes, and database observations in a separate verification receipt.
...
| Deprecated interceptor compatibility | `DrizzleTransactionInterceptor` and `MongooseTransactionInterceptor` remain deprecated 1.x bridges. `PrismaTransactionInterceptor` is removed; migrate its registrations to an application-owned interceptor or controller boundary that calls `PrismaService.requestTransaction(...)` with the request `AbortSignal`. Prefer service `@Transaction()` and explicit request boundaries for new code. |

When migrating NestJS controller or interceptor transaction patterns, keep normal business atomicity on service `@Transaction()` methods. Existing Drizzle or Mongoose applications may retain their deprecated compatibility interceptor while migrating. Prisma applications must move to an application-owned `requestTransaction(...)` boundary. Pass the request `AbortSignal` when available.

See [Migrate Prisma Registration](../getting-started/migrate-prisma-registration.md) for the removed facade and interceptor migration steps.
