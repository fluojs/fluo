...
| Named Drizzle handles | Each named Drizzle handle owns a separate ALS context. Multi-client services select a named handle explicitly with `@Transaction((self) => self.analytics)` rather than relying on decorator target discovery. | `packages/drizzle/src/module.ts`, `packages/drizzle/src/transaction.ts` |
| Named Drizzle handles | Each named Drizzle handle owns a separate ALS context. Multi-client services select a named handle explicitly with `@Transaction((self) => self.analytics)` rather than relying on decorator target discovery. | `packages/drizzle/src/module.ts`, `packages/drizzle/src/transaction.ts` |
| Named Drizzle handles | Each named Drizzle handle owns a separate ALS context. Multi-client services select a named handle explicitly with `@Transaction((self) => self.analytics)` rather than relying on decorator target discovery. | `packages/drizzle/src/module.ts`, `packages/drizzle/src/transaction.ts` |
