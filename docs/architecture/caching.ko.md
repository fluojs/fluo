...
| eviction 시점 | GET이 아닌 handler에서는 downstream handler가 성공하고 `response.send(...)`가 성공적으로 settle되며 response가 commit 결과를 보고한 뒤에만 eviction이 실행됩니다. Send reject, commit 확인 없이 resolve된 send, 또는 commit 전 request abort는 지연 eviction을 취소하므로 shutdown과 disconnect에서도 이전 cached read를 유지합니다. 경과 시간 fallback으로 commit을 추론하지 않습니다. `response.send(...)`를 호출하지 않는 adapter 또는 outer-pipeline 경로는 표준 lifecycle을 완료하거나 `CacheService.del(...)`로 명시적으로 무효화해야 합니다. | `packages/cache-manager/src/interceptor.ts`, `packages/cache-manager/src/deferred-eviction.ts` |
