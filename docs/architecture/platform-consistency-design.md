...
| `getRealtimeCapability?()` | Optional method in `packages/http/src/adapter.ts` | MAY describe server-backed, fetch-style, or unsupported realtime capability through the documented adapter capability union. Fetch-style version 1 may expose the optional, independently versioned `bindingInstallation` extension used to install a realtime binding before `listen()` without changing the stable capability version. |
