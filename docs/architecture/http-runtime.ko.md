...
13. Handler가 수동 `SseResponse`를 반환하면 명시적 close, request abort 또는 raw stream close까지 dispatch를 유지한다. 그 completion 뒤에만 middleware가 settle하고 request observer가 success와 finish를 받으며 request-scoped resource가 dispose된다.
