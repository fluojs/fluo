...
Cron scheduling discoverability도 패키지 문서, governed package-surface docs, intermediate book으로 나뉜다. `packages/cron/README.ko.md`는 `CronModule.forRoot(...)`, five-field 및 six-field cron expression 지원, decorator와 dynamic registry startup behavior, Redis distributed locking, bounded shutdown, public scheduling/status type, status snapshot을 문서화한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 scheduling, distributed lock, shutdown, health/readiness visibility에 대한 `@fluojs/cron` responsibility boundary를 기록한다. [`book/intermediate/ch12-cron.ko.md`](../book/intermediate/ch12-cron.ko.md)는 동일한 bootstrap 및 dynamic-start lifecycle guarantee를 FluoShop 학습 경로에서 설명한다.
