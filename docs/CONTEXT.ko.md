...
Realtime WebSockets discoverability는 `packages/websockets/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md)로 나뉜다. 패키지 README는 루트 `@fluojs/websockets` Node.js 기본값, 런타임별 서브패스 `@fluojs/websockets/node`, `@fluojs/websockets/bun`, `@fluojs/websockets/deno`, `@fluojs/websockets/cloudflare-workers`, 그리고 fetch-style 애플리케이션이 루트 Node.js 기반 entrypoint를 로드하지 않고 gateway를 작성할 수 있도록 각 서브패스가 노출하는 공유 gateway decorator와 metadata authoring primitive를 문서화한다.
