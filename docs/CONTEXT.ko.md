...
Cron scheduling discoverability도 패키지 문서, governed package-surface docs, package chooser, intermediate book으로 나뉜다. `packages/cron/README.ko.md`는 `CronModule.forRoot(...)`, five-field 및 six-field cron expression 지원, decorator와 dynamic registry startup behavior, Redis distributed locking, bounded shutdown, public scheduling/status type, status snapshot을 문서화하며, Redis peer loading 및 resolution은 distributed-only이고 `distributed.enabled`가 `true`일 때만 발생한다는 경계도 명시한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 scheduling, distributed lock, shutdown, health/readiness visibility 및 동일한 distributed-only Redis peer boundary에 대한 `@fluojs/cron` responsibility boundary를 기록한다. [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md)는 Cron distributed lock을 활성화할 때만 `@fluojs/redis`를 추가하라고 안내한다. [`book/intermediate/ch12-cron.ko.md`](../book/intermediate/ch12-cron.ko.md)는 동일한 bootstrap, dynamic-start lifecycle guarantee, Redis distributed-lock 선택 경계를 FluoShop 학습 경로에서 설명한다.
...
Slack discoverability도 패키지 README와 governed package-surface docs로 나뉜다. `packages/slack/README.ko.md`는 `SlackModule.forRoot(...)` / `forRootAsync(...)`, `createSlackProviders(...)`를 통한 manual provider composition, 직접 `SlackService` 전달, `@fluojs/notifications` channel 통합, abort-signal 전파, lifecycle-gated send, platform status snapshot을 문서화한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 직접 `process.env`를 읽지 않는 webhook-first, transport-agnostic Slack delivery에 대한 정식 `@fluojs/slack` responsibility boundary를 기록한다.
