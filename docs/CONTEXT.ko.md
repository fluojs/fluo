...
[지식 색인](./knowledge-index.json)에서 contract ID에 대응하는 EN/KO 담당 문서와 실제 source/test 파일을 찾으세요. `operations.health-readiness`는 health와 트래픽 수용 계약을, `runtime.lifecycle-and-shutdown`은 runtime lifecycle과 cleanup을 소유합니다.

[Health and Readiness Contract](./contracts/health-and-readiness.ko.md)가 아래 health 규범 계약을 소유하며, [라이프사이클 및 종료 보장](./architecture/lifecycle-and-shutdown.ko.md)이 시작, terminal admission, cleanup, host 종료 경계를 소유합니다. 이 CONTEXT와 연결된 legacy Book은 요약이며 독립적인 권위 문서가 아닙니다.

Docs는 AI가 사용하는 프레임워크의 규범적 계약 계층입니다. 이 CONTEXT에서 제약을 확인한 다음 [문서 지도](./README.ko.md#원본-책임)와 [패키지 선택기](./reference/package-chooser.ko.md)로 담당 Docs 또는 위임된 패키지 README를 찾고, 그 계약과 구현·테스트·실행 근거를 읽으세요. 패키지 README는 설치·공개 API·기본값·제약의 원본으로 유지하며 `docs/`에 복제하지 않습니다. [문서 권위 정책](./contracts/documentation-authority.ko.md)은 적용 범위, 입력·출력·기본값, 순서·실패·자원 소유권, 제약·공개 import·전제·근거를 간결한 Markdown으로 기록하는 형식을 정의합니다.

사람의 중심 학습 경로는 [제품·패턴 중심 3권 시리즈](../book/README.ko.md)입니다. FluoBlog를 만들고 같은 서비스에 머천다이즈 숍을 추가한 뒤 Fluo 내부 구조를 살펴봅니다. Book은 같은 Docs 계약을 제품 서사로 설명하며, 앱 도메인 정책은 프레임워크 보장이 아닙니다. AI가 계약을 확인하기 위해 Book을 먼저 읽을 필요는 없습니다. [짧은 HTTP 실습](../apps/docs/content/docs/tutorial/index.mdx)과 [초기 실행 체크포인트](../examples/fluo-blog/README.ko.md)는 보조 자료입니다.
...
`@fluojs/cron` 3은 portable `@fluojs/runtime` 3 dependency를 가지며, 이와 별도로 패키지 자체의
지원 계약으로 Node.js `>=24.0.0 <27`을 선언합니다. Consumer는 업그레이드하기 전에
Node.js 20/22를 포함한 Node.js 24 미만과 Node.js 27+ host를 이 지원 범위로 옮겨야 합니다.
...
`@fluojs/platform-cloudflare-workers` lifecycle contract는 package README, [NestJS migration map](./getting-started/migrate-from-nestjs.ko.md), [Cloudflare Workers Edge Deployment](../book/intermediate/ch24-cloudflare.ko.md)에 문서화합니다. 직접 Worker application은 `CloudflareWorkerHttpApplicationAdapter.create(...)`, `FluoFactory.create(...)`, no-socket `app.listen()`을 사용하며, `CloudflareWorkerApplicationHost.create(...)`는 lazy bootstrap, 첫 environment 재사용, retry, generation 교체, drain recovery를 별도로 소유합니다. Worker `fetch` handler에는 host가 호출하는 shutdown callback이 없으므로 애플리케이션이 close trigger를 소유합니다. `worker.fetch` 밖의 trigger는 `await worker.close()`를 직접 호출할 수 있지만, 그 fetch 안의 management route는 현재 response를 반환한 뒤 `executionContext.waitUntil(worker.close())` 또는 동등한 non-self-awaiting mechanism으로 close를 관찰해야 합니다. 성공한 host close는 재시작 가능합니다. 이후의 `fetch(...)`는 새 application을 bootstrap하고 bootstrap lifecycle hook을 다시 실행하며 application singleton provider를 다시 생성합니다.
...
`@fluojs/terminus`는 NestJS `@HealthCheck()` decorator의 치환물이 아니라 애플리케이션이 소유하는 health composition입니다. 작성한 module에 `TerminusModule.forRoot(...)`를 등록하고 runtime이 `/health`, `/ready`를 노출하게 하세요. `/health`는 집계된 `HealthCheckReport` 진단을 반환합니다. `/ready`는 binary traffic-admission 결정을 내립니다. HTTP `200`은 트래픽을 수용하고 HTTP `503`은 인스턴스를 rotation에서 제외하며, body status는 `ready`, `starting`, `unavailable` 중 하나입니다. Indicator는 기본적으로 readiness를 차단하며, `readiness: false`는 해당 indicator를 `/health`에 유지하면서 트래픽을 차단하지 않게 하고 `readinessChecks`는 조건만 추가합니다. Terminus는 기본 process-only liveness route를 만들지 않으며 runtime-owned route는 controller `@UseGuards()` metadata를 거부합니다. 보호는 path-scoped application 또는 adapter middleware, network policy, deployment-owned probe boundary에 두세요. indicator 등록과 failure semantics의 운영 기준은 package README이고, [NestJS 마이그레이션 맵](./getting-started/migrate-from-nestjs.ko.md)은 마이그레이션 경계를, [Chapter 18](../book/beginner/ch18-health.ko.md)은 학습 경로를 설명합니다.

## 정적 에셋 제공

정적 제공은 [HTTP Runtime Contract](./architecture/http-runtime.ko.md)에 문서화한 portable `@fluojs/http` middleware 계약입니다. 애플리케이션은 `createStaticAssetsMiddleware(...)`에 명시적 `StaticAssetSource`를 전달하므로 fetch-style 및 edge host에 암묵적 filesystem claim이 제공되지 않습니다. `@fluojs/platform-nodejs`는 Node, Express, Fastify deployment용 `createNodeFileSystemAssetSource(...)`를 소유하며 lexical 및 realpath 해석을 구성된 root에 제한하고 precompressed sibling을 선택할 수 있습니다. API 예제와 deployment configuration은 `@fluojs/http`, `@fluojs/runtime` package README를 사용하세요.
...
NestJS 마이그레이션은 [NestJS migration map](./getting-started/migrate-from-nestjs.ko.md)에서 시작한다. i18n handoff는 `Accept-Language` 해석을 정규 `createAcceptLanguageLocalePolicyResolver(...)`로, custom resolver를 `HttpLocaleResolver`로 매핑하고, application-owned `Middleware` 하나를 `FluoFactory.create(AppModule, { middleware })`로 등록하며, 선택된 locale은 현재 `RequestContext`에만 저장한다. global locale fallback은 없다.

NestJS `app.enableShutdownHooks()`를 Node의 `SIGINT`/`SIGTERM` 종료로 이전하려면 `@fluojs/platform-nodejs`의 `createNodeShutdownSignalRegistration()`을 Factory의 `shutdownRegistration`으로 명시적으로 전달하세요. `FluoFactory.create(...)`는 Node signal을 암묵적으로 선택하지 않습니다. Bun과 Deno는 각 host callback을 사용하고, Cloudflare Workers 같은 host는 자체 shutdown event에서 `app.close(signal?)`를 호출합니다. Portable runtime에 Node process-signal wiring을 넣지 않습니다. Fluo lifecycle hook은 `adapter.close(signal?)`보다 먼저 완료되므로 listener close 또는 완료된 drain이 필요한 작업은 adapter나 host shutdown boundary에 둡니다.
...
NestJS HTTP pipeline migration에서 portable bootstrap `middleware`는 `handle(MiddlewareContext, next)`를 구현하며, Express `(req, res, next)` handler는 `ExpressHttpApplicationAdapter.create({ nativeMiddleware: [...] })`를 사용하는 Express adapter boundary에 둔다.
...
- PlatformShell lifecycle exclusivity는 `@fluojs/testing/platform-shell-lifecycle-conformance`의 `PlatformShellLifecycleConformanceHarness.create(...)`로 검증하며, active lifecycle overlap 전체는 `PlatformLifecycleConflictError`로 reject되어야 한다.
...
CQRS dispatch context discoverability는 `packages/cqrs/README.ko.md`, [`docs/architecture/cqrs.ko.md`](./architecture/cqrs.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [NestJS migration map](./getting-started/migrate-from-nestjs.ko.md)로 나뉜다. `CqrsModule.forRoot(...)`가 정식 모듈 등록 경로이고 decorated handler와 saga는 업무 module `providers`에 직접 등록하며, 호출자는 레거시 심볼 토큰 대신 `CommandBusLifecycleService`, `QueryBusLifecycleService`, `CqrsEventBusService`를 직접 주입하고, `CqrsDispatchContext`는 신뢰하는 topology, continuation, shutdown-drain state를 비공개로 유지하는 opaque, frozen fieldless runtime-agnostic pass-through 값이며, event handler와 saga fan-out identity는 singleton provider token을 따르고 low-level provider assembly helper는 내부 구현에 머문다. Saga execution은 singleton provider token마다 하나의 owner를 가지며, 활성 token의 다른 route로 향하는 context-preserving nested publication은 deadlock-safe FIFO continuation이 되고, 이미 활성 상태인 provider-token/event route로 다시 진입하면 `SagaTopologyError`가 발생한다. CQRS local handler는 saga와 위임 `@fluojs/event-bus` 발행보다 먼저 완료되고, `publishAll(...)`은 각 event마다 이 전체 pipeline을 기다린다. Migration map의 CQRS checklist는 class-based saga rewrite, opaque context propagation, 격리된 event clone, bounded shutdown drain을 다룬다.
...
Validation package-surface discoverability는 `packages/validation/README.ko.md`, [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`docs/contracts/nestjs-parity-gaps.ko.md`](./contracts/nestjs-parity-gaps.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`book/beginner/ch06-validation.ko.md`](../book/beginner/ch06-validation.ko.md)로 나뉜다. `@fluojs/validation`은 표준 데코레이터 기반 입력 검증, DTO 실체화, 호출자 속성을 대체하지 않고 plain nested 값을 임시로 실체화할 수 있는 validation-only check, mapped DTO helper, 그리고 `@fluojs/validation/mapped-types` helper subpath를 담당한다. `@ValidateNested(...)`는 `each: true` 없이 array, `Set`, `Map` member를 재귀적으로 검증하고 실체화한다. NestJS `@ValidateNested()`와 class-transformer `@Type(() => ChildDto)` 조합은 명시적인 fluo `@ValidateNested(() => ChildDto)`로 바꿔야 하며, fluo는 class-transformer 또는 reflected design metadata를 소비하지 않는다. 일반 field validator는 `null`과 `undefined`를 건너뛰고, `@IsDefined()`가 필수 여부를 담당하며, plain 객체 materialization은 안전한 own enumerable 추가 속성을 기본적으로 유지하고 `{ undeclaredProperties: 'reject' }`로 명시적인 재귀 거부를 지원하며, class-validator 스타일 `groups` / `always` 실행은 지원하지 않는다. 이는 NestJS `ValidationPipe`와 일대일로 호환되는 동작이 아니라 fluo 고유 계약이다. Mapped DTO helper는 `@fluojs/validation` 또는 mapped-types subpath에서 가져온다. Subset 및 partial helper는 class-level validator를 복사하지 않고 field-level metadata를 보존하는 반면, `IntersectionType`은 모든 input DTO의 class-level rule을 보존한다. 릴리스 안정성은 이러한 migration behavior 격차와 별개입니다. 공개 `1.0+` 패키지는 Official이고 breaking change에는 major version bump가 필요하며, 현재 stable `@fluojs/validation` `1.x` 릴리스도 이 계약을 따릅니다. Parity map에서는 더 넓은 생태계 성숙도를 별도 격차로 유지합니다.
OpenAPI package-surface discoverability는 `packages/openapi/README.ko.md`, [`docs/architecture/openapi.ko.md`](./architecture/openapi.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`docs/contracts/nestjs-parity-gaps.ko.md`](./contracts/nestjs-parity-gaps.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`book/beginner/ch10-openapi.ko.md`](../book/beginner/ch10-openapi.ko.md)로 나뉜다. `@fluojs/openapi`는 descriptor-driven OpenAPI 3.1.0 문서 생성, `OpenApiModule.forRoot(...)` 또는 `forRootAsync(...)`를 통한 명시적 `sources`/`descriptors` 등록, discovered source 뒤에 explicit descriptor를 합성하는 later-descriptor operation 우선순위, 기본값을 보존하며 여러 document instance의 정규화된 HTTP collision을 fail-fast 처리하는 설정 가능한 `documentPath`/`uiPath` runtime route, 설정 가능한 `swaggerUiAssets`를 갖춘 `ui: true` opt-in Swagger UI 제공, 추론된 path/query/header/cookie/body 문서를 대체하는 request decorator override, handler return shape 추론 대신 명시적인 response content metadata, legacy `nullable` 및 boolean exclusive bound를 거부하고 OpenAPI 3.1 null union과 finite numeric bound를 요구하는 동작, 기본 error response policy 처리, 그리고 문서가 제공 및 정규화되기 전의 `documentTransform` hook을 담당한다. 이 패키지는 자체적으로 검증한 지원 계약으로 `engines.node >=24.0.0 <27`을 선언하며 portable `@fluojs/runtime`에는 package-wide Node engine이 없다.

GraphQL package-surface discoverability는 `packages/graphql/README.ko.md`, [`examples/graphql/README.ko.md`](../examples/graphql/README.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`book/intermediate/ch18-graphql.ko.md`](../book/intermediate/ch18-graphql.ko.md)로 나뉜다. `@fluojs/graphql`은 모든 decorated root 또는 object-field resolver class가 compiled module의 provider로 등록된 뒤에만 이를 discovery한다. 선택적 `GraphqlModule.forRoot({ resolvers })` list는 이 discovery에 allowlist filter로 적용되며, 생략하거나 빈 list를 전달하면 등록된 candidate를 모두 허용한다. `OperationScopedDataLoader.create(batch, options)`는 canonical operation 범위 loader accessor 경로이며, package는 `createDataLoader`를 export하거나 upstream `DataLoader` constructor를 재export하지 않는다. `GraphqlModule.forRootAsync({ inject, useFactory })`는 GraphQL lifecycle이 시작되기 전에 application context마다 GraphQL option을 한 번 해석하고, 명시적인 `inject` token과 `useFactory`만 지원하며, NestJS 스타일의 `imports`, `useClass`, `useExisting`, 암시적 discovery는 거부한다. 제한된 code-first resolver surface에는 두 category가 있다. Root operation은 `@Query`, `@Mutation`, `@Subscription`을 사용하고, object field resolver는 `@Resolver('TypeName')`과 `@FieldResolver(...)`로 연결하며, decorator field name은 문자열 shorthand 대신 `{ fieldName }` option object를 사용한다. 표준 데코레이터에는 parameter decorator가 없으므로 TC39 표준 `@Args(...)`, `@Parent(...)`, `@Context(...)` method decorator가 explicit positional argument를 바인딩한다. Field target은 code-first root output에서 도달 가능해야 하고 중복 `TypeName.fieldName` 등록은 실패하며, field argument DTO binding은 `@FieldResolver({ input: InputDto, argTypes? })`와 `@Args(index?)`로 지원한다. `input`과 `@Args()`는 서로 필요하고 binding index는 고유해야 하며 이 binding은 root operation에서 유효하지 않다. Schema-first field-resolver attachment는 계속 지원하지 않는다. `@Subscription({ topics })` 계약도 제공하지 않는다. TypeScript 반환 타입은 GraphQL schema를 정의하지 않는다. Operation output type을 생략하면 GraphQL `String`을 사용하고, object 결과에는 `outputType`, array에는 `outputType: listOf(ItemType)`이 필요하다. Package는 자체적으로 검증한 지원 계약으로 Node.js `>=24.0.0 <27`을 선언한다. Portable `@fluojs/runtime`과 `@fluojs/config`에는 package-wide `engines.node`가 없으며 config의 Node 전용 env-file 및 watch 기능은 실행 시점에 보호된다. HTTP와 SSE는 이 지원 runtime 경계 안에서 Web-standard HTTP seam을 사용하며, 선택적 WebSocket subscription에는 upgrade listener를 제공하는 server-backed Node HTTP/S adapter도 필요하다. Dependency metadata 정렬과 native runtime 검증이 없는 Bun, Deno, Cloudflare Workers는 지원되는 GraphQL package runtime이 아니다.
...
React page catalog 및 route diagnostics discoverability는 `packages/react/README.ko.md`, `packages/runtime/README.ko.md`, `packages/cli/README.ko.md`, `packages/studio/README.ko.md`, [`docs/contracts/testing-guide.ko.md`](./contracts/testing-guide.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/toolchain-contract-matrix.ko.md`](./reference/toolchain-contract-matrix.ko.md)로 나뉜다. `@Path(...)`는 runtime-neutral `react-page` marker를 기록하고, runtime은 authoritative compiled `HandlerDescriptor` 값을 immutable route inspection entry로 투영하며, `createReactPageCatalog(...)`는 HTTP matching, conflict detection, dispatch에 참여하지 않고 marker가 있는 React page만 노출한다. Stable `@fluojs/react/typegen` tooling subpath는 이 catalog를 deterministic path-only TypeScript declaration, absolute href builder, route-bound real-anchor props, 기존 client API에 일반 href string을 resolve하는 typed `push`/`replace` method로 변환하고 versioned route를 거부한다. `fluo typegen --check`는 stable status code를 가진 exact non-mutating CI comparison을 제공하고 bounded `--watch` generation은 coalesced module-directory change를 serialize하며 마지막 valid atomic artifact를 보존한다. Canonical consumer testing loop는 React-specific testing runtime을 추가하지 않고 render-policy unit, real `Test.createApp(...)` dispatch, generated-route compile fixture, hydration mismatch reporting, JavaScript-disabled native form coverage를 결합한다. Adapterless `fluo inspect`는 같은 compiled route를 직렬화하고, Studio는 live/static artifact 양쪽에서 `kind`와 parameter name만 담는 `params` field를 검증하고 표시하며 legacy entry는 일반 HTTP diagnostics로 기본 처리한다.

Serialization response-ownership discoverability는 `packages/serialization/README.ko.md`, `packages/runtime/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`book/beginner/ch07-serialization.ko.md`](../book/beginner/ch07-serialization.ko.md), package-local `packages/serialization/src/serializer-interceptor.test.ts` unit coverage, 그리고 `packages/runtime/src/application.test.ts`의 runtime request-pipeline coverage로 나뉜다. `@fluojs/serialization`은 decorator-aware response serialization, `excludeExtraneous` 같은 class-level `Expose(...)` option을 표현하는 `ExposeClassOptions`, `TransformFunction`, `serialize(value)`, HTTP `SerializerInterceptor` response-boundary 통합을 담당한다. Runtime은 commit되지 않은 framework-managed 결과를 소유하므로 success response를 쓰기 전에 `SerializerInterceptor`가 이를 shaping할 수 있다. Handler 또는 runtime response helper가 `RequestContext.response`를 commit한 뒤에는 `SerializerInterceptor`가 serialization을 우회하고 `next.handle()`에서 받은 값을 그대로 반환한다. 다른 interceptor는 chain 결과를 계속 변환할 수 있다. 이와 별개로 dispatcher는 commit된 response를 확인하고 두 번째 success-response write를 건너뛴다. Request-facing interceptor behavior는 serialization-to-testing workspace dependency edge를 추가하지 않고 `FluoFactory.create(...)`과 `app.dispatch(...)`를 쓰는 runtime request pipeline에서 계속 검증해야 한다.

정식 패키지 및 런타임 범위는 [`docs/reference/package-surface.md`](./reference/package-surface.md)에 있으며, `@fluojs/core`에만 의존하고 Node.js engine floor가 없으며 `I18nModule.forRoot(...)`로 `I18nService`를 기본 global provider로 등록하되 `global: false`를 module-local opt-out으로 제공하는 fluo-native `@fluojs/i18n` root package boundary, plural/select localization을 위한 `@fluojs/i18n/icu` ICU MessageFormat subpath, `@fluojs/i18n/http` HTTP locale helper 및 opt-in `Accept-Language` policy helper, opt-in non-HTTP locale resolution과 header policy helper를 위한 `@fluojs/i18n/adapters` subpath, `@fluojs/i18n/validation` validation localization, opt-in remote cache wrapper를 포함한 `@fluojs/i18n/loaders/fs` 및 `@fluojs/i18n/loaders/remote` catalog loader, `@fluojs/i18n/typegen` catalog key 및 typed translation helper declaration generation 같은 core 추가 항목, portable `@fluojs/runtime` root와 `@fluojs/runtime/web`, Node-owned `@fluojs/platform-nodejs` root 및 internal seam, runtime-neutral `@fluojs/runtime/internal*` package-integration seam 경계, `@fluojs/serialization`의 decorator-aware response serialization, output DTO shaping, HTTP `SerializerInterceptor` response-boundary 통합 같은 request-pipeline 책임, TCP, Redis Pub/Sub, Redis Streams, NATS, Kafka, RabbitMQ, MQTT, gRPC를 지원하는 `@fluojs/microservices` messaging 책임, bootstrap-time handler discovery를 갖춘 `@fluojs/cqrs` command/query bus, saga orchestration 및 event-bus delegation, optional Redis Pub/Sub transport, inherited event channel fan-out, bounded publish cancellation/timeout, local publish와 inbound transport callback 모두에 대한 shutdown drain semantic을 갖춘 `@fluojs/event-bus` in-process domain event fan-out, `@fluojs/mongoose`의 ALS/session transaction 책임 같은 persistence 책임도 포함한다. 작업 기반 패키지 발견성은 [`docs/reference/package-chooser.md`](./reference/package-chooser.md)에 있으며, `@fluojs/i18n`과 그 ICU, HTTP locale policy, adapters header policy, validation, loader/cache, typegen subpath를 위한 localization/i18n 선택 guidance, `@fluojs/serialization`을 위한 response serialization 및 output DTO shaping 선택 guidance, `@fluojs/event-bus/redis` 및 `@fluojs/redis`를 함께 쓰는 optional cross-process `@fluojs/event-bus` fan-out 선택 guidance, `@fluojs/terminus`, Node memory/disk indicator용 `@fluojs/terminus/node` subpath, Redis indicator용 `@fluojs/terminus/redis` subpath, `execution.indicatorTimeoutMs` slow-indicator timeout guardrail을 위한 operations guidance도 포함한다. NestJS i18n, i18next, next-intl, request/validation convenience glue에 대한 ecosystem bridge compatibility decision은 [`docs/reference/i18n-ecosystem-bridges.ko.md`](./reference/i18n-ecosystem-bridges.ko.md)에 있다.

NestJS i18n migration discoverability는 `packages/i18n/README.ko.md`, [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`docs/reference/i18n-ecosystem-bridges.ko.md`](./reference/i18n-ecosystem-bridges.ko.md)로 나뉜다. NestJS runtime bridge를 추가하거나 framework-agnostic root contract를 바꾸지 않고, 비동기 catalog 또는 configuration loading을 application-owned bootstrap boundary에서 완료한 뒤 동기 `I18nModule.forRoot(...)` registration을 수행한다. Standalone service 및 catalog loader 인스턴스는 정규 정적 create 메서드(`I18nService.create(...)`, `IcuI18nService.create(...)`, `FileSystemI18nLoader.create(...)`, `RemoteI18nLoader.create(...)`, `CachedRemoteI18nLoader.create(...)`)로 구성하며 생성자는 DI를 위해 public으로 유지된다. Migration guide의 namespace-preserving catalog aggregation과 명시적 fallback conversion을 사용한다. Application-owned request boundary에서는 정규 `createAcceptLanguageLocalePolicyResolver(...)`로 `Accept-Language` 헤더 정책을 처리하며 ordered `resolveHttpLocale(...)` resolver chain을 실행하고, `getHttpLocale(...)`로 해당 `RequestContext`의 선택된 locale을 읽어 translation 또는 `localizeDtoValidationError(...)`에 명시적으로 전달한다. Validation helper는 transport-agnostic 상태를 유지하며 request 또는 global locale state를 암묵적으로 읽지 않는다.

Microservices discoverability는 `packages/microservices/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [RabbitMQ](../book/intermediate/ch04-rabbitmq.ko.md), [Kafka](../book/intermediate/ch05-kafka.ko.md), [NATS](../book/intermediate/ch06-nats.ko.md) transport chapter, root/subpath export 테스트, transport regression suite로 나뉜다. 애플리케이션 등록은 `MicroservicesModule.forRoot(...)`에 남고, `MICROSERVICE`는 raw transport instance가 아니라 programmatic `Microservice` facade를 노출한다. `send()`는 상관관계가 유지된 원격 응답에서 settle하고, `emit()`은 원격 handler 완료가 아니라 outbound transport publication이 완료될 때 settle한다. Kafka와 RabbitMQ는 별도로 handler 실행과 response publication이 끝날 때까지 inbound consumer callback을 pending 상태로 유지한다. `Microservice.close(signal?: string)`은 shutdown signal을 `MicroserviceLifecycleService.close(signal)`로 전달하지만, 설정된 transport는 signal-aware shutdown을 문서화하지 않는 한 `close(): Promise<void>`를 유지하며, caller-owned NATS, Kafka, RabbitMQ collaborator는 facade shutdown 뒤 애플리케이션이 close 또는 drain해야 한다. Transport-specific subpath는 TCP, Redis Pub/Sub, Redis Streams, NATS, Kafka, RabbitMQ, MQTT, gRPC를 노출한다. 각 transport class와 option은 전용 subpath에서만 import하고 `create(...)` factory를 canonical construction path로 사용한다. Root barrel은 registration, lifecycle, decorator, status, shared type만 export한다. Root TCP import는 listen 또는 outbound socket construction 경로 전까지 `node:net`을 lazy하게 유지하고, request/reply 및 event payload는 handler dispatch 전에 clone되며, TCP `port: 0`은 outbound 호출을 OS가 할당한 listener port로 라우팅한다. Internally-created gRPC server는 transport가 shutdown하고, supplied gRPC server는 caller-owned로 유지되는 반면 cached outbound client는 shutdown과 status diagnostics에서 framework-owned로 유지되며, internally-created MQTT client는 listen setup 실패 또는 실패한 in-flight listen을 close가 unwinding하는 경로에서 닫히고, 직접 `@fluojs/core/internal` import는 애플리케이션-facing surface가 아니라 metadata/clone helper를 위한 first-party package-integration seam으로 취급한다.

Microservices transport safety guidance는 `packages/microservices/README.ko.md`, [`docs/architecture/lifecycle-and-shutdown.ko.md`](./architecture/lifecycle-and-shutdown.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [microservices introduction](../book/intermediate/ch01-microservices-intro.ko.md)에 동일하게 반영된다. Facade ingress evidence는 `packages/runtime/src/microservice-application.lifecycle.test.ts`, `packages/microservices/src/lifecycle-regression.test.ts`, `packages/microservices/src/stream-shutdown-regression.test.ts`에 있고, transport evidence는 `nats-transport.cleanup.test.ts`, `nats-transport.request-callback.test.ts`, `tcp-transport.test.ts`, `tcp-transport.utf8-framing.test.ts`, `grpc-transport.test.ts`에 남는다. `close()` 시작은 `listen()`이 pending 상태여도 새 `send()`, `emit()`, `serverStream()`, `clientStream()`, `bidiStream()` 호출을 transport handoff 전에 거부하는 terminal facade gate를 설정하며, runtime shell은 terminal gate를 `send()`와 `emit()`에 적용한다. Transport는 자체 shutdown guard를 유지한다. NATS는 소유한 모든 subscription cleanup을 시도하고, 여러 실패를 aggregate하며, 실패한 subscription만 listen을 다시 시작하기 전 재시도를 위해 유지한다. NATS request subscription callback boundary는 malformed frame, response encoding 실패, 예외를 던지는 `respond()` callback을 격리하고 raw console fallback 없이 설정된 transport logger로 보고하며 caller-owned NATS client를 열린 상태로 유지한다. TCP는 불완전한 newline-delimited frame을 완성된 각 프레임을 UTF-8로 한 번 디코딩할 수 있을 때까지 byte 상태로 유지하고, frame을 1 MiB로 제한하며, `port: 0` outbound 작업을 OS가 할당한 listener로 라우팅하고, 동시에 또는 반복해서 호출된 `close()`가 첫 shutdown promise를 공유해 listener와 socket teardown을 한 번만 수행한다. gRPC는 unary call이 settle한 뒤, streaming call이 reader iteration 시작 전이라도 end/error로 끝난 뒤, 또는 reader가 early return한 뒤 각 `AbortSignal` abort listener를 제거하며, terminal 경로가 겹쳐도 cleanup은 한 번만 수행한다.
...
[패키지 README](../packages/event-bus/README.ko.md)는 `EventBusService.publish(event, options?): Promise<EventPublishResult>`를 유일한 event-bus 발행 API로 정의한다. 각 결과는 로컬 handler와 outbound transport outcome을 보고하며 discovery/preparation 실패는 promise를 reject하고 lifecycle 거부는 `{ status: 'rejected', reason }`로 resolve한다. `background` 작업은 shutdown 추적에 남고 호출자의 제한된 관측보다 오래 실행될 수 있으며, timeout과 cancellation은 관측만 끝내고 시작된 작업을 취소하지 않는다. 시작 전 cancellation은 해당 작업을 건너뛴다. Shutdown은 구성된 deadline 안에서 awaited 작업과 background 작업을 drain한다. 로깅은 outbound에만 적용되는 안전한 로깅으로 target/status 정보만 기록하고 raw handler/transport error와 handler 반환값은 기록하지 않으며, 애플리케이션 소유 로그는 애플리케이션의 책임이다.

소비자 정책은 [인증 후 last-used bookkeeping과 필수 결과 검사](../apps/docs/content/docs/guides/messaging-workflows.mdx), 현재 학습 적용은 [FluoShop 도메인 이벤트](../book/02-fluoshop/ch13-domain-events.ko.md), 실행 근거는 [publish-results 예제](../packages/event-bus/examples/publish-results.ts)와 [결과](../packages/event-bus/src/publish-result.test.ts)·[bound](../packages/event-bus/src/publish-result-bounds.test.ts)·[lifecycle](../packages/event-bus/src/publish-result-lifecycle.test.ts) 테스트를 따른다. 인증은 이미 성공한 상태이며 이벤트에는 credential이 아닌 token record ID만 담는다. 소유자 검증 명령은 `pnpm --dir packages/event-bus test`와 `pnpm --filter '@fluojs/event-bus...' build`다.
...
Vite tooling discoverability는 `packages/vite/README.ko.md`, [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md), [`docs/reference/toolchain-contract-matrix.ko.md`](./reference/toolchain-contract-matrix.ko.md)로 나뉩니다. Node.js `>=24.0.0 <27` Vite 프로젝트가 TC39 표준 데코레이터가 포함된 TypeScript를 Babel의 `@babel/plugin-proposal-decorators` `{ version: '2023-11' }` transform 및 `@babel/preset-typescript`로 빌드해야 하고 fluo의 Vite 파일 경계 skip을 유지해야 할 때 `@fluojs/vite`를 사용합니다. 생성된 non-Deno starter는 두 config 파일 모두에서 `@fluojs/vite`를 사용합니다. `vite.config.ts`는 애플리케이션 파일에 `fluoDecoratorsPlugin()`을 사용하고, `vitest.config.ts`는 test/spec module과 그 애플리케이션 import에 `fluoDecoratorsPlugin({ sourceMaps: true, transformBoundary: 'test' })`를 사용합니다. decorator가 있는 모든 변환 module은 선언을 평가하기 전에 `@fluojs/core/metadata-preload`를 preload하며, Vite `>=6.2.0`과 `@babel/core`, `@babel/plugin-proposal-decorators`, `@babel/preset-typescript`의 lazy transform-hook 진단은 이 단일 public transform에 적용됩니다. 워크스페이스 suite는 Vite 8.2.2/Rolldown에서 실행되고, `packages/vite/src/vite8-rolldown.test.ts`는 `enforce: 'pre'` 순서를 고정하면서 field-decorator metadata를 빌드하고 실행한다.

Generated starter tooling은 [`docs/reference/fluo-new-support-matrix.ko.md`](./reference/fluo-new-support-matrix.ko.md)와 [`docs/reference/toolchain-contract-matrix.ko.md`](./reference/toolchain-contract-matrix.ko.md)가 정의한다. Non-Deno starter는 Vite `^8.2.2`, Vitest와 `@vitest/coverage-v8` `^4.1.11`, ESM `build.rolldownOptions`를 사용하며 application/test Babel decorator boundary를 유지한다. `pnpm --dir packages/cli sandbox:matrix`는 React SSR, Node HTTP, TCP microservice, mixed matrix에서 생성 프로젝트의 install, build, test와 실행 가능한 HTTP DTO field binding을 검증하고, microservice scenario는 non-HTTP message-routing test를 유지한다. Node.js `>=24.0.0 <27`과 runtime-native metadata는 바뀌지 않는다.

Testing package discoverability는 `packages/testing/README.ko.md`, [`docs/contracts/testing-guide.ko.md`](./contracts/testing-guide.ko.md), [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`book/beginner/ch20-testing.ko.md`](../book/beginner/ch20-testing.ko.md)로 나뉜다. `@fluojs/testing`은 명시적 `rootModule` graph를 요구하는 app helper를 가진 Node.js `>=24.0.0 <27` 패키지이며, request-facing guard/interceptor override는 dispatch 또는 `Test.createApp(...)`으로 검증하고, `Test.createApp()` cleanup은 `finally`에 두며, request-level helper는 `app.request(...).send()`와 `TestingModuleRef.dispatch(...)` 양쪽에서 isolated request-scoped DI를 보존하고, `@fluojs/testing/http`는 문서화된 request helper subpath이며, portability harness는 setup/assertion cleanup failure를 보존하고 truthful listener floor를 통해 body-bearing RFC `QUERY`를 실행하며, `ShallowMocked<T>`는 문서화된 Vitest-compatible root/types/mock boundary의 비재귀 mock type이다. `@fluojs/testing/mock`은 shallow proxy mock을 위한 `ShallowMock.create(...)`와 prototype-method spy를 위한 `PrototypeMock.create(Type)`을 노출한다. `mockToken(token, value)`는 tuple이 아닌 공개 `ValueProvider` descriptor 헬퍼로 유지되며 override에는 descriptor 대신 값을 사용한다. Non-mock 생성은 root/module entrypoint의 `Test`와 harness subpath에 둔다.

HTTP runtime discoverability는 `packages/http/README.ko.md`, [`docs/architecture/http-runtime.ko.md`](./architecture/http-runtime.ko.md), NestJS migration docs, intermediate Express/Node book chapter로 나뉜다. Package README는 DTO field-level binding decorator, 동기 callback 반환/throw 동작을 보존하면서 promise continuation을 격리하는 lazy `RequestContext` storage resolution, managed SSE `AsyncIterable` 지원, `FrameworkRequest.signal`과 `FrameworkRequest.isAborted()` cancellation surface를 모두 보존하는 dispatcher 동작을 문서화한다. [`docs/architecture/http-runtime.ko.md`](./architecture/http-runtime.ko.md)는 정식 request lifecycle 및 abort check ordering을 담고, request observer는 module-level 및 application-level middleware가 `await next()` 이후 작업까지 모두 settle한 뒤에만 성공한 request를 관찰하며, `next()` 이후 middleware failure는 앞선 `onRequestSuccess` 없이 `onRequestError`를 발생시키고, `onRequestFinish`는 어느 outcome에서든 그 뒤에 실행된다. 수동 SSE는 명시적 또는 raw stream close에서 성공으로 완료되어 `onRequestSuccess`, `onRequestFinish`, request-scope disposal 순서로 진행하며, request abort는 completion 뒤 cancellation을 다시 검사해 success를 건너뛴 뒤 finish와 disposal을 수행한다. `packages/platform-express/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`book/intermediate/ch21-express-node.ko.md`](../book/intermediate/ch21-express-node.ko.md)는 Express host compatibility boundary를 담는다. 즉 Express는 platform engine으로 남고, application middleware는 fluo `Middleware`이며, native Express/Connect `(req, res, next)` function은 portable fluo middleware가 아니고, `getServer()` / `getListenTarget()` / `getRealtimeCapability()`는 infrastructure-boundary helper에 머물며, 시작된 adapter에 대한 중복 `listen()` 호출은 live dispatcher를 보존하는 idempotent 동작이다. `packages/platform-nodejs/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`book/intermediate/ch21-express-node.ko.md`](../book/intermediate/ch21-express-node.ko.md), `apps/docs/content/docs/guides/runtime-adapters.mdx`는 raw Node.js adapter boundary를 담는다. 즉 `@fluojs/platform-nodejs`는 `engines.node >=24.0.0 <27`을 선언하는 Node.js 패키지이며, concrete `NodeHttpApplicationAdapter`와 `NodeHttpAdapterOptions`, 직접 Node `http`/`https` listener ownership, truthful RFC `QUERY` ingress, retry/body-limit/shutdown 계약, `x-correlation-id` error fallback coverage, 명시적 `multipart.maxTotalSize` override coverage, non-racy `port: 0` signal-shutdown regression coverage를 함께 문서화한다. 같은 migration/book surface는 `SseResponse`, `AsyncIterable`, 지원되지 않는 Observable return conversion의 SSE migration/learning-path 구분과 함께 NestJS Pipe/`ValidationPipe` 가정을 `@RequestDto(...)`, field-level `@From*` decorator, `@Convert(...)`, DTO materialization 이후 validation으로 옮기는 DTO binding boundary도 담는다.

Express SSE handler는 [`apps/docs/content/docs/guides/runtime-adapters.mdx`](../apps/docs/content/docs/guides/runtime-adapters.mdx)에 문서화되어 있습니다. `@Sse('events')`를 사용하고 `(_input: undefined, context: RequestContext)`를 받아 그 context로 `SseResponse`를 생성하세요. NestJS 스타일 `@Res()` parameter injection은 지원하지 않습니다.
...
스트리밍 멀티파트 discoverability는 `packages/runtime/README.ko.md`와 [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)에 나뉜다. `@fluojs/runtime/web`은 요청 body마다 buffered `parseMultipart(...)`와 streaming `parseMultipartStream(...)`을 서로 배타적으로 유지하고, single-consumer file `ReadableStream`을 가진 typed field/file part를 노출하며, Node.js, Express, Fastify, Bun, Deno, Cloudflare Workers에서 같은 limit, abort, cancellation, parser-failure cleanup을 보존한다.

HTTP custom-method discoverability는 `packages/http/README.ko.md`, [`docs/architecture/http-runtime.ko.md`](./architecture/http-runtime.ko.md), [`book/beginner/ch05-routing-controllers.ko.md`](../book/beginner/ch05-routing-controllers.ko.md), `packages/testing/README.ko.md`, Fastify/Bun adapter README에서 동기화한다. `@Query(path)`는 RFC `QUERY`를 등록하고 `@Route(method, path)`는 HTTP token을 검증해 uppercase로 canonicalize하며 framework-reserved `ALL` sentinel을 거부한다. Exact method는 `@All(...)`보다 먼저 매칭되고 custom method는 일반 versioning, DTO binding/validation, 기본 `200` behavior를 유지하며 shared portability harness는 body-bearing `QUERY`와 `PURGE`를 real listener 또는 fetch dispatch seam으로 실행한다. Fastify는 wildcard fallback이 custom method를 받을 수 있도록 method 이름만 등록하고 Bun은 native `routes` 가속에서 제외하며, `CONNECT`는 일반 routing conformance 범위 밖에 남고 custom method가 자동으로 OpenAPI Path Item operation이 되지는 않는다.

HTTP fast-path scope identity는 `packages/http/README.ko.md`, [`docs/architecture/http-runtime.ko.md`](./architecture/http-runtime.ko.md), [`book/advanced/ch11-request-pipeline.ko.md`](../book/advanced/ch11-request-pipeline.ko.md)에서 동기화한다. Dispatcher-local fast-path cache는 handler method metadata만 소유하고, 매 dispatch가 active DI container를 통해 controller를 resolve하므로 singleton 공유는 container가 계속 소유하며 transient controller 또는 dependency는 resolution마다 새로운 identity를 유지한다.

HTTP trusted connection identity는 `packages/http/README.ko.md`, [`docs/architecture/http-runtime.ko.md`](./architecture/http-runtime.ko.md), NestJS migration map에서 동기화한다. adapter는 제공 가능한 경우 direct transport peer와 protocol을 `FrameworkRequest.connection`에 snapshot하고, `resolveHttpConnection(request, { trustProxy })`는 선택된 immutable client address, direct remote address, trusted proxy chain, protocol, security, host identity를 노출한다. Forwarding metadata는 direct peer가 명시적인 hop-count, CIDR/address-list 또는 predicate `trustProxy` boundary를 만족한 뒤에만 이 identity에 영향을 줄 수 있고, malformed forwarding data는 direct transport identity로 fail-closed 된다. fetch-style HTTPS `Request`는 Node transport parity가 아니다. adapter가 제공한 `connection` snapshot이나 명시적 header가 없으면 peer, host, port가 없고 `resolveHttpConnection(...)`은 URL에서 HTTPS, `secure`, host, port를 추론하지 않는다. `trustProxyHeaders: true`는 광범위한 legacy full-chain compatibility 전용이며 새 deployment에는 권장하지 않는다.

HTTP cancellation 및 request-context isolation evidence는 `packages/http/src/dispatch/dispatcher-cancellation.test.ts`, `packages/http/src/dispatch/dispatcher-sse-backpressure-cancellation.test.ts`, `packages/http/src/context/request-context-isolation.test.ts`, package의 기존 dispatcher/context suite에 있다. Dispatcher는 `FrameworkRequest.signal`과 `FrameworkRequest.isAborted()`를 독립된 cancellation surface로 취급하고 어느 한쪽이라도 abort를 보고하면 중단한다. Managed SSE는 request abort와 response-stream close를 settle되지 않은 `waitForDrain()` backpressure wait에도 적용하고, iterator를 정확히 한 번 닫으며, stream write 또는 drain error를 대체하지 않은 채 request-scope disposal 전에 cleanup을 기다린다. Runtime-specific Node와 Bun root entry는 helper를 노출하기 전에 host async storage를 등록하므로 promise를 반환하는 non-async callback은 동기 호출/반환/throw 동작을 유지하고 요청이 겹쳐도 각 continuation을 request-local하게 보존한다. Portable entry는 Node built-in import 없이 유지되고, request-context helper는 `Promise.prototype`을 patch하지 않으며 관련 없는 continuation은 request context를 관찰하지 않는다.

Express native middleware seam은 construction-time에 제공하는 platform-specific `nativeMiddleware` adapter 옵션입니다. Adapter가 Express application을 직접 생성하고 소유하므로 기존 Express application을 채택하거나 재사용하는 방식은 지원하지 않습니다. Express/Connect request 및 error handler를 Express Router와 fluo dispatch보다 먼저 배열 순서대로 mount하며, `next()`는 fluo로 계속 진행하고 native response는 dispatch 전에 종료되며 native error는 Express error chain에 남고 native handler가 캡처한 resource는 애플리케이션이 소유합니다. bootstrap 이후 `use(...)`로 native stack에 middleware를 추가하는 방식은 지원하지 않습니다. 이식 가능한 동작은 fluo `Middleware`로 재작성하는 방식을 우선하세요.

Express runtime 및 NestJS migration boundary discoverability는 `packages/platform-express/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`book/intermediate/ch21-express-node.ko.md`](../book/intermediate/ch21-express-node.ko.md), website runtime adapter guide에서 동기화된다. `@fluojs/platform-express`는 `engines.node >=24.0.0 <27`을 선언하는 Node.js 패키지이고, HTTP host 교체는 NestJS legacy decorator 또는 reflection metadata semantics를 보존하지 않으며, controller와 provider는 먼저 TC39 표준 데코레이터, class-level `@Inject(...)`, 명시적 DI/module wiring으로 이동해야 하고, `getListenTarget()`은 `getServer()`, `getRealtimeCapability()`와 함께 infrastructure-boundary helper로 유지된다.

HTTP learning-path alignment는 [`book/advanced/ch11-request-pipeline.ko.md`](../book/advanced/ch11-request-pipeline.ko.md), [`book/advanced/ch13-custom-adapter.ko.md`](../book/advanced/ch13-custom-adapter.ko.md), [`book/beginner/ch05-routing-controllers.ko.md`](../book/beginner/ch05-routing-controllers.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)가 담당한다. Request context는 host가 async-context primitive를 제공할 때만 비동기로 유지되고 그 외에는 synchronous-only fallback을 사용하며, dispatch abort check는 임의의 application work를 자동 취소하는 기능이 아니라 문서화된 boundary이고, dispatcher는 승격된 request scope를 dispose하지만 임의의 app-owned file이나 stream은 닫지 않는다. Custom adapter 예제는 완전한 `FrameworkRequest` / `FrameworkResponse` shape를 만족하고 `setStatus(...)`를 사용해야 하며, route hierarchy는 nested Controller가 아니라 route path에 직접 표현하고, NestJS controller parameter decorator는 DTO field binding과 low-level 접근이 필요할 때의 명시적 `RequestContext` parameter로 마이그레이션한다.

Fastify adapter discoverability는 `packages/platform-fastify/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md), `apps/docs/content/docs/guides/runtime-adapters.mdx`, [`book/advanced/ch13-custom-adapter.ko.md`](../book/advanced/ch13-custom-adapter.ko.md), beginner prerequisite/bootstrap/production chapter로 나뉜다. `@fluojs/platform-fastify`는 기본 고성능 Node.js `>=24.0.0 <27` adapter 경로이고 `engines.node >=24.0.0 <27`을 선언하며 Fastify 기반 Node `http`/`https` listener를 소유하고, 프로세스가 TLS를 직접 종료할 때 `FastifyHttpApplicationAdapter.create(...)`을 통해 Node.js `https` server option을 받는다. 이 하한은 RFC `QUERY` ingress를 정확하게 유지한다. Website Fastify section은 같은 bounded range를 반복하고 Node 21, 22.2.0 미만 Node 22, Node 27+를 제외한다고 안내한다. Website runtime guide와 beginner docs는 local startup을 일반 HTTP로 유지하면서 production TLS를 infrastructure termination으로 처리하거나 application configuration 또는 secret-management boundary에서 certificate material을 로드한 뒤 명시적인 adapter `https` option으로 전달하도록 안내하고, chapter package metadata는 이 계약을 가르치는 위치에서 Fastify package를 discoverable하게 유지한다.

Realtime Socket.IO discoverability는 `packages/socket.io/README.ko.md`, [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md), NestJS migration docs, intermediate Socket.IO book chapter로 나뉜다. 패키지 README는 `SocketIoModule.forRoot(...)`, runtime-neutral `SocketIoHandshakeRequest` root type, `true` / `undefined` / return 없음은 허용하고 `false` / `SocketIoGuardRejection`은 거부하는 guard 계약, deny-by-default CORS, positive-integer numeric option validation, Node-backed path와 Bun engine path의 일관된 configured-transport enforcement, Node.js `>=24.0.0 <27` server-backed 및 공식 Bun engine runtime limit, Bun static CORS와 모든 runtime의 no-`serverBacked` gateway caveat, namespace와 Engine.IO path mapping 및 singleton-only gateway discovery, Deno/Workers unsupported status, shutdown 중 adapter-owned/shared HTTP listener ownership, bounded accepted-work drain과 force-disconnect/retry semantic, Socket.IO gateway return value가 무시되므로 필요한 명시적 ACK callback 처리, native multi-room/volatile emit을 위한 raw `SOCKETIO_SERVER` migration seam, `@fluojs/websockets` companion decorator 요구사항을 문서화한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 정식 `@fluojs/socket.io` runtime limitation 요약을 담는다.

Realtime WebSockets discoverability는 `packages/websockets/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md)로 나뉜다. 패키지 README는 runtime-neutral 루트 `@fluojs/websockets` authoring surface와 `@fluojs/websockets/node`의 `NodeWebSocketModule.forRoot(...)` 및 token-only `NodeWebSocketGatewayLifecycleService`를 사용하는 Node registration, 런타임별 서브패스 `@fluojs/websockets/node`, `@fluojs/websockets/bun`, `@fluojs/websockets/deno`, `@fluojs/websockets/cloudflare-workers`, 루트만 소유하는 공유 gateway decorator와 metadata authoring primitive 및 runtime subpath가 소유하는 registration과 runtime-specific projection, runtime lifecycle service가 구현하고 `@Inject(...)`로 `@fluojs/websockets/node`의 `NodeWebSocketGatewayLifecycleService`, 또는 다른 runtime subpath의 해당 `*WebSocketGatewayLifecycleService` token을 주입하는 type-only contract `WebSocketRoomService` 기반 room 관리, Node.js 기반 adapter만 적용하고 fetch-style runtime은 room broadcast에 backpressure policy를 적용하지 않는 room broadcast backpressure, positional `(payload, socket, request, socketId)` message input을 포함한 text/binary payload normalization, upgrade context/guard/rejection 및 gateway descriptor typed runtime seam, platform package가 소유하는 native host/socket/binding type, Node `IncomingMessage`와 fetch-style `Request` upgrade guard typing 차이, boolean, structured rejection object 또는 throw된 HTTP exception을 통한 rejected pre-upgrade guard outcome, raw WebSocket handler return value가 기본적으로 무시되며 reply는 socket send로 명시하거나 `replies: { mode: 'event-envelope' }`를 통해 `{ event, data? }` reply를 opt-in할 수 있다는 점, upgrade accept 직전에 다시 확인하는 terminal Node shutdown admission, shutdown 전후에 queue된 disconnect cleanup을 bounded drain 안에 유지하는 connection별 lifecycle state retention을 문서화한다.

Redis 통합의 discoverability는 책임별로 나뉜다. `packages/redis/README.ko.md`는 동기 `RedisModule.forRoot(...)` registration, `getRedisClientToken(name)`을 통한 named client token, `RedisModule.forRoot({ lifecycle })`의 connect/quit timeout guardrail, platform health/readiness status snapshot helper 책임, Pub/Sub subscriber에 전용 Redis 연결이 필요하다는 raw-client 규칙을 문서화한다. 각 `RedisModule.forRoot(...)` 등록은 최종 option으로 새 client를 생성하고 외부 client를 채택하지 않으며, 애플리케이션이 소유한 raw publisher와 duplicate subscriber client는 애플리케이션이 닫아야 한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 정식 `@fluojs/redis` surface 요약을 담고, [`book/intermediate/ch03-redis-transport.ko.md`](../book/intermediate/ch03-redis-transport.ko.md)는 Redis Pub/Sub과 Redis Streams 학습 경로를 설명하며 공유 command client를 subscribed Pub/Sub connection으로 재사용하면 안 되는 이유와 named subscriber registration으로 fluo가 subscriber lifecycle timeout을 소유하게 하는 방법을 포함한다. [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)는 NestJS Redis async-module 가정을 애플리케이션 경계로 옮긴 뒤 최종 option을 fluo의 동기 Redis module registration에 전달해야 한다고 기록한다. 또한 named token consumer는 importing module의 `providers`에 나열하기 전에 명시적으로 선언해야 하며, `@Inject(...)`는 constructor token만 선언하고 provider를 등록하지 않는다.

Observability discoverability는 metrics와 health 패키지로 나뉜다. `packages/metrics/README.ko.md`, [`docs/architecture/observability.ko.md`](./architecture/observability.ko.md), [`book/beginner/ch19-metrics.ko.md`](../book/beginner/ch19-metrics.ko.md)는 Node.js `>=24.0.0 <27` `@fluojs/metrics` 경계, `MetricsModule.forRoot(...)`, isolated/shared registry ownership, `MetricsService.getRegistry()`, re-exported `Registry` type, low-level `METER_PROVIDER` / `PrometheusMeterProvider`, `MeterProvider`, `MeterCounter`, `MeterGauge`, `MeterHistogram` 같은 meter abstraction type, shared-registry path-label compatibility check를 포함한 opt-in HTTP request collector, endpoint-scoped `endpointMiddleware`, 별도의 module-level `middleware`, built-in `/metrics` controller와 advanced `MetricsService.getRegistry().metrics()` caller 양쪽의 active-registry scrape refresh, Registry-scoped platform telemetry stale-series cleanup을 문서화한다. [`docs/contracts/deployment.ko.md`](./contracts/deployment.ko.md)는 metrics route를 안전하게 노출하기 위한 deployment expectation을 기록하고, `examples/ops-metrics-terminus/README.ko.md`는 현재 health/readiness/metrics project structure를 보여준다. `packages/terminus/README.ko.md`는 `TerminusModule.forRoot(...)`, indicator/provider registration, exported indicator/provider DI token, execution timeout, service-scoped in-flight indicator serialization, Prisma/Drizzle lifecycle-aware diagnostics, `@fluojs/terminus/redis`를 통한 Redis lifecycle-aware diagnostics를 문서화한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 metrics package manifest `engines.node >=24.0.0 <27` floor를 포함해 두 패키지의 정식 responsibility boundary를 담는다.
...
Cache-manager discoverability는 패키지 문서, governed package-surface docs, cache contract, beginner book으로 나뉜다. `packages/cache-manager/README.ko.md`는 동기 `CacheModule.forRoot(options)`, injected-factory `CacheModule.forRootAsync({ inject, useFactory, global? })`, store selection, DI로 주입받은 `CacheService`, cache decorator, low-level metadata helper export, platform status/diagnostic helper를 문서화하며 정규화 configuration type은 private provider-assembly 세부사항이다. Redis-backed caching은 optional `@fluojs/redis` integration에서 기본 또는 named raw client를 해석하거나 더 높은 우선순위의 `redis.client`로 애플리케이션 소유 `RedisCompatibleClient`를 받을 수 있고, top-level `keyPrefix`가 Redis key/reset namespace를 소유한다. `@CacheEvict(...)`는 일반 service-method interception 계약이 아니라 non-GET controller handler에서 `CacheInterceptor`가 소비하는 HTTP route metadata다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 패키지 자체의 지원 계약인 `engines.node >=24.0.0 <27` 선언을 포함해 정식 `@fluojs/cache-manager` responsibility boundary를 기록하고, [`docs/architecture/caching.ko.md`](./architecture/caching.ko.md)는 현재 cache behavior contract를 담으며, [`book/beginner/ch17-cache.ko.md`](../book/beginner/ch17-cache.ko.md)는 application-facing configuration flow를 설명한다. NestJS cache migration boundary는 `packages/cache-manager/README.ko.md`와 [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)에서 미러링한다. `ttl`은 초 단위이고 memory 경로 기본값은 `300`초, 나머지 store는 `0`이며, `ttl: 0`은 만료 없음을 뜻하고 음수나 유한하지 않은 값은 cache 읽기와 쓰기를 모두 건너뛰며, `@CacheTTL(...)`은 정적 숫자만 받고, `httpKeyStrategy`의 기본값은 query-aware `route+query`이며 `route`, function strategy 또는 `@CacheKey(...)`가 의도적인 대안이다. NestJS `isGlobal`은 module-local 기본값 `false`를 가진 `global`로 바뀌고, injected factory는 bootstrap runtime provider 또는 globally visible export가 필요하며 NestJS `imports`, `useClass`, `useExisting`은 지원하지 않는다. NestJS store adapter는 먼저 Fluo `CacheStore` 계약에 맞게 적응해야 하고 adapted custom store shutdown은 `close()`를 우선하며 `dispose()` 전용 store도 호환되고, 직접 전달한 `redis.client`는 애플리케이션 소유로 남는다.
...
Queue lifecycle 및 migration discoverability도 패키지 문서와 governed docs로 나뉜다. `packages/queue/README.ko.md`는 package manifest에 선언된 Node.js `>=24.0.0 <27` runtime floor, `QueueModule.forRoot(...)`, `global: false` worker/Redis reachability를 포함하는 `QueueModuleOptions.global` module-scope control, DI 전용 `clientName`이 아닌 application-supplied `ownershipNamespace`에 기반한 cross-scope worker ownership을 문서화한다. namespace가 없으면 같은 `jobName` backend를 공유할 가능성이 있다고 보수적으로 처리하며, 명시적으로 다른 namespace만 isolation을 확립하고 `ownershipEnforcement: 'reject'`는 compatible collision을 BullMQ resource 생성 전에 실패시킨다. 이 문서는 NestJS/Bull processor metadata에서 명시적 singleton `@QueueWorker(JobClass)`로 옮기는 migration, bootstrap-ready worker processor handoff, `workerShutdownTimeoutMs` bounded shutdown, dead-letter access, lifecycle status snapshot도 문서화한다. [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)는 NestJS Bull/BullMQ가 하나의 `queueName` 아래 여러 named job 값을 영속화할 수 있지만 fluo는 `jobName`을 queue name과 named job 양쪽에 사용하므로 `jobName`만으로 해당 topology나 영속 payload 및 metadata compatibility를 보존할 수 없다는 점을 기록한다. [`book/intermediate/ch11-queue.ko.md`](../book/intermediate/ch11-queue.ko.md)는 필요한 drain, payload 변환 후 다시 enqueue, 별도 queue cutover, cross-scope worker ownership rule, lifecycle checklist를 설명한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 정식 Queue responsibility 및 runtime boundary를 기록한다. [`docs/contracts/behavioral-contract-policy.md`](./contracts/behavioral-contract-policy.md)는 readiness 및 shutdown ordering을 구현, 문서, regression test가 함께 바뀌어야 하는 behavioral contract로 다루며, [`docs/contracts/testing-guide.md`](./contracts/testing-guide.md)는 queue lifecycle behavior 변경 시 실행할 가까운 package test와 governance command를 안내한다.

Notifications discoverability도 패키지 README, governed package-surface docs, NestJS migration docs, intermediate book으로 나뉜다. `packages/notifications/README.ko.md`는 `NotificationsModule.forRoot(...)` / `forRootAsync({ inject, useFactory, global? })`, `channels`로 전달되는 명시적인 `NotificationChannel` 값, `global: false` local opt-out을 포함한 기본 global provider visibility, 직접 `dispatch(...)`, batch `dispatchMany(...)`, `queue: true`가 `bulkThreshold`와 무관하게 queue delivery를 명시적으로 강제하는 optional queue-backed delivery, lifecycle event publication, operation mode, dependencies, queue threshold, queue configuration, event-publisher configuration을 `details` 아래에 담는 `NotificationsService.createPlatformStatusSnapshot()` 및 `createNotificationsPlatformStatusSnapshot(...)` 기반 platform status snapshot, recursive opaque-payload 계약이 아니라 plain envelope용 best-effort fallback인 deterministic generated queue/fallback delivery ID, concrete queue/event-bus ownership에 대한 intentional limitation을 문서화한다. 이 status snapshot은 publisher wiring을 `details.eventPublisherConfigured`로, 실제 publication 여부를 `details.eventPublicationEnabled`로 분리해 보고하며, enablement만 event-backed operation mode, active dependency, external ownership을 결정하므로 configured-but-disabled publisher는 inactive 상태로 남는다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 concrete queue와 event-bus 구현이 foundation 패키지 밖에 남고 cross-release delivery identity가 중요할 때 caller-provided `notification.id`가 durable identity contract라는 점을 포함해 정식 `@fluojs/notifications` responsibility boundary를 기록한다. [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)는 NestJS provider discovery/decorator metadata에서 explicit channel registration, 애플리케이션 소유 queue/event publisher resource, 기본 global service/token visibility로 옮기는 migration boundary를 기록하고, [`book/intermediate/ch15-notifications.ko.md`](../book/intermediate/ch15-notifications.ko.md)는 FluoShop 학습 경로에서 동일한 registration, batch/status, queue, lifecycle, diagnostics surface를 설명한다.

Cron scheduling discoverability도 패키지 문서, governed package-surface docs, package chooser, lifecycle docs, NestJS migration docs, intermediate book으로 나뉜다. `packages/cron/README.ko.md`는 `CronModule.forRoot(...)`, five-field 및 six-field cron expression 지원, public instance method decorator target, immutable descriptor snapshot, dynamic blank-name rejection, rollback-safe cron expression 및 interval cadence update를 포함한 descriptor 기반 `SCHEDULING_REGISTRY` control, decorator와 dynamic registry startup behavior, Redis distributed locking, bounded shutdown, public scheduling/status type, status snapshot을 문서화하며, Redis peer loading 및 resolution은 distributed-only이고 `distributed.enabled`가 `true`일 때만 발생한다는 경계도 명시한다. [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)는 scheduling, rollback-safe runtime cadence change, distributed lock, shutdown, health/readiness visibility, Redis I/O 전 enabled distributed TTL validation 및 동일한 distributed-only Redis peer boundary에 대한 `@fluojs/cron` responsibility boundary를 기록한다. [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md)는 Cron distributed lock을 활성화할 때만 `@fluojs/redis`를 추가하라고 안내한다. [`docs/architecture/lifecycle-and-shutdown.ko.md`](./architecture/lifecycle-and-shutdown.ko.md)는 Cron scheduler ownership, active-task drain, distributed lock release/shutdown semantic을 기록하며, shutdown 중 Redis owned-lock release I/O가 `shutdown.timeoutMs`로 제한되고 release I/O timeout 시 local ownership이 보존된다는 경계도 포함한다. [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)는 `@nestjs/schedule` migration boundary를 `SchedulerRegistry`/`CronJob` handle에서 public-method decorator와 read-only descriptor로 옮기는 방향으로 기록한다. [`book/intermediate/ch12-cron.ko.md`](../book/intermediate/ch12-cron.ko.md)는 동일한 bootstrap, dynamic-start lifecycle guarantee, public-method scheduling boundary, descriptor 기반 registry control, rollback-safe interval cadence update, enabled TTL validation, bounded lock release, Redis distributed-lock 선택 경계를 FluoShop 학습 경로에서 설명한다.

Persistence transaction discoverability는 패키지 README, governed transaction architecture docs, package-surface docs, package chooser docs, NestJS migration docs, intermediate book으로 나뉜다. `packages/prisma/README.ko.md`, `packages/drizzle/README.ko.md`, `packages/mongoose/README.ko.md`는 패키지별 표준 `@Transaction()` 데코레이터를 기본 서비스 계층 트랜잭션 경계로 문서화하며, `transaction(...)`, `requestTransaction(...)`, `current()`, `currentSession()`은 명시적 low-level escape hatch로 유지된다. `packages/drizzle/README.ko.md`는 루트 `@fluojs/drizzle`의 Node.js `>=24.0.0 <27` runtime boundary(`node:async_hooks`, `engines.node >=24.0.0 <27`), Bun, Deno, Cloudflare Workers 및 다른 비 Node driver를 위한 raw Drizzle provider guidance, `database.transaction(...)`을 사용할 수 없고 `strictTransactions`가 `false`일 때의 fail-open direct execution, rollback 보장이 필요한 production 경로의 `strictTransactions: true` guidance, host-object heuristic 대상 선택(`this.db`, 직접 property, 중첩 `.db`), 여러 Drizzle client에 대한 `@Transaction((self) => self.ordersDb)` 같은 명시적 accessor 선택, 명시적 controller/request-wide `requestTransaction(...)` boundary와 애플리케이션 등록이 `DrizzleModule.forRoot(...)` / `forRootAsync(...)`에 남는다는 점을 기록한다. [`docs/architecture/transactions.ko.md`](./architecture/transactions.ko.md)는 cross-package boundary semantic, deprecated `PrismaTransactionInterceptor` 및 `MongooseTransactionInterceptor` 호환성 export 복원, 드문 요청 전체 경계를 위한 명시적 Drizzle `requestTransaction(...)` 경로, Drizzle fail-open vs strict transaction behavior, Drizzle decorator 대상 선택, Mongoose가 지원하는 auto-session facade method set을 기록한다. [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md)와 [`book/intermediate/ch20-drizzle.ko.md`](../book/intermediate/ch20-drizzle.ko.md)는 이러한 Drizzle transaction 선택에 대한 migration 및 learning-path guidance를 담고, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md)와 [`docs/reference/package-chooser.ko.md`](./reference/package-chooser.ko.md)는 Drizzle Node-only selection, 비 Node raw-provider fallback, public API naming summary의 정식 내용을 담는다.

Drizzle transaction drain discoverability는 특히 `packages/drizzle/README.ko.md`, [`docs/architecture/transactions.ko.md`](./architecture/transactions.ko.md), [`book/intermediate/ch20-drizzle.ko.md`](../book/intermediate/ch20-drizzle.ko.md)에 실린다. fail-open callback은 root handle을 ALS에 유지하므로 중첩 request 작업이 원자성을 얻지 않으면서 ambient abort와 shutdown-drain 소유권을 상속하고, 모든 직접 fallback 실행은 `dispose(database)` 전에 settle되어야 한다. Controller-level `@Transaction()`은 호환성 경로로만 문서화되며 명시적 `requestTransaction(...)`이 더 명확한 request-wide boundary로 남는다.
...
Application shutdown terminality는 `packages/runtime/README.ko.md`와 [`docs/architecture/lifecycle-and-shutdown.ko.md`](./architecture/lifecycle-and-shutdown.ko.md)에 동기화되어 있다. 공개 `Application.state` 계약은 `bootstrapped`, `ready`, `closed`를 유지한다. `Application.close()`가 시작되면 성공한 teardown 전까지 기존 공개 lifecycle state를 유지하면서 `Application.get()`, `ApplicationContext.get()`, `Application.listen()`, `connectMicroservice()`, `startAllMicroservices()`를 위한 별도의 terminal operation gate를 동기적으로 닫는다. 실패한 close 시도는 이 gate를 다시 열지 않으며, 이후 close 호출은 완료된 runtime teardown phase를 건너뛰면서 adapter와 lifecycle-stage retry ownership을 보존한다. Container-managed `onDestroy()` hook은 terminal best-effort cleanup이다. 첫 disposal에서 materialize된 hook을 모두 시도하고, 실패한 hook만 이후 명시적 application 또는 context close에서 재시도하며, 성공한 hook은 다시 실행하지 않는다.

HTTP adapter raw-body portability discoverability도 testing package와 governed platform docs로 나뉜다. `packages/testing/README.md`는 byte-sensitive payload를 위한 `HttpAdapterPortabilityHarness.create(...)`와 `assertPreservesExactRawBodyBytesForByteSensitivePayloads()`를 문서화한다. [`docs/contracts/platform-conformance-authoring-checklist.ko.md`](./contracts/platform-conformance-authoring-checklist.ko.md)는 HTTP adapter가 Unicode replacement, newline normalization, re-encoding 없이 정확한 `rawBody` byte를 보존해야 한다고 요구하며, [`docs/contracts/testing-guide.ko.md`](./contracts/testing-guide.ko.md)는 HTTP adapter byte preservation behavior가 바뀔 때 실행할 platform portability test와 governance command를 안내한다.

Bun adapter lifecycle 및 migration discoverability는 `packages/platform-bun/README.ko.md`, [`book/intermediate/ch22-bun.ko.md`](../book/intermediate/ch22-bun.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), website runtime/realtime guide로 나뉜다. 패키지 README는 duplicate `listen()` idempotency, pre-listen realtime binding requirement, byte-exact raw-body 및 stale native handoff regression coverage, log/`process.exitCode`를 통한 signal-driven shutdown reporting, 동기 `createBunFetchHandler(...)` 사용법을 문서화한다. Intermediate book은 manual fetch-host lifecycle boundary와 `createBunShutdownSignalRegistration(...)` signal helper boundary를 담고, NestJS migration docs는 Bun fetch-style hosting이 late host mutation 또는 reflection metadata 가정을 되살리지 않는다고 기록하며, website guide는 `createBunFetchHandler(...)`가 동기임을 유지하면서 adapter-owned shutdown, raw fetch, `server.upgrade(...)` 제어가 realtime binding seam 밖에 남는다는 점을 맞춘다.

Deno adapter lifecycle 및 NestJS migration discoverability는 `packages/platform-deno/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), [`book/intermediate/ch23-deno.ko.md`](../book/intermediate/ch23-deno.ko.md), website runtime adapter guide로 나뉜다. Managed `listen()`은 `Deno.serve(...)`, bounded adapter close/drain, configured websocket upgrade를 소유한다. Factory는 명시적인 `shutdownRegistration`으로 받은 `createDenoShutdownSignalRegistration(...)` Deno signal callback을 설치하고 해제한다. 동기 `createDenoFetchHandler(...)`는 이미 bootstrap된 `app.dispatcher`를 받아 shared Web request parsing과 SSE framing을 보존하고 server를 시작하지 않으며 shutdown, signal, websocket upgrade를 주변 host에 남긴다. Managed startup은 `deno run --allow-net main.ts`를 사용하며 signal listener 등록에는 별도의 Deno permission이 필요하지 않다. Adapter 자체에는 environment 접근 권한이 필요하지 않으므로 `--allow-env=<keys>`는 애플리케이션 코드가 읽는 key로만 제한한다. Process signal을 소유하는 host는 host-owned lifecycle coordination을 선택하기 위해 `shutdownRegistration`을 생략하고 애플리케이션 shutdown을 조율해야 한다.

Cloudflare Workers adapter lifecycle 및 migration discoverability는 `packages/platform-cloudflare-workers/README.ko.md`, [`docs/reference/package-surface.ko.md`](./reference/package-surface.ko.md), [`book/intermediate/ch24-cloudflare.ko.md`](../book/intermediate/ch24-cloudflare.ko.md), [`docs/getting-started/migrate-from-nestjs.ko.md`](./getting-started/migrate-from-nestjs.ko.md), website runtime/realtime guide로 나뉜다. 이 surface들은 직접 Factory adapter 경로, socket을 열지 않는 `listen()` dispatcher binding, Worker `fetch(request, env, ctx)` 및 `executionContext.waitUntil(...)` request/SSE/WebSocket tracking, frozen first-listen binding 전의 완전한 `CloudflareWorkersWebSocketModule` 등록, bounded `close()` drain behavior, drain 중 또는 close 이후 새 HTTP 및 WebSocket ingress에 대한 JSON `503` shutdown response, timed-out close가 나중에 settle된 뒤 lazy host가 recover되는 동작을 함께 맞춘다. Fetch-time `env` boundary는 package README, intermediate book, NestJS migration map, website runtime guide에만 명시적으로 반영됩니다. 별도로 사용할 수 있는 pre-registration 값은 `{ fromEnv }`로 bootstrap을 구성할 수 있지만, request-bound binding은 application-shaped 값이 provider method에 전달되기 전에 검증하고 좁혀야 합니다.
...
evidence가 React/renderer stability, manifest 및 Server Function transport compatibility,
browser/server separation, SSR/CSR/prerendering, hydration mismatch recovery, safe transfer rule,
HTTP route 및 #2506 navigation ownership, dual-import test, bilingual docs, Changesets intent를 다룰
때까지 `@fluojs/react/rsc`를 unavailable 상태로 유지합니다. Graduation은 stable root RSC export를
...
| --- | --- | --- |
| Passport refresh-token module 소유권과 alias visibility | `packages/passport/README.ko.md` | `apps/docs/content/docs/guides/auth.mdx` 및 `packages/passport/src/refresh/refresh-token.ts` |
...
| HTTP conditional cache validator와 byte-range 동작 확인 | `docs/architecture/http-runtime.ko.md` | `docs/contracts/testing-guide.ko.md`, `packages/http/README.ko.md`, `packages/testing/README.ko.md` |
