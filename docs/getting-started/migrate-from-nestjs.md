...
NestJS `app.enableShutdownHooks()` does not map to implicit signal wiring on every fluo bootstrap path. Use `runNodeApplication(...)` from `@fluojs/runtime/node` when a direct Node application needs the default `SIGINT` / `SIGTERM` registration. If the application uses `FluoFactory.create(...)`, `bootstrapNodeApplication(...)`, or adapter-first Node bootstrap instead, register signals explicitly at that Node boundary with `createNodeShutdownSignalRegistration(...)` or `registerShutdownSignals(...)`. Bun, Deno, Cloudflare Workers, and other fetch-style hosts own their shutdown events and call `app.close(signal?)`; they must not inherit Node process-signal wiring.
...
`GraphqlModule.forRootAsync({ inject, useFactory })` supports explicit application tokens and a factory. NestJS `imports`, `useClass`, `useExisting`, and implicit provider discovery remain unsupported. `@Subscription({ topics })` remains unsupported, and subscription methods must return an `AsyncIterable`. `@fluojs/graphql` requires Node.js `>=20.19.3 <21 || >=22.2.0 <27`, the effective range of its mandatory first-party dependency graph through the Node listener-capable `@fluojs/runtime`; `@fluojs/config` is not part of that mandatory dependency graph and independently retains Node.js `>=20.16.0`. HTTP and SSE use a Web-standard HTTP seam within that boundary, while optional WebSocket subscriptions additionally require a server-backed Node HTTP/S adapter with upgrade listeners. Do not treat that internal seam as Bun, Deno, or Cloudflare Workers package support without aligned dependency metadata and native runtime verification.
