...
| NestJS JWT async registration with dynamic-module `imports`, `useClass`, `useExisting`, or provider discovery | `JwtModule.forRootAsync({ inject, useFactory })` from `@fluojs/jwt` | `JwtModule.forRootAsync(...)` requires dependencies named by `inject` to be registered in the application module graph before its JWT options provider resolves, and `useFactory` returns the final JWT options. It does not accept `imports`, `useClass`, or `useExisting`; those fields are outside the options shape rather than accepted-and-ignored compatibility fields. `JwtModule.forRootAsync(...)` performs no implicit module or provider discovery. |
...
- NestJS JWT async registration is not a dynamic-module shape clone. `JwtModule.forRootAsync({ inject, useFactory })` requires every injected dependency to be registered in the application module graph first, and `useFactory` must return the final JWT options. `JwtModule.forRootAsync(...)` does not accept NestJS `imports`, `useClass`, or `useExisting`; they are outside the supported options shape and are not silently ignored. `JwtModule.forRootAsync(...)` performs no implicit module or provider discovery.
