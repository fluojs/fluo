...
- `@fluojs/mongoose` requires the application to provide a concrete Mongoose connection; it does not create the connection, own model compilation, or close the connection unless a `dispose(connection)` hook is supplied.
