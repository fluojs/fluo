...
| NestJS `PartialGraphHost` startup-failure graph access | no direct replacement; use CLI stderr and the underlying bootstrap error diagnostics | <!-- fluo-studio-report-bootstrap-failure-contract --> Studio reports are post-bootstrap artifacts: `fluo inspect` must complete application creation before it can emit a snapshot, timing data, or a report. A failed or hanging bootstrap emits no partial graph or report; design partial-graph support separately if required. |
