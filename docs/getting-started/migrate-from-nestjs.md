...
- NestJS testing migration is not an implicit imports-array replacement. Use `createTestingModule({ rootModule })`, call `overrideModule(OriginalModule).useModule(ReplacementModule)` before `compile()`, and create HTTP tests with `createTestApp(moduleRef, options)` when adapter, provider, filter, or lifecycle options must be forwarded into runtime bootstrap.
- `TestingModuleRef` owns the compiled application context. Close it after each test; any app returned by `createTestApp(...)` shares that lifecycle boundary, so do not treat it as a separately owned NestJS-style application instance.
