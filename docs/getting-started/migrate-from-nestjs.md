...
| NestJS Redis async module registration or shared Redis Pub/Sub clients | `RedisModule.forRoot(...)`, named `RedisModule.forRoot({ name, ... })`, and `getRedisClientToken(name)` from `@fluojs/redis` | fluo Redis registration is synchronous. Resolve environment-specific options before registration; each `forRoot(...)` call creates a new lifecycle-managed client rather than adopting an externally created client. Keep raw external clients application-owned, and keep Pub/Sub subscribers on a dedicated duplicate or named client instead of reusing the ordinary command client. |
...
- Redis migration is not an async dynamic-module replacement. `@fluojs/redis` exposes synchronous `RedisModule.forRoot(...)`; resolve secrets, hosts, TLS options, and other final options at the application boundary before registration. `forRoot(...)` creates a new lifecycle-managed client and does not adopt an externally constructed client, whose shutdown remains application-owned.
