...
| Nest Devtools `SerializedGraph` | no static one-to-one replacement; use `fluo dev --studio` on the supported Node live path when compiled DI graph analysis is required | Static `fluo inspect` artifacts are not compatible with Nest Devtools `SerializedGraph`: they omit the compiled module/provider graph, provider scopes, and lifecycle diagnostics. Keep workflows that require that graph on Node live Studio rather than treating the file-first migration fallback as graph parity. |
