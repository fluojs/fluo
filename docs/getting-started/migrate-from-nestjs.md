...
| NestJS request transaction interceptor | Service `@Transaction()` from the persistence package, or explicit `requestTransaction(...)` at the controller/request boundary | Package-provided request transaction interceptors are removed and have no compatibility exports. Drizzle has no compatibility interceptor export. Keep business transactions on services. When an entire request must share one boundary, use an application-owned interceptor or controller boundary that calls `PrismaService.requestTransaction(...)`, `DrizzleDatabase.requestTransaction(...)`, or `MongooseConnection.requestTransaction(...)` and forwards `RequestContext.request.signal`. <!-- fluo-prisma-transaction-boundary: interceptor=removed;replacement=application-owned-request-transaction --> |
