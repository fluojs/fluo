...
NestJS `app.enableShutdownHooks()` does not map to implicit signal wiring on every fluo bootstrap path. Use `runNodeApplication(...)` from `@fluojs/platform-nodejs` when a direct Node application needs the default `SIGINT` / `SIGTERM` registration. If the application uses `FluoFactory.create(...)`, `bootstrapNodeApplication(...)`, or adapter-first Node bootstrap instead, register signals explicitly at that Node boundary with `createNodeShutdownSignalRegistration(...)` or `registerShutdownSignals(...)`. Bun, Deno, Cloudflare Workers, and other fetch-style hosts own their shutdown events and call `app.close(signal?)`; they must not inherit Node process-signal wiring.
...
| `NestFactory.create(AppModule)` | `FluoFactory.create(AppModule, { adapter })` from `@fluojs/runtime` | HTTP listening requires an explicit platform adapter such as `createFastifyAdapter()`. `FluoFactory.create(AppModule)` can still build an adapterless application shell, but that shell cannot call `listen()`. The CLI codemod preserves this bootstrap and reports a required adapter-selection warning unless you explicitly choose a supported platform; use `fluo migrate ./src --apply --platform express` to generate `createExpressAdapter({ port })` for Express. |
| NestJS `app.enableShutdownHooks()` | `runNodeApplication(...)` from `@fluojs/platform-nodejs`, or explicit `createNodeShutdownSignalRegistration(...)` / `registerShutdownSignals(...)` at a lower-level Node boundary | `runNodeApplication(...)` owns the default `SIGINT` / `SIGTERM` registration. `FluoFactory.create(...)`, `bootstrapNodeApplication(...)`, and adapter-first Node bootstrap do not install it implicitly. Fetch-style hosts own shutdown signaling and call `app.close(signal?)` themselves. |
