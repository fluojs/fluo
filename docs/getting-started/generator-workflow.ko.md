...
| --- | --- | --- | --- | --- |
| Module | `module`, `mo` | `fluo generate module Billing --with-slice-test` | 파일만 생성 | 독립 모듈 파일, 선택적 slice test |
...
| --- | --- | --- |
| Module | `post.module.ts`; `--with-slice-test`를 추가하면 `post.slice.test.ts` | 없음. 상위 모듈 import는 별도로 처리합니다. |
...
- Provider를 직접 배선하기 전에 최소 module compilation test가 필요하면 `fluo g module <name> --with-slice-test`를 사용합니다.
...
- `--with-slice-test`는 `module`과 `resource`에서 지원됩니다. `fluo g service User --with-slice-test` 같은 지원되지 않는 조합은 non-zero exit로 실패합니다.
...
- generate 명령이 문서화하는 옵션은 `--target-directory`, `--force`, `--dry-run`, `--with-slice-test`, `--help`입니다. `--with-slice-test`는 module과 resource 생성기에서만 사용할 수 있습니다.
