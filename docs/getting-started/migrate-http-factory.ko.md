...
| Worker `bootstrapCloudflareWorkerApplication(...)`, `createCloudflareWorkerEntrypoint(...)`, `createCloudflareWorkerEnvEntrypoint(...)` | fixed module에는 `CloudflareWorkerApplicationHost.create(AppModule, options)`를 사용하고, 첫 environment가 configuration을 선택할 때는 같은 메서드에 `{ fromEnv }`를 전달하세요. |
