...
| NestJS 요청 transaction interceptor | 영속성 패키지의 서비스 `@Transaction()` 또는 controller/request 경계의 명시적 `requestTransaction(...)` | Package가 제공하는 request transaction interceptor는 제거되었으며 compatibility export가 없다. 비즈니스 transaction은 서비스에 둡니다. 전체 요청이 하나의 경계를 공유해야 하면 애플리케이션 소유 interceptor 또는 controller boundary에서 `PrismaService.requestTransaction(...)`, `DrizzleDatabase.requestTransaction(...)`, `MongooseConnection.requestTransaction(...)` 중 하나를 호출하고 `RequestContext.request.signal`을 전달하세요. <!-- fluo-prisma-transaction-boundary: interceptor=removed;replacement=application-owned-request-transaction --> |
