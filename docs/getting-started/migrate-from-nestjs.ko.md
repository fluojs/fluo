...
- NestJS testing migration은 암묵적 imports-array 치환이 아니다. `createTestingModule({ rootModule })`을 사용하고, `compile()` 전에 `overrideModule(OriginalModule).useModule(ReplacementModule)`을 호출하며, adapter, provider, filter, lifecycle option을 runtime bootstrap으로 전달해야 하는 HTTP 테스트에서는 `createTestApp(moduleRef, options)`를 사용한다.
- `TestingModuleRef`가 컴파일된 application context를 소유한다. 각 테스트 뒤에 이를 닫아야 하며, `createTestApp(...)`이 반환한 app은 같은 lifecycle boundary를 공유하므로 별도로 소유되는 NestJS-style application instance로 취급하지 않는다.
