...
| NestJS Redis async module registration 또는 shared Redis Pub/Sub client | `@fluojs/redis`의 `RedisModule.forRoot(...)`, named `RedisModule.forRoot({ name, ... })`, `getRedisClientToken(name)` | fluo Redis registration은 동기 방식이다. 환경별 option은 registration 전에 해석하며, `forRoot(...)`를 호출할 때마다 외부에서 만든 client를 채택하는 대신 새 lifecycle-managed client를 생성한다. Raw 외부 client는 애플리케이션 소유로 두고, Pub/Sub subscriber는 일반 command client를 재사용하지 말고 전용 duplicate 또는 named client로 분리한다. |
...
- Redis migration은 async dynamic-module 치환이 아니다. `@fluojs/redis`는 동기 `RedisModule.forRoot(...)`를 제공한다. Secret, host, TLS option, 기타 최종 option은 application boundary에서 먼저 해석한 뒤 등록한다. `forRoot(...)`는 새 lifecycle-managed client를 생성하고 외부에서 만든 client를 채택하지 않으므로, 해당 client의 shutdown은 애플리케이션 소유로 남는다.
