...
## Fastify raw request 및 response 객체

여러 HTTP adapter에서 실행해야 하는 controller 코드는 `RequestContext`의 이식 가능한 `FrameworkRequest` 및 `FrameworkResponse` 작업을 사용하세요. 예를 들어 `context.request`에서 `method`, `path`, `headers`, `query`, `body`를 읽고 `context.response`에서 `setStatus(...)`, `setHeader(...)`, `send(...)`, `redirect(...)`를 사용합니다.

`@fluojs/platform-fastify`는 공유 `raw` field를 통해 의도적으로 비대칭 native 객체를 노출합니다.

- `context.request.raw`는 Fastify `FastifyRequest`가 아니라 기반 Node.js `IncomingMessage`입니다.
- `context.response.raw`는 Fastify `FastifyReply`입니다.

이 field들이 일치하는 native type이라고 가정하거나, 마이그레이션한 `@Req()` 또는 `@Res()` 코드를 컴파일하기 위해 안전하지 않은 `unknown` cast를 추가하지 마세요. 현재 Fastify adapter는 typed Fastify-native request accessor를 공개하지 않습니다. 이식 가능한 `FrameworkRequest` 또는 `FrameworkResponse` 작업으로 충족할 수 없는 native Fastify 요구 사항이 있으면 shared controller 코드를 cast에 결합하지 말고 계약을 보존하는 typed accessor를 요청하세요.
...
Studio report는 bootstrap 이후 artifact다. `fluo inspect`는 snapshot, timing data, report를 내보내기 전에 애플리케이션 생성을 완료해야 한다. Bootstrap이 실패하거나 멈추면 partial graph나 report가 생성되지 않으며, 필요하다면 partial-graph 지원은 별도로 설계한다. |
...
| Cloudflare Workers로 이동할 때의 NestJS HTTP server lifecycle hook 또는 late WebSocket server mutation | `@fluojs/platform-cloudflare-workers`와 `@fluojs/websockets/cloudflare-workers`의 `CloudflareWorkersWebSocketModule.forRoot()` | Workers는 server socket 대신 host-owned `fetch(request, env, ctx)` boundary를 노출합니다. Exported `fetch` handler에는 host가 호출하는 shutdown callback이 없으므로 애플리케이션이 close trigger를 소유합니다. `worker.fetch` 밖의 application-owned trigger는 `await worker.close()`를 직접 호출할 수 있습니다. 같은 `worker.fetch` 안의 management route는 현재 response를 반환한 뒤 `ctx.waitUntil(worker.close())` 또는 동등한 non-self-awaiting mechanism으로 close를 관찰해야 합니다. 그 안에서 await하면 자기 자신의 active request drain을 기다리다 shutdown timeout에 도달합니다. `listen()`은 fluo dispatcher만 binding합니다. Application graph에 Worker WebSocket module을 등록하여 bootstrap이 해당 listen boundary 전에 binding을 구성하도록 하세요. 수락된 각 request는 `ctx.waitUntil(...)`로 추적됩니다. 성공한 host `close()`는 재시작 가능합니다. 다음 `fetch(...)`는 새 application을 bootstrap하여 bootstrap hook을 다시 실행하고 singleton provider를 다시 생성합니다. Bootstrap에는 미리 선언한 root module과 option만 전달되고 request `env`는 dispatch 중에 연결되므로, `env`는 `ConfigModule.forRoot(...)` 또는 singleton bootstrap provider를 구성할 수 없습니다. 별도로 사용할 수 있는 pre-registration 값만 bootstrap configuration에 두세요. `RequestContext`에서 선택한 fetch-time binding을 읽고 검증하고 좁힌 뒤 application-shaped 값으로 provider method에 전달하세요. |
...
| `@nestjs/graphql` resolver discovery, reflected return type, parameter decorator, `forRootAsync(...)` | `@fluojs/graphql`의 `GraphqlModule.forRoot(...)`, `GraphqlModule.forRootAsync({ inject, useFactory })`, module provider, `@Resolver`, root operation decorator, `@FieldResolver`, `@Args`, `@Parent`, `@Context`, `listOf(...)` | Resolver class를 compiled module의 provider로 등록한다. `resolvers` option은 discovery 가능한 class에 적용하는 선택적 allowlist/filter다. 이를 생략하거나 빈 list를 전달하면 등록된 decorated candidate를 모두 허용한다. fluo는 metadata에서 provider나 GraphQL output type을 추론하지 않는다. Object 결과에는 `outputType`, array에는 `outputType: listOf(ItemType)`이 필요하며 생략한 output type은 GraphQL `String`을 사용한다. Object field는 `@Resolver('TypeName')`으로 named code-first output type에 연결한다. TC39 표준 데코레이터는 parameter decorator를 지원하지 않으므로 field resolver method에 서로 다른 index의 `@Args(index?)`, `@Parent(index?)`, `@Context(index?)`를 배치한다. Code-first field argument DTO binding은 `@FieldResolver({ input: InputDto })`, 선택적 `argTypes`, `@Args(index?)`로 지원하며 `input`과 `@Args()`는 서로 필요하고 root operation에서는 유효하지 않다. Async registration은 명시적인 `inject` token과 `useFactory`만 지원합니다. NestJS 스타일 `imports`, `useClass`, `useExisting`, 암시적 provider discovery는 계속 지원하지 않으며 schema-first field-resolver attachment와 `@Subscription({ topics })`도 지원하지 않습니다. 선택적 WebSocket subscription에는 server-backed Node HTTP/S adapter가 필요하다. |
...
| `Test.createTestingModule({ imports: [...] }).overrideModule(...)` | `@fluojs/testing`의 `Test.createTestingModule({ rootModule }).overrideModule(...)` | fluo testing은 명시적 `rootModule`과 replacement compile seam을 사용하므로 전역 module metadata를 mutate하지 않고 authored module identity를 보존한다. |
<!-- fluo-prisma-transaction-boundary: interceptor=removed;replacement=application-owned-request-transaction -->
| NestJS 요청 transaction interceptor | 영속성 패키지의 서비스 `@Transaction()` 또는 controller/request 경계의 명시적 `requestTransaction(...)` | Package가 제공하는 request transaction interceptor는 제거되었으며 compatibility export가 없다. Drizzle은 호환성 interceptor export를 제공하지 않는다. 비즈니스 transaction은 서비스에 둡니다. 전체 요청이 하나의 경계를 공유해야 하면 애플리케이션 소유 interceptor 또는 controller boundary에서 `PrismaService.requestTransaction(...)`, `DrizzleDatabase.requestTransaction(...)`, `MongooseConnection.requestTransaction(...)` 중 하나를 호출하고 `RequestContext.request.signal`을 전달하세요. |
| `HealthCheckService.check([...])`를 호출하는 `@HealthCheck()` 컨트롤러 메서드 | `@fluojs/terminus`의 `TerminusModule.forRoot({ indicators, indicatorProviders, readinessChecks })` | Module-level registration이 기본 API이므로 runtime `/health`와 `/ready` route가 indicator 및 platform diagnostics를 일관되게 포함한다. Custom `HealthIndicator` instance는 `indicators`에, DI-backed built-in은 `create*HealthIndicatorProvider()` factory를 통해 `indicatorProviders`에 둔다. Runtime-owned route는 NestJS controller의 `@HealthCheck()` 또는 `@UseGuards()` metadata를 채택하지 않으므로 application/adapter middleware, network policy, deployment boundary에서 보호하세요. |
| NestJS Terminus memory/disk, Redis, Prisma, Drizzle, HTTP 또는 custom check | `@fluojs/terminus/node`, `@fluojs/terminus/redis`, root built-in, `HealthIndicator` | Node memory/disk helper는 `@fluojs/terminus/node`에서, Redis helper는 `@fluojs/terminus/redis`에서 import하세요. Prisma, Drizzle, HTTP indicator는 root export입니다. Redis subpath는 optional-peer 경계를 보존하고 Node helper가 root에서도 export되는 것은 호환성을 위한 것입니다. Custom check는 `HealthIndicator`를 구현해 `indicators`에 등록하거나 dependency가 DI에서 올 때 해당 provider factory를 사용해 포팅하세요. |
| NestJS Prometheus module registration 또는 shared `prom-client` Registry | `@fluojs/metrics`의 `MetricsModule.forRoot(...)`, `MetricsService`, `METRICS_REGISTRY`; `@fluojs/metrics/integration`의 `Registry` | 이는 NestJS Dynamic Module 호환 계층이 아니라 fluo 고유의 Prometheus integration입니다. 최종 option은 module composition 전에 동기적으로 구성하세요. 기본 scrape route는 `GET /metrics`이고 HTTP collector는 `http`로 opt-in합니다. `FluoFactory.create(..., { providers: [{ provide: METRICS_REGISTRY, useValue: registry }] })`로 공유 Registry를 명시하지 않으면 application bootstrap마다 격리된 Registry를 만듭니다. |
...
| `@nestjs/event-emitter` / `@OnEvent()` handler | `@fluojs/event-bus`의 `EventBusModule.forRoot(...)`, `EventBusService.publish(event, options?): Promise<EventPublishResult>`, `@OnEvent(EventClass)` | Event routing은 class 기반이고, `static eventKey`는 distributed transport channel을 안정적으로 유지하며, handler는 singleton provider/controller에서만 discovery되고 awaited/background publish 작업은 shutdown drain tracking에 남는다. 이것이 유일한 공개 발행 경로다. Throw하거나 reject한 listener 실패는 local 및 inbound transport dispatch에서 log되고 격리되므로 다른 matching listener는 계속 실행되며, 그 listener 실패만으로 `publish(...)`가 reject되거나 inbound callback completion을 통해 실패가 외부로 드러나지는 않는다. 좁은 `@fluojs/event-bus/integration` 서브패스는 first-party integration용 shutdown coordinator seam일 뿐이다. |
...
2. email module이 소유할 1st-party Node SMTP transporter에는 `@fluojs/email/node`의 `NodemailerEmailTransport.createFactory(...)`를 사용합니다. 이 factory는 ownership을 알리므로 bootstrap verification과 shutdown close가 자신이 만든 transporter에 적용됩니다.
3. 다른 애플리케이션 component가 만들고 닫는 기존 Nodemailer transporter에는 `NodemailerEmailTransport.create(...)` (예: `NodemailerEmailTransport.create({ transporter })`)로 감쌉니다. 이 wrapper는 resource ownership을 넘기지 않고 shared transport contract를 만족하므로 `EmailService`를 통한 두 번째 close 경로를 만들지 마세요.
...
- `@fluojs/drizzle`은 서비스 `@Transaction()`을 기본 경계로 사용하고, controller/request-wide boundary에는 명시적 `DrizzleDatabase.requestTransaction(...)`을 사용한다.
...
- Mongoose transaction migration은 interceptor-for-interceptor 치환이 아니다. 패키지 interceptor import를 제거하고 같은 chain 위치에 애플리케이션 소유 interceptor를 두며 `RequestContext.request.signal`을 `MongooseConnection.requestTransaction(...)`에 전달한다. 비즈니스 원자성에는 서비스 `@Transaction()`을 사용한다.
...
- NestJS testing migration은 암묵적 imports-array 치환이 아니다. `Test.createTestingModule({ rootModule })`을 사용하고, `compile()` 전에 `overrideModule(OriginalModule, ReplacementModule)`을 호출하며, adapter, provider, filter, lifecycle option을 runtime bootstrap으로 전달해야 하는 virtual request HTTP 테스트에서는 `Test.createApp({ rootModule, ...options })`를 사용한다.
- `TestingModuleRef`는 assertion, provider resolution, dispatch helper를 위한 컴파일된 module context를 노출하고, `Test.createApp(...)`은 자체 `close()` lifecycle을 가진 request-driven app facade를 반환한다. NestJS-style 공유 application instance 소유권에 의존하지 말고 각 HTTP 테스트 뒤에 반환된 test app을 닫아야 한다.
...
- NestJS Prometheus migration은 async Dynamic Module 또는 implicit global Registry replacement가 아니다. `MetricsModule.forRoot(...)`는 최종 동기 option만 받으므로 environment-specific configuration은 application module을 구성하기 전에 해석하세요. `FluoFactory.create(..., { providers })`에 `METRICS_REGISTRY`를 제공하지 않으면 application bootstrap마다 새 `Registry`를 만듭니다. `Registry`는 `@fluojs/metrics/integration`에서 import하며 NestJS 또는 `prom-client` global Registry를 채택한다고 가정하면 안 됩니다. 기존 module `registry` option은 제거되었습니다. 기본으로 `GET /metrics`가 활성화되고 `path: false`는 이 scrape route를 비활성화하며, 내장 HTTP request collector는 `http: true` 또는 `http` option object를 전달할 때만 설치됩니다.
...
- WebSocket migration은 decorator-for-decorator 치환이 아닙니다. Runtime-neutral `@fluojs/websockets` root에서 `@OnMessage(event?)`와 `WebSocketRoomService`를 import하고, handler 입력은 `(payload, socket, request, socketId)` positional argument로 읽으며 NestJS gateway-server injection이나 parameter decorator가 그대로 이어진다고 가정하지 마세요. Runtime lifecycle service가 room contract를 구현합니다. `@fluojs/websockets/node`의 `NodeWebSocketGatewayLifecycleService` 또는 다른 runtime subpath의 해당 runtime lifecycle token을 `@Inject(...)`로 주입하고 constructor parameter를 `WebSocketRoomService`로 type 지정하세요. Node registration은 Node subpath의 `NodeWebSocketModule.forRoot(...)`이며 guard는 `IncomingMessage`를 받습니다. Bun, Deno, Cloudflare Workers registration은 native guard/request type과 lifecycle service가 올바른 owner boundary에 머물도록 각 subpath에 둡니다. Room-broadcast backpressure는 Node에만 적용됩니다. Raw gateway 반환값은 기본적으로 await된 뒤 무시됩니다. Reply는 runtime socket으로 보내거나 선택한 runtime module의 `forRoot({ replies: { mode: 'event-envelope' } })`를 구성하세요.
- Socket.IO migration도 같은 명시적 websocket handler 모델을 유지합니다. `@fluojs/socket.io`는 `@fluojs/websockets`의 `@WebSocketGateway`, `@OnMessage`, lifecycle decorator를 재사용하므로 companion 패키지를 설치하세요. Handler return value는 await된 뒤 무시됩니다. Client가 acknowledgement를 기대하면 제공된 ACK callback을 호출하고, native Socket.IO emit, multi-room fan-out, `.volatile`, `@WebSocketServer()` 대체 코드에는 `@fluojs/socket.io`의 `SOCKETIO_SERVER`를 주입하세요. 이 패키지는 Node.js `>=24.0.0 <27` server-backed adapter와 공식 Bun engine path를 대상으로 하며 Deno와 Workers는 지원하지 않습니다. Bun은 static CORS shape를 요구하고 모든 runtime에서 `@WebSocketGateway({ serverBacked })`를 거부합니다. `@WebSocketGateway({ path })`는 고정된 `/socket.io/` Engine.IO request path가 아니라 Socket.IO namespace를 선택하며 migration한 gateway는 singleton provider/controller여야 합니다. Request/transient registration은 warning 후 skip됩니다.
...
- 기본 HTTP cache key는 query-aware `'route+query'`로, 구체적 요청 path와 정규화한 query 값을 함께 사용한다. 이 동작에는 `httpKeyStrategy`를 생략하고, query 값이 의도적으로 응답에 영향을 주지 않을 때만 `'route'`를 선택한다. function strategy와 `@CacheKey(...)`는 custom key에 계속 사용할 수 있다.
...
GraphQL migration에서는 schema와 discovery wiring을 명시적으로 유지한다. 모든 resolver class를 authored module의 provider로 등록해 compiled module graph에서 discovery할 수 있게 한다. `GraphqlModule.forRoot({ resolvers: [...] })`는 이 class들을 등록하지 않으며, `resolvers`를 전달하면 해당 allowlist로 discovery를 제한한다. `resolvers`를 생략하거나 빈 list를 전달하면 provider로 이미 등록된 decorated resolver class를 모두 discovery한다. TypeScript 반환 타입이나 NestJS design metadata가 provider를 등록하거나 output type을 만들지 않는다.

Code-first runtime은 두 category로 제한된 resolver surface를 지원한다. Root `Query`, `Mutation`, `Subscription` operation은 `@Query(...)`, `@Mutation(...)`, `@Subscription(...)`을 사용한다. Object field는 `@Resolver('TypeName')`과 `@FieldResolver(...)`로 연결하지만, 해당 named object type이 code-first root operation output에서 도달 가능할 때만 연결된다. Field resolver를 등록해도 임의의 detached type이 도달 가능해지지는 않는다. TC39 표준 데코레이터는 parameter-decorator 문법을 지원하지 않으므로 `@Parent(index?)`와 `@Context(index?)`는 method decorator이며, 기본값으로 parent/source object를 position `0`에, `GraphQLContext`를 position `1`에 바인딩한다. Code-first field argument DTO binding은 `@FieldResolver({ input, argTypes? })`와 `@Args(index?)`로 지원하며 schema-first field-resolver attachment는 계속 지원하지 않는다.

`GraphqlModule.forRootAsync({ inject, useFactory })`는 명시적인 application token과 factory를 지원한다. NestJS의 `imports`, `useClass`, `useExisting`, 암시적 provider discovery는 계속 지원하지 않는다. `@Subscription({ topics })`도 계속 지원하지 않으며 subscription method는 `AsyncIterable`을 반환해야 한다. `@fluojs/graphql`은 패키지 자체의 검증된 지원 계약으로 Node.js `>=24.0.0 <27`을 선언한다. Portable `@fluojs/runtime`과 `@fluojs/config`에는 package-wide `engines.node`가 없으며 config의 Node 전용 env-file 및 watch 기능은 실행 시점에 보호된다. HTTP와 SSE는 이 경계 안에서 Web-standard HTTP seam을 사용하고, 선택적 WebSocket subscription에는 upgrade listener를 제공하는 server-backed Node HTTP/S adapter도 필요하다. Dependency metadata 정렬과 native runtime 검증이 없다면 이 내부 seam을 Bun, Deno, Cloudflare Workers package 지원으로 해석해서는 안 된다.
...
- transport 구현은 전용 subpath에서만 import합니다: `@fluojs/microservices/tcp`, `@fluojs/microservices/redis`, `@fluojs/microservices/redis-streams`, `@fluojs/microservices/nats`, `@fluojs/microservices/kafka`, `@fluojs/microservices/rabbitmq`, `@fluojs/microservices/grpc`, `@fluojs/microservices/mqtt`. Root barrel은 transport class나 option type을 re-export하지 않습니다.
...
gRPC adapter를 사용하기 전에 `@grpc/grpc-js@^1.14.4`와 `@grpc/proto-loader@^0.8.0`을 설치하세요. 오래된 gRPC peer에서 업그레이드한다면 lockfile을 갱신하고 proto-loader dependency chain이 `protobufjs@7.6.5` 이상으로 resolve되는지 확인하세요. NestJS `Transport.GRPC`는 단일 `options.credentials` 필드로 credential을 설정했습니다. fluo의 `GrpcMicroserviceTransport`는 인바운드 서버와 아웃바운드 클라이언트를 모두 담당하므로, 역할 경계를 넘는 불안전한 credential 재사용을 방지하기 위해 `Server.bindAsync`용 `serverCredentials`(`grpc.ServerCredentials.createInsecure()` 기본값)와 아웃바운드 클라이언트용 `channelCredentials`(`grpc.credentials.createInsecure()` 기본값)로 분리합니다. 기존 `options.credentials`는 마이그레이션 중 서버 credential fallback으로 허용되며 아웃바운드 클라이언트로 불안전하게 전달되지 않습니다.
...
- NestJS cache option의 단위, 기본값, 소유권이 그대로 유지된다고 가정하는 방식. fluo `ttl`은 초 단위이고 memory 경로 기본값은 `300`이며, `@CacheTTL(...)`은 정적 숫자만 받고, 설치한 underlying `cache-manager` dependency generation이 millisecond를 정의할 때에만 NestJS TTL을 `1000`으로 나누어 변환하며, `httpKeyStrategy`를 생략하면 query-aware `'route+query'`를 사용하고 명시적 `'route'`는 query-insensitive이며, `global`이 `isGlobal`을 대체하며 기본값은 module-local인 `false`다. NestJS store adapter는 먼저 Fluo `CacheStore` 계약에 맞게 적응해야 하고 adapted custom store의 shutdown은 `close()` 또는 `dispose()`를 통해서만 전달되며, 직접 전달한 Redis client는 애플리케이션 소유로 남는다.
...
`@OnEvent(...)`는 public instance 메서드만 지원합니다. 유일한 공개 발행 경로는 `EventBusService.publish(event, options?): Promise<EventPublishResult>`입니다. `EventBusModule.forRoot()`는 기본 global이며 provider를 모듈-local로 유지하려면 `{ global: false }`를 전달하세요. Handler와 transport 실패는 log되고 격리되므로 publish result는 attempt가 settle된 뒤 resolve하며(또는 shutdown-tracked background work를 scheduling한 뒤 resolve), 패키지는 Node.js `>=24.0.0 <27`이 필요합니다. 좁은 `@fluojs/event-bus/integration` 서브패스는 first-party integration용 shutdown coordinator seam일 뿐입니다.
