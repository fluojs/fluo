...
| NestJS Socket.IO gateway return value 또는 `@WebSocketServer()` | `@fluojs/socket.io`와 `@OnMessage(...)`, 명시적 acknowledgement callback, `@Inject(SOCKETIO_SERVER)` | Socket.IO handler 반환값은 암묵적인 emit 또는 ACK reply가 되지 않습니다. Gateway server 접근, multi-room emit, volatile delivery를 마이그레이션할 때는 `SOCKETIO_SERVER`를 주입하세요. |
...
| named client, `forFeature(...)`, custom client token, 또는 `isGlobal`을 가정하는 NestJS Slack module | `@fluojs/slack`의 `SlackModule.forRoot({ ..., global? })` 또는 `SlackModule.forRootAsync({ inject, useFactory, global? })` | fluo Slack registration은 singleton 중심이다. 이 패키지는 `global: false`가 설정되지 않으면 `SlackService`, `SlackChannel`, `SLACK`, `SLACK_CHANNEL`을 기본 global로 export한다. 여러 Slack client가 필요하면 app-owned module/provider 또는 facade를 조합한다. |
...
- Socket.IO migration도 같은 명시적 websocket handler 모델을 유지합니다. Handler return value는 await된 뒤 무시됩니다. Client가 acknowledgement를 기대하면 제공된 ACK callback을 호출하고, native Socket.IO emit, multi-room fan-out, `.volatile`, `@WebSocketServer()` 대체 코드에는 `@fluojs/socket.io`의 `SOCKETIO_SERVER`를 주입하세요.
...
- Slack migration은 named multi-client registration clone이 아니다. `@fluojs/slack`은 singleton compatibility token인 `SLACK`과 `SLACK_CHANNEL`을 노출하며, `forFeature(...)`, named registration, named client token factory, per-client custom token surface를 노출하지 않는다. 또한 NestJS `isGlobal` 대신 기본 global visibility를 가진 `global?: boolean`을 사용한다.
...
- NestJS HTTP adapter lifecycle hook을 Bun에서 시작 후 live server mutation으로 옮길 수 있다고 가정하는 방식. `@fluojs/platform-bun`은 `listen()`이 시작되기 전에 dispatcher와 realtime seam을 바인딩하고, 중복 `listen()` 호출을 idempotent하게 유지하며, NestJS-style late host mutation 대신 외부 소유 `Bun.serve(...)` host를 위한 동기 `createBunFetchHandler(...)`를 노출한다.
...
- Slack `forFeature(...)`, named registration, named client token factory, per-client custom token surface, 또는 NestJS `isGlobal` option이 존재한다고 가정하는 방식. fluo Slack은 singleton `SLACK` / `SLACK_CHANNEL` token과 기본 global module visibility에서 벗어나기 위한 `global?: boolean`을 사용한다.
