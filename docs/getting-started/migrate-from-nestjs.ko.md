...
| Nest Devtools `SerializedGraph` | static 일대일 대응 없음; compiled DI graph 분석이 필요하면 지원되는 Node live 경로에서 `fluo dev --studio` 사용 | Static `fluo inspect` artifact는 Nest Devtools `SerializedGraph`와 호환되지 않습니다. Compiled module/provider graph, provider scope, lifecycle diagnostics를 생략하므로, 해당 graph가 필요한 workflow는 file-first migration fallback을 graph parity로 취급하지 말고 Node live Studio에 유지하세요. |
