...
| dynamic-module `imports`, `useClass`, `useExisting` 또는 provider discovery를 사용하는 NestJS JWT async registration | `@fluojs/jwt`의 `JwtModule.forRootAsync({ inject, useFactory })` | `JwtModule.forRootAsync(...)`의 `inject`에 지정한 의존성은 JWT options provider가 resolve되기 전에 application module graph에 먼저 등록해야 하며, `useFactory`는 final JWT options를 반환한다. `imports`, `useClass`, `useExisting`는 지원하지 않고, 받아서 무시하는 compatibility field가 아니라 options shape 밖의 field다. `JwtModule.forRootAsync(...)`는 암묵적 module 또는 provider discovery를 지원하지 않는다. |
...
- NestJS JWT async registration은 dynamic-module 형태를 그대로 복제하지 않는다. `JwtModule.forRootAsync({ inject, useFactory })`에서 주입할 모든 의존성은 application module graph에 먼저 등록해야 하고, `useFactory`는 final JWT options를 반환해야 한다. `JwtModule.forRootAsync(...)`는 NestJS `imports`, `useClass`, `useExisting`를 지원하지 않으며, 이 field들은 options shape 밖에 있고 조용히 무시되지 않는다. `JwtModule.forRootAsync(...)`는 암묵적 module 또는 provider discovery를 지원하지 않는다.
