...
- `TestingModuleRef`는 컴파일된 module context를 소유하고, `createTestApp(...)`은 자체 runtime context를 가진 request-driven app facade를 반환한다. NestJS-style 공유 application instance 소유권에 의존하지 말고 각 테스트 뒤에 반환된 module 또는 app을 닫아야 한다.
