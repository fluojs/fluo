...
| NestJS `PartialGraphHost`의 startup-failure graph 접근 | 직접 대응 없음; CLI stderr와 기본 bootstrap error diagnostic 사용 | Studio report는 bootstrap 이후 artifact다. `fluo inspect`는 snapshot, timing data, report를 내보내기 전에 애플리케이션 생성을 완료해야 한다. Bootstrap이 실패하거나 멈추면 partial graph나 report가 생성되지 않으며, 필요하다면 partial-graph 지원은 별도로 설계한다. |
