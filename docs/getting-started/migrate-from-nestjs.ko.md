...
| NestJS 요청 transaction interceptor | 영속성 패키지의 서비스 `@Transaction()` 또는 controller/request 경계의 명시적 `requestTransaction(...)` | `PrismaTransactionInterceptor`는 기존 import를 위한 deprecated 1.x 호환성 bridge로 유지된다. Mongoose에는 호환성 interceptor가 없으므로 같은 request-chain 위치에 애플리케이션 소유 interceptor를 두고 `RequestContext.request.signal`을 `MongooseConnection.requestTransaction(...)`에 전달한다. `DrizzleTransactionInterceptor`는 기존 import를 위한 deprecated 1.x 호환성 bridge로 유지된다. |
