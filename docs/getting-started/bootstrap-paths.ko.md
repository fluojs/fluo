...
- `ConfigModule.forRoot(...)`는 동기적으로 config provider를 등록합니다. Config 로딩과 전달된 동기 schema 검증은 bootstrap 중 `ConfigService`를 resolve할 때 listen 전에 수행됩니다. 명시적 `ConfigModule.load(...)`는 호출 시 실행됩니다. Adapter 입력이 config에 의존하면 adapter 생성 전에 preload와 검증을 마치고 같은 snapshot을 `envFilePaths: []`로 등록합니다. `ConfigService`와 `CONFIG_RELOADER`는 하나의 `ConfigModule.forRoot(...)` 등록을 공유하며, `ConfigReloadManager.create(...)`는 standalone 대안입니다. 순서형 `envFilePaths`는 낮은 우선순위에서 높은 우선순위 순으로 적용되고, 생략하면 기본 `.env`를 사용하며 `[]`는 파일 로드를 비활성화합니다. Schema 실패는 `INVALID_CONFIG`로 load를 거절하며, 생성 스타터에는 엄격한 port schema가 없습니다. [설정 규칙](../architecture/config-and-environments.ko.md)을 참고하세요.
