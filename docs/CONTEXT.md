...
Vite tooling discoverability is split across `packages/vite/README.md`, [`docs/reference/package-chooser.md`](./reference/package-chooser.md), and [`docs/reference/toolchain-contract-matrix.md`](./reference/toolchain-contract-matrix.md): use `@fluojs/vite` when Vite must build TypeScript with TC39 standard decorators through Babel's `@babel/plugin-proposal-decorators` `{ version: '2023-11' }` transform while preserving fluo's Vite file-boundary skips.

Realtime Socket.IO discoverability is split across `packages/socket.io/README.md` and governed package-surface docs: the package README documents `SocketIoModule.forRoot(...)`, `SocketIoHandshakeRequest`, deny-by-default CORS, positive-integer numeric option validation, Node.js server-backed and Bun engine runtime limits, adapter-owned/shared HTTP listener ownership during shutdown, and bounded shutdown force-disconnect/retry semantics; [`docs/reference/package-surface.md`](./reference/package-surface.md) carries the canonical `@fluojs/socket.io` runtime limitation summary.

Realtime WebSockets discoverability is split across `packages/websockets/README.md`, [`docs/reference/package-surface.md`](./reference/package-surface.md), and [`docs/reference/package-chooser.md`](./reference/package-chooser.md): the package README documents the root `@fluojs/websockets` Node.js default, runtime-specific subpaths `@fluojs/websockets/node`, `@fluojs/websockets/bun`, `@fluojs/websockets/deno`, and `@fluojs/websockets/cloudflare-workers`, and the shared gateway decorator and metadata authoring primitives exposed from each subpath so fetch-style applications can author gateways without loading the root Node.js-backed entrypoint.

Redis integration discoverability is split by responsibility: `packages/redis/README.md` documents `RedisModule.forRoot({ lifecycle })` connect/quit timeout guardrails, platform health/readiness status snapshot helper responsibility, and the raw-client rule that Pub/Sub subscribers need dedicated Redis connections; [`docs/reference/package-surface.md`](./reference/package-surface.md) carries the canonical `@fluojs/redis` surface summary; and [`book/intermediate/ch03-redis-transport.md`](../book/intermediate/ch03-redis-transport.md) explains the Redis Pub/Sub versus Redis Streams learning path, including why shared command clients should not be reused as subscribed Pub/Sub connections.

Observability discoverability is split across metrics and health packages: `packages/metrics/README.md`, [`docs/architecture/observability.md`](./architecture/observability.md), and [`book/beginner/ch19-metrics.md`](../book/beginner/ch19-metrics.md) document `MetricsModule.forRoot(...)`, isolated/shared registry ownership, `MetricsService`, low-level `METER_PROVIDER` / `PrometheusMeterProvider`, endpoint-scoped `endpointMiddleware`, and module-level `middleware`; `packages/terminus/README.md` documents `TerminusModule.forRoot(...)`, indicator/provider registration, execution timeouts, and Redis lifecycle-aware diagnostics through `@fluojs/terminus/redis`; and [`docs/reference/package-surface.md`](./reference/package-surface.md) carries the canonical responsibility boundary for both packages.
...
Cron scheduling discoverability is split across the package, governed package-surface docs, package chooser, and the intermediate book: `packages/cron/README.md` documents `CronModule.forRoot(...)`, five-field and six-field cron expression support, decorator and dynamic registry startup behavior, Redis distributed locking, bounded shutdown, public scheduling/status types, and status snapshots, including that Redis peer loading and resolution are distributed-only and occur only when `distributed.enabled` is `true`; [`docs/reference/package-surface.md`](./reference/package-surface.md) records the `@fluojs/cron` responsibility boundary for scheduling, distributed locks, shutdown, health/readiness visibility, and the same distributed-only Redis peer boundary; [`docs/reference/package-chooser.md`](./reference/package-chooser.md) directs users to add `@fluojs/redis` only when enabling Cron distributed locks; and [`book/intermediate/ch12-cron.md`](../book/intermediate/ch12-cron.md) explains the FluoShop learning path with the same bootstrap, dynamic-start lifecycle guarantees, and Redis distributed-lock selection boundary.

Persistence transaction discoverability is split across package READMEs, governed transaction architecture docs, and package-surface docs: `packages/prisma/README.md`, `packages/drizzle/README.md`, and `packages/mongoose/README.md` document package-local standard `@Transaction()` decorators as the primary Service-layer transaction boundary, while `transaction(...)`, `requestTransaction(...)`, `current()`, and `currentSession()` remain explicit low-level escape hatches. [`docs/architecture/transactions.md`](./architecture/transactions.md) records the cross-package boundary semantics, removal of `*TransactionInterceptor` exports, the explicit `requestTransaction(...)` path for rare request-wide boundaries, and the Mongoose-supported auto-session facade method set; [`docs/reference/package-surface.md`](./reference/package-surface.md) carries the canonical package summary.

Mongoose integration discoverability is split across the package and governed package-surface docs: `packages/mongoose/README.md` documents `MongooseModule.forRoot(...)` / `forRootAsync(...)`, exported option and status input types such as `MongooseAsyncModuleOptions<TConnection>` and `MongoosePlatformStatusSnapshotInput`, lifecycle/session behavior, service `@Transaction()` session facade behavior, `MongooseConnection.model(...)` auto-session support for `create`, `find`, `findOne`, `aggregate`, and `bulkWrite`, unsupported `doc.save()` / model-method escape hatches through `currentSession()`, and the compatibility-only role of `createMongooseProviders(...)`; [`docs/reference/package-surface.md`](./reference/package-surface.md) carries the namespace-facade policy that keeps application registration on `MongooseModule.forRoot(...)` / `forRootAsync(...)` while reserving provider helpers for manual composition compatibility; and [`docs/contracts/behavioral-contract-policy.md`](./contracts/behavioral-contract-policy.md) governs transaction lifecycle, readiness, and shutdown documentation parity.
...
Slack discoverability is split across the package README and governed package-surface docs: `packages/slack/README.md` documents `SlackModule.forRoot(...)` / `forRootAsync(...)`, manual provider composition through `createSlackProviders(...)`, direct `SlackService` delivery, `@fluojs/notifications` channel integration, abort-signal propagation, lifecycle-gated sends, and platform status snapshots; [`docs/reference/package-surface.md`](./reference/package-surface.md) records the canonical `@fluojs/slack` responsibility boundary for webhook-first, transport-agnostic Slack delivery without direct `process.env` reads.
...
CLI and Studio diagnostics discoverability is split across the CLI package, Studio package, and governed tooling docs: `packages/cli/README.md` documents `fluo inspect` default JSON output, `--timing` snapshot-plus-timing envelopes, `--report` support artifacts, `--mermaid` Studio delegation, and `--output <path>` artifact writes; `packages/studio/README.md` documents `fluo dev --studio` live sidecar events, static/report compatibility, and public helpers including `isStudioLiveEvent(value)`; [`docs/reference/toolchain-contract-matrix.md`](./reference/toolchain-contract-matrix.md) carries the canonical CLI scaffolding and inspect artifact output contracts, including that `--timing` defaults to JSON when no explicit output mode is selected; and [`docs/reference/package-surface.md`](./reference/package-surface.md) records the package responsibility split between `@fluojs/cli` artifact emission and `@fluojs/studio` live devtool/artifact viewing/rendering.
