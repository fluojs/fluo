...
Cron scheduling discoverability is split across the package, governed package-surface docs, and the intermediate book: `packages/cron/README.md` documents `CronModule.forRoot(...)`, five-field and six-field cron expression support, decorator and dynamic registry startup behavior, Redis distributed locking, bounded shutdown, public scheduling/status types, and status snapshots; [`docs/reference/package-surface.md`](./reference/package-surface.md) records the `@fluojs/cron` responsibility boundary for scheduling, distributed locks, shutdown, and health/readiness visibility; and [`book/intermediate/ch12-cron.md`](../book/intermediate/ch12-cron.md) explains the FluoShop learning path with the same bootstrap and dynamic-start lifecycle guarantees.
