...
Canonical package and runtime coverage lives in [`docs/reference/package-surface.md`](./reference/package-surface.md), including core additions such as the fluo-native `@fluojs/i18n` package boundary, its `@fluojs/i18n/icu` ICU MessageFormat subpath for plural/select localization, `@fluojs/i18n/http` HTTP locale helpers and opt-in `Accept-Language` policy helpers, its `@fluojs/i18n/adapters` subpath for opt-in non-HTTP locale resolution and header policy helpers, `@fluojs/i18n/validation` validation localization, `@fluojs/i18n/loaders/fs` and `@fluojs/i18n/loaders/remote` catalog loaders with opt-in remote cache wrappers, `@fluojs/i18n/typegen` catalog key and typed translation helper declaration generation, messaging responsibilities such as `@fluojs/microservices` support for TCP, Redis Pub/Sub, Redis Streams, NATS, Kafka, RabbitMQ, MQTT, and gRPC, and persistence responsibilities such as `@fluojs/mongoose` ALS/session transaction ownership. Task-based package discovery lives in [`docs/reference/package-chooser.md`](./reference/package-chooser.md), including localization/i18n selection guidance for `@fluojs/i18n`, its ICU, HTTP locale policy, adapters header policy, validation, loader/cache, and typegen subpaths, plus operations guidance for `@fluojs/terminus`, its `@fluojs/terminus/node` Node memory/disk indicator subpath, its `@fluojs/terminus/redis` Redis indicator subpath, and `execution.indicatorTimeoutMs` slow-indicator timeout guardrails. Ecosystem bridge compatibility decisions for NestJS i18n, i18next, next-intl, and request/validation convenience glue live in [`docs/reference/i18n-ecosystem-bridges.md`](./reference/i18n-ecosystem-bridges.md).
...
| Public API authoring and documentation baseline | `docs/contracts/public-export-tsdoc-baseline.md` | `docs/contracts/platform-conformance-authoring-checklist.md` for platform diagnostics, including non-empty diagnostic `code` and `message` requirements |
