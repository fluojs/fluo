...
Keep the base image on Node.js `>=24.0.0 <27`. The default Fastify adapter path uses `@fluojs/platform-fastify`, whose package manifest declares that exact `engines.node` range. Do not select Node versions below 24 or Node 27+.
...
As the blog grows, database read performance can become a bottleneck. `@fluojs/prisma` does not automatically route generated delegate calls between primary and replica clients. Configure replica behavior with Prisma's supported client/extension facilities or register separate named clients, then select the intended service explicitly. Keep writes on the primary, account for replica lag in read-after-write flows, and use an explicit `@Transaction((self) => self.primaryPrisma)` accessor when a service owns multiple Prisma integrations.
