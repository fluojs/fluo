...
`@Module()`은 definition이 명시적 `undefined`인 경우까지 포함해 `@Module({})`의 축약입니다.
Metadata를 등록하고 앞선 부분 필드와 어느 순서의 `@Module({ global: true })`도 보존합니다. Provider나
constructor token을 추론하지 않습니다.
...
모든 애플리케이션에는 최소 하나의 모듈이 있으며, 보통 그 이름은 `AppModule`입니다. 처음에는 모듈을 “공개 표면과 내부 구현 영역을 가진 경계”로 이해하면 충분합니다. 이 관점은 이후 `imports`, `providers`, `controllers`, `exports`가 왜 나뉘는지도 자연스럽게 설명해 줍니다.
...
프로바이더는 fluo가 대신 관리해 주는 재사용 가능한 의존성입니다. 가장 흔한 예는 서비스지만, 설계에 따라 팩토리, 리포지토리, 헬퍼, 어댑터도 프로바이더가 될 수 있습니다. fluo에서는 클래스를 `@Module(...).providers`에 등록할 때 그 클래스가 컨테이너에 참여하고, 생성자에 다른 토큰이 필요하면 소비하는 쪽에서 `@Inject(...)`로 의존성을 명시합니다.
...
같은 로직을 두 컨트롤러에 복사하게 된다면, 그 로직이 프로바이더가 되기를 원한다는 신호인 경우가 많습니다. 중복이 보이는 순간 책임의 위치를 다시 묻는 습관을 들이면, 컨트롤러는 얇게 유지되고 테스트 가능한 단위도 더 자연스럽게 생깁니다.
...
컨트롤러는 들어오는 요청을 받고 응답을 반환합니다. 즉, 기능의 전송 계층과 맞닿은 가장자리입니다. HTTP 중심 코드에서 컨트롤러는 라우트 경로가 메서드에 매핑되는 곳이며, 서비스나 프로바이더가 가진 로직을 외부 요청과 연결하는 얇은 조율자 역할을 합니다.
...
이제 용어 설명에서 실제 애플리케이션 구조로 넘어갈 차례입니다. 이 아이디어를 FluoBlog에 적용해 게시물 전용 기능 모듈을 만들어 봅시다. 앞에서 본 모듈, 프로바이더, 컨트롤러 개념이 하나의 작은 기능 안에서 어떻게 맞물리는지 확인할 수 있습니다.
