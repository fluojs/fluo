...
Now that the goal is clear, let's prepare the project environment first. `@fluojs/prisma` is the Node.js `>=24.0.0 <27` Prisma integration path because its transaction context depends on host `AsyncLocalStorage`. We need to install the required packages.
...
`PrismaModule` deliberately keeps its public lifecycle contract small. The module calls Prisma's built-in `$connect()` during startup and `$disconnect()` during clean shutdown when those methods exist on the provided client, but it does not expose extra pre-connection or post-disconnection hooks. If shutdown overlaps startup, it waits for the in-flight `$connect()` before disconnecting, and the late connect completion cannot make the module ready again or admit new transaction work. If you need startup health checks or shutdown telemetry, compose that logic in your own providers around the Prisma client rather than relying on undocumented module callbacks.
...
The transaction context that `PrismaServiceFacade` and `PrismaService` rely on is backed by host-provided `AsyncLocalStorage`. Although `@fluojs/prisma` targets Node.js `>=24.0.0 <27`, that engine range does not guarantee that the host lookup will yield a usable `AsyncLocalStorage`. The package first checks `globalThis.AsyncLocalStorage`, then makes a guarded `process.getBuiltinModule('node:async_hooks')` lookup. If neither path returns an `AsyncLocalStorage` constructor, or if the built-in module lookup throws, the package fails closed instead of falling back to a synchronous stack-based context that would silently lose `current()` across async boundaries. When the client supports interactive transactions, `transaction()` and `requestTransaction()` reject before opening one, and `createPlatformStatusSnapshot()` reports `readiness.status: 'not-ready'` with `details.transactionContext: 'unavailable'`.
