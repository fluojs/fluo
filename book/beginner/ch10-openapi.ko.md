...
`@fluojs/openapi` 패키지를 사용하면 이러한 기존 구조에 필요한 정보만 "태그"로 붙일 수 있습니다. 명시적으로 등록된 request DTO를 변경하면 생성 schema도 함께 갱신됩니다. 새로운 route는 controller를 `sources`에 나열하거나 handler mapping을 `descriptors`로 전달한 뒤에만 문서에 나타납니다. 문서가 구현과 가까이, 말 그대로 코드 바로 윗줄에 머물면 업데이트 누락 가능성이 크게 줄어듭니다.
...
- `ui: true`: fluo가 `/docs`에서 Swagger UI를 제공하도록 opt-in합니다. 이 옵션이 없어도 `/openapi.json`은 제공되지만 `/docs`는 not found를 반환합니다.
- `documentPath`와 `uiPath`: JSON 문서와 Swagger UI를 위한 선택적 runtime route입니다. 기본값은 `/openapi.json`과 `/docs`입니다.
...
생성된 JSON 문서와 UI는 다음 기본 경로에서 확인할 수 있습니다. 이 두 경로는 각각 자동화 도구와 사람이 읽는 문서를 위한 진입점입니다.
...
- **응답 기대값**: `@ApiResponse`가 없으면 fluo는 method-derived 기본 success status(`POST`는 `201`, 그 외는 `200`)와 `OK` description을 생성합니다. Handler 반환값이나 TypeScript 반환 타입에서는 response body schema를 추론하지 않습니다.
...
`ui: true`는 `/docs`에서 내장 Swagger UI를 제공하며, opt-in하지 않으면 UI serving은 비활성화됩니다. 기본 페이지는 고정된 `swagger-ui-dist` CSS와 JavaScript URL을 사용합니다. Offline 또는 CSP-controlled deployment에서는 `swaggerUiAssets.cssUrl`과 `swaggerUiAssets.jsBundleUrl`로 교체할 수 있습니다. `documentTransform`, `securitySchemes`, `extraModels` 같은 option으로 생성 문서 자체도 계속 조정할 수 있습니다.
...
하나의 application에서 두 문서를 모두 제공하려면 각 module instance에 서로 다른 `documentPath`와 `uiPath`를 지정하세요.
...
fluo는 HTTP route rule로 이 path들을 정규화합니다. 두 OpenAPI module route 또는 다른 application `GET` route가 같은 path로 정규화되면 하나를 암묵적으로 선택하지 않고 bootstrap이 `RouteConflictError`로 실패합니다.
...
- 문서화할 handler는 `sources`, `descriptors`, 또는 둘 모두를 통해 명시적으로 등록해야 하며, explicit descriptor가 충돌하는 source operation을 덮어씁니다.
