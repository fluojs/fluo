...
헬스 체크가 실패했을 때, 단순히 인프라에 `503` 에러를 반환하는 것만으로는 부족할 때가 많습니다. 개발자들이 조사할 수 있도록 실패의 세부 정보를 로그로 남기고 싶을 것입니다. `@fluojs/terminus`는 헬스 보고서에 구조화된 실패 세부 정보를 반환합니다. 애플리케이션에서 로그가 필요하다면, 애플리케이션 경계에서 이 세부 정보를 자체 logger, interceptor, platform observability pipeline에 연결하세요. 이러한 애플리케이션 레벨 통합에는 indicator key, status, message, 그리고 custom indicator가 반환한 metadata를 포함할 수 있습니다.
...
Node.js resource indicator는 `@fluojs/terminus/node`에서만 import하고 Redis indicator는 `@fluojs/terminus/redis`에서 import합니다. Standalone probe는 `XHealthIndicator.create(options)`로 만들며 memory/disk indicator는 root export가 아닙니다. 이 subpath 경계는 Node probe를 runtime-specific으로, Redis lifecycle-aware probe를 Redis subpath를 통한 opt-in으로 유지하고, root `@fluojs/terminus` import를 선택적 Redis 또는 Prisma peer 및 선택적 Drizzle peer 없이 안전하게 만듭니다. 유지되는 `createPrismaHealthIndicatorProvider(...)` DI helper로 등록한 Prisma indicator는 해당 provider가 있을 때 named registration을 포함한 `@fluojs/prisma` service lifecycle status를 계속 사용합니다. 유지되는 `createDrizzleHealthIndicatorProvider(...)` DI helper로 등록한 Drizzle indicator는 lifecycle-aware `@fluojs/drizzle` wrapper를 우선 사용하고 non-Node driver registration에서는 raw Drizzle handle로 fallback합니다.
...
- **Terminus**는 애플리케이션 health/readiness를 집계하고 shutdown-readiness signal을 제공합니다. 종료 신호, 시간 설정, 정리 순서는 호스트, 어댑터, 런타임이 소유합니다.
