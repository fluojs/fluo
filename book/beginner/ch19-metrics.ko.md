...
Google의 SRE 핸드북은 모니터링의 "네 가지 황금 신호"로 지연 시간(Latency), 트래픽(Traffic), 에러(Errors), 포화도(Saturation)를 정의합니다. Fluo의 기본 scrape endpoint는 process 및 Node.js collector 가시성을 제공하고, 요청 지연 시간·트래픽·에러 메트릭은 이 장 뒤에서 보여 주는 `http: true` 또는 `http` 옵션을 명시적으로 활성화해야 기록됩니다. 이 신호를 먼저 보면 프로덕션 문제를 분석할 때 출발점이 분명해집니다. 예를 들어 높은 포화도와 결합된 지연 시간 급증은 대개 CPU나 메모리 리소스를 확장해야 한다는 신호입니다.
...
## 19.3 Basic Setup
기본 설정은 의도적으로 작습니다. 커스텀 대시보드를 만들기 전에 먼저 유용한 텔레메트리를 노출할 수 있게 해 줍니다. 처음부터 많은 지표를 설계하려고 하기보다, 기본 엔드포인트를 열고 실제 데이터가 쌓이는 흐름을 확인하는 것이 더 안전한 출발점입니다.

패키지를 설치합니다: `pnpm add @fluojs/metrics`

그 다음 루트 `AppModule`에 모듈을 등록합니다:
...
각 `MetricsModule.forRoot()` 호출은 bootstrap이 `METRICS_REGISTRY`를 제공하지 않는 한 격리된 `prom-client` `Registry`를 소유합니다. legacy `registry` module option은 제거되었습니다. shared collector는 `FluoFactory.create(...)`에서 `METRICS_REGISTRY`를 제공하고, 독립 application마다 별도 provider를 제공하도록 migration하세요. `/metrics` 엔드포인트가 호출되면 모듈은 해당 active Registry를 스크레이프하고 현재 값을 수집하여 Prometheus 텍스트 응답 형식으로 변환합니다. 이 과정은 스크랩 요청이 애플리케이션 성능에 불필요한 부담을 주지 않도록 가볍게 유지되어야 합니다.
...
## 19.5 Custom Metrics
`MetricsService`를 사용하여 비즈니스 전용 이벤트를 추적할 수 있습니다. 이 서비스는 non-global service이므로, `MetricsModule.forRoot(...)` registration을 직접 import한 module 또는 `MetricsService`를 re-export하는 module을 import한 module에서 inject할 수 있습니다. 관련 없는 sibling module에는 자동으로 제공되지 않습니다. 커스텀 메트릭은 범용 서버 모니터링을 애플리케이션의 실제 가치 흐름과 연결하는 장치입니다.
...
메트릭 수가 매우 많거나 스크랩 빈도가 매우 높은 경우, 메트릭 응답을 생성하는 행위 자체가 성능 병목 현상이 될 수 있습니다. 현재 `@fluojs/metrics` 모듈은 내장 응답 캐시 옵션을 제공하지 않으므로, 이 동작이 필요하다면 short-lived caching을 reverse proxy, platform middleware, 또는 별도 애플리케이션 middleware 경계에서 추가하세요.

이는 서버가 이미 부하를 받고 있는 트래픽 급증 시에 특히 유용합니다. 해당 캐시를 `MetricsModule` 밖에 두면 소유권 경계가 명확해지고, 모니터링 데이터 신선도에 큰 영향을 주지 않으면서 서버 CPU 사용량을 줄일 수 있습니다. 모니터링 시스템을 가볍게 유지하면 중요한 성능 이벤트 중에도 관측 데이터 수집이 애플리케이션 부하를 더 키우지 않습니다.

## 19.7 Platform Telemetry
`fluo`는 platform shell 상태도 메트릭으로 노출합니다. 이를 통해 런타임이 등록된 platform component에 대해 보고하는 readiness 및 health snapshot을 모니터링 도구에서 확인할 수 있습니다. 이러한 "자가 모니터링" 기능은 애플리케이션 구조와 관련된 문제를 디버깅할 때 유용한 출발점입니다.

- `fluo_component_ready`: `PLATFORM_SHELL` snapshot의 라벨을 사용하여 platform component가 ready이면 `1`, 아니면 `0`을 기록합니다.
- `fluo_component_health`: `PLATFORM_SHELL` snapshot의 라벨을 사용하여 platform component가 healthy이면 `1`, 아니면 `0`을 기록합니다.
- `fluo_metrics_registry_mode`: 현재 메트릭 레지스트리가 어떤 모드로 동작하는지를 `mode="isolated"` 또는 `mode="shared"` label과 gauge value `1`로 노출합니다.
...
이 경계를 이해하면 대시보드 해석도 더 쉬워집니다. 내장 platform telemetry는 등록된 platform component가 준비되었고 건강한지를 보여주며, HTTP traffic 가시성은 HTTP 계측을 활성화한 경우에만 나타납니다. 더 깊은 인프라 분석은 그 위에 별도로 추가한 계측에 맡기는 식입니다.
