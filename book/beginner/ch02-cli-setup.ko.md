...
명시적인 `--platform fastify` 경로는 `@fluojs/platform-fastify`를 선택하므로 해당 패키지의 정확한 Node.js `>=20.19.3 <21 || >=22.2.0 <27` runtime 범위를 그대로 따릅니다. Starter는 로컬 개발을 위해 일반 HTTP로 실행됩니다. Node 프로세스가 HTTPS/TLS를 직접 소유해야 한다면 Chapter 21에서처럼 bootstrap boundary에 `https` option을 명시적으로 전달하세요.
