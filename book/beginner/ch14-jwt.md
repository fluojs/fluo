...
In a multi-tenant environment where a single Fluo application serves multiple organizations, JWT configuration must be planned around the supported module contract. `JwtModule.forRootAsync(...)` can resolve signing and verification settings from injected providers at module startup, but the factory does not receive per-request context such as a tenant ID in a custom header. Treat request-context tenant routing as application-level strategy or guard logic.

For verification, prefer token-bound key selection. Issue tokens with a tenant-specific `kid` header and configure `keys[]`, `jwksUri`, or `secretOrKeyProvider` so the verifier can select the right key from token metadata. `secretOrKeyProvider` receives the decoded JWT header only; if your routing decision depends on request headers, route parameters, or session state, resolve that tenant context before calling `verifyAccessToken(...)` and keep that wrapper outside the JWT module options.
