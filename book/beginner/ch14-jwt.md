...
### Rotation Strategy
Fluo implements **Refresh Token Rotation**. Every time a client uses a refresh token to receive a new access token, the server invalidates that refresh token and issues a *new* refresh token. If an attacker and the legitimate user try to use the same refresh token, Fluo detects reuse. A store with `revokeByFamily(...)` invalidates only that token family; a legacy store without it falls back to revoking every refresh-token family for the subject.

This proactive security approach greatly narrows the window in which a leaked refresh token can be abused. Invalidating the compromised token family protects users from ongoing unauthorized access without terminating independent device or session families. The subject-wide compatibility fallback is deliberately broader and safer for stores that have not adopted family revocation yet. This is an important defense-in-depth measure for any application that handles sensitive user data.

For production stores, implement the refresh-token store's durable `rotate(...)` operation so the current token is consumed and the replacement token is persisted together, plus `revokeByFamily(...)` so replay response remains family-scoped. Consume-only stores remain compatible and save the replacement after consumption, but those writes are not atomic.
...
If an attacker tries to use an old refresh token, Fluo's detection mechanism identifies the reuse and asks the store to revoke the compromised family. Stores that do not implement `revokeByFamily(...)` use the safer legacy fallback of revoking every family for that subject. For durable production stores, the consume step and replacement-token persistence should be one atomic store operation so a successful refresh never leaves the client without a stored successor.
