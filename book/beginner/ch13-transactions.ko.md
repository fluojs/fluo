...
`@Transaction()`을 적용하면 해당 메서드 전체가 단일 데이터베이스 트랜잭션 안에서 실행되어야 함을 fluo에 알리게 됩니다. 등록된 client가 Prisma interactive transaction을 지원하면 메서드 성공 시 commit되고 예외가 발생하면 Prisma에 rollback을 위임합니다. Client가 `$transaction(...)`을 노출하지 않고 `strictTransactions`가 `false`인 기본 상태라면 fluo는 rollback 원자성 없이 메서드를 직접 실행합니다. 이 보장이 필요한 production 흐름에서는 `strictTransactions: true`를 설정하세요.
...
서비스 `@Transaction()`은 데이터베이스 원자성을 하나의 비즈니스 작업에 맞추므로 계속 기본 boundary입니다. 더 드물게 controller-level orchestration이 전체 request를 하나의 transaction으로 유지해야 한다면 `requestTransaction(...)`을 명시적으로 호출하고 request `AbortSignal`을 전달하세요.
...
모든 request를 transaction으로 만들지 마세요. Request-wide boundary는 관련 없는 controller 작업이 실행되는 동안에도 database lock을 유지할 수 있으므로, 하나의 집중된 서비스 메서드로 표현할 수 없는 경우에만 사용하세요.
...
- **요청 트랜잭션 경계**는 정말로 드문 요청 전체 트랜잭션이 필요할 때만 `PrismaService.requestTransaction(...)`으로 명시적으로 엽니다.
...
또한 시스템 가용성을 유지하는 데서 트랜잭션 타임아웃이 맡는 역할도 생각해 보아야 합니다. 중요한 테이블에 잠금을 건 채 오래 실행되는 트랜잭션은 애플리케이션 전체를 사실상 멈춰 세울 수 있습니다. `fluo`에서는 서비스 트랜잭션을 짧게 유지하고, 요청 전체 `requestTransaction(...)` 경계가 정말 필요할 때는 애플리케이션 코드에서 명시적인 timeout/cancellation 정책을 전달하며, 데이터베이스 수준에서도 하나의 비정상 요청이 리소스를 독점하지 못하도록 timeout을 설정하는 것을 권장합니다.
