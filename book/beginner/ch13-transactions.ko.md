...
Prisma(따라서 Fluo)는 이미 활성화된 트랜잭션 클라이언트를 재사용하고 모든 것을 가장 바깥쪽 트랜잭션의 일부로 취급함으로써 "중첩된" 트랜잭션을 처리한다는 점에 유의할 필요가 있습니다. 내부 `transaction()` 또는 요청 범위 트랜잭션 호출은 이 컨텍스트에 참여할 수 있지만, 다른 격리 수준 같은 두 번째 트랜잭션 옵션 세트를 전달해서는 안 됩니다. Fluo는 그런 옵션을 조용히 버리지 않고 거부합니다. 일부 데이터베이스는 "Savepoints"를 통해 진정한 중첩 트랜잭션을 지원하지만, Fluo 철학은 혼란을 피하기 위해 트랜잭션 블록을 서비스 계층에 유지할 것을 권장합니다. 여러 번 `this.prisma.transaction`을 호출하고 있다면, 이는 로직을 전체 작업을 조율하는 하나의 응집력 있는 서비스 메서드로 리팩토링해야 한다는 신호일 수 있습니다.
...
요청 범위 트랜잭션 경계는 애플리케이션 종료에도 참여합니다. 종료가 시작되면 fluo는 새로운 Prisma 요청 트랜잭션을 거부하고, 아직 활성화된 요청 트랜잭션을 abort하며, 트랜잭션 경계가 settle될 때까지 기다린 다음 Prisma 클라이언트를 disconnect합니다. 기존 수동 트랜잭션 안에서 요청 범위 경계가 열리면 바깥 트랜잭션 클라이언트를 재사용하고, 그 수동 블록이 끝날 때까지 shutdown tracking에 남습니다.
...
여기까지 왔다면 트랜잭션을 만드는 방법은 보았습니다. 이제는 그것을 사용하면서도 FluoBlog의 데이터 계층을 깔끔하고 효율적으로 유지하는 설계가 중요합니다. 대규모 트래픽이 발생하는 많은 애플리케이션에서, 오래 지속되는 트랜잭션은 성능 저하의 주범입니다. 트랜잭션이 특정 데이터베이스 행을 점유하고 있을 때, 해당 행에 접근하려는 다른 모든 프로세스는 대기해야 하며 이는 전체 시스템에 연쇄적인 병목 현상을 초래합니다. 따라서 트랜잭션을 간결하게 유지하면 데이터베이스의 동시성을 극대화하고, 사용자 기반이 확장되더라도 FluoBlog가 안정적인 응답 속도를 유지하도록 보장할 수 있습니다. 트랜잭션 블록에서 절약된 매 밀리초는 시스템 전체의 처리량 향상으로 이어집니다.
