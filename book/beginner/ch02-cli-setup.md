...
The explicit `--platform fastify` path selects `@fluojs/platform-fastify`, so it inherits that package's Node.js 20.19.3+ runtime floor (`engines.node >=20.19.3`). The starter runs plain HTTP for local development; when the Node process should own HTTPS/TLS directly, pass `https` options at the bootstrap boundary as shown in Chapter 21.
