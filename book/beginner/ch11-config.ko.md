...
또한 많은 서버리스 플랫폼은 환경 변수를 주입하는 고유한 방식을 가지고 있습니다. fluo의 우선순위 규칙은 이런 플랫폼 주입 값을 명시적인 `processEnv` 스냅샷이나 `runtimeOverrides`로 `ConfigModule.forRoot(...)`에 매핑한 뒤에만 적용됩니다. 일단 매핑된 값은 `.env` 입력보다 높은 우선순위를 가지므로, 서버리스 함수가 코드 변경 없이 호스트 환경에 원활하게 적응할 수 있습니다.
