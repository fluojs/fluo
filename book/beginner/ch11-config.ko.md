...
2. **명시적 프로세스 환경 스냅샷(Explicit ProcessEnv Snapshot)**: config loading 또는 module option의 `processEnv`에 전달한 값.
...
HTTP bootstrap은 ambient `process.env`를 다시 읽거나 `PORT`를 두 번째 방식으로 검증하지 않습니다. `validatedConfig`가 module source이자 adapter input입니다. Async secret store 또는 namespaced configuration factory는 동기 module registration 전에 완료해야 하지만, nested object는 deep merge와 dot-path `ConfigService` access가 유지되도록 nested 상태로 전달합니다.
...
두 번째 bootstrap parser를 만들지 말고 registration 예시의 공유 `ConfigSchema`와 `configSources.processEnv`를 확장합니다.
...
3. 우선순위 규칙에 따라 `processEnv`나 `runtimeOverrides`로 명시적으로 전달한 값이 `.env` 파일을 덮어쓰고 있지 않은지 확인하세요.
