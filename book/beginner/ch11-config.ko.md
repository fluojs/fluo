...
보이지 않는 곳에서 `ConfigService`는 병합된 설정 값의 정규화된 인메모리 스냅샷을 유지합니다. `get()`, `getOrThrow()`, `snapshot()`이 객체 형태의 값을 반환할 때는 분리된 clone을 반환하므로 호출자 mutation이 활성 설정 스냅샷을 바꿀 수 없습니다. 이 서비스는 source metadata registry를 노출하지 않습니다. 값의 출처 정보가 필요하다면 `ConfigModule.forRoot(...)`에 넘기는 옵션과 함께 bootstrap 코드에서 별도로 보관하세요.
