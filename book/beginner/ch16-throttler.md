...
Fluo provides the `@fluojs/throttler` package, which integrates with the `fluo` request lifecycle. The package uses an asynchronous design to provide the protection you need while keeping request overhead low. Model keys, limits, stores, and guard placement as application-level policy so that your rate limiting design stays portable when you adapt the rest of your `fluo` code. That policy portability does not mean the root package can be imported on every runtime today: the published root `@fluojs/throttler` package currently targets Node.js `>=24.0.0 <27`.
...
In more advanced scenarios, you may need policy to vary with current system load or user status. `@Throttle()` is not a per-request policy seam: its `{ ttl, limit }` values are validated and copied into static decorator metadata when the decorator is applied. `ThrottlerGuard` then resolves the module-, class-, and method-level precedence once per handler and caches that policy. A runtime policy service cannot select or mutate a different `@Throttle()` configuration for each request.

Keep adaptive decisions in an explicit, application-owned seam. Resolve final `ThrottlerModule.forRoot(...)` options from configuration before bootstrap, use an application guard wrapper to enforce a runtime branch before or alongside the fixed `ThrottlerGuard` policy, or provide a custom `ThrottlerStore` when the variation belongs to the counting or storage algorithm. A `keyGenerator` can choose requester identity per request, but it does not change the resolved limit. For example, a guard wrapper can consult database health or subscription status and enforce the application's dynamic policy; any delegation to `ThrottlerGuard` still uses the handler's cached static metadata. fluo does not ship built-in adaptive rate limiting that changes those limits from CPU or cluster telemetry.
