...
For example, your own policy service can check system health and choose a stricter `@Throttle()` configuration or guard-wrapper branch when database latency crosses a threshold. Or it can check a user's subscription status and grant higher limits to "premium" members before delegating to `ThrottlerGuard`. fluo does not ship built-in adaptive rate limiting that mutates limits from CPU or cluster telemetry; connect that feedback loop through configuration, a custom store, or an application-owned guard if your deployment needs it.
