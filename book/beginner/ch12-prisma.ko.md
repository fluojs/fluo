...
`AppModule` 또는 루트 그래프에서 한 번만 import되는 공용 인프라 모듈에서 `forRoot`를 사용하세요. 현재 `@fluojs/prisma` 패키지는 애플리케이션 컨테이너마다 unnamed Prisma 등록 1개만 노출하므로, 하나의 모듈 그래프에 여러 `PrismaModule` 등록을 import하는 방식은 지원되지 않습니다.
