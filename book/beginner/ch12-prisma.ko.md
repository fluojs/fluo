...
`PrismaServiceFacade`와 `PrismaService`가 의존하는 트랜잭션 컨텍스트는 호스트가 제공하는 `AsyncLocalStorage`를 기반으로 합니다. Node.js 20+에서는 항상 사용할 수 있지만, 만약 사용 가능한 `AsyncLocalStorage`를 노출하지 않는 호스트에서 `@fluojs/prisma`를 실행하면 패키지는 동기 stack 기반 컨텍스트로 fallback하지 않습니다. async boundary 사이에서 `current()`를 조용히 잃기 때문입니다. 대신 `transaction()`과 `requestTransaction()`은 Prisma 트랜잭션을 열기 전에 예외를 던지고, `createPlatformStatusSnapshot()`은 `readiness.status: 'not-ready'`와 `details.transactionContext: 'unavailable'`을 보고합니다.
