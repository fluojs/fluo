...
목표가 분명해졌으니, 먼저 프로젝트 환경부터 준비하겠습니다. `@fluojs/prisma`는 transaction context가 host `AsyncLocalStorage`에 의존하므로 Node.js `>=24.0.0 <27`용 Prisma 통합 경로입니다. 필요한 패키지를 설치해야 합니다.
...
### 트랜잭션 인지 (Transaction Awareness)
fluo에서 주입된 `PrismaServiceFacade`는 컨텍스트를 인지합니다. 즉, `this.prisma.post.create()`를 호출할 때 facade는 현재 활성 트랜잭션(다음 장에서 다룰 내용) 내부에서 실행 중인지 아니면 단독 작업으로 실행 중인지를 자동으로 결정합니다.

`PrismaServiceFacade`의 생성된 모델 delegate를 직접 호출함으로써 리포지토리는 실행 컨텍스트에 신경 쓰지 않고 쿼리 자체에만 집중할 수 있습니다. Transaction boundary를 열거나 확인하기만 하는 provider는 대신 `PrismaService<TClient>`를 주입해야 합니다. 서비스 레벨 트랜잭션에서 여러 쓰기 작업을 하나로 묶더라도 동일한 리포지토리 코드를 계속 사용할 수 있습니다.

이 패턴은 fluo의 "트랜잭션 중립적(Transaction Agnostic)" 리포지토리 설계의 핵심입니다. 이를 통해 데이터 접근 계층을 깔끔하고 타입 안전하게 유지하면서 다양한 트랜잭션 경계에서도 성능을 보장할 수 있습니다.

### AsyncLocalStorage 없는 readiness
`PrismaServiceFacade`와 `PrismaService`가 의존하는 트랜잭션 컨텍스트는 호스트가 제공하는 `AsyncLocalStorage`를 기반으로 합니다. `@fluojs/prisma`가 Node.js `>=24.0.0 <27`를 대상으로 하더라도, 이 engine 범위 자체가 host lookup에서 사용 가능한 `AsyncLocalStorage`를 항상 얻는다는 보장은 아닙니다. 패키지는 먼저 `globalThis.AsyncLocalStorage`를 확인한 뒤, 예외가 발생해도 안전하도록 보호된 `process.getBuiltinModule('node:async_hooks')` 조회를 시도합니다. 두 경로 모두 `AsyncLocalStorage` 생성자를 반환하지 않거나 built-in module 조회가 예외를 던지면, async boundary 사이에서 `current()`를 조용히 잃는 동기 stack 기반 컨텍스트로 fallback하지 않고 fail closed합니다. 클라이언트가 interactive transaction을 지원하는 경우 `transaction()`과 `requestTransaction()`은 Prisma 트랜잭션을 열기 전에 예외를 던지고, `createPlatformStatusSnapshot()`은 `readiness.status: 'not-ready'`와 `details.transactionContext: 'unavailable'`을 보고합니다.

이 `not-ready` 상태는 일반 database readiness 실패와 구분됩니다. Prisma 클라이언트 자체는 연결되어 있고 기능적으로 정상일 수 있습니다. fallback boundary는 애플리케이션이 소유합니다. 트랜잭션 컨텍스트 없이도 데이터베이스 접근이 필요한 handler는 (예: `PRISMA_CLIENT` 토큰을 통해) 원시 `PrismaClient`를 직접 호출하고 자체 일관성 semantics를 관리하거나, `AsyncLocalStorage`를 제공하는 호스트에서 실행해야 합니다. `unavailable` readiness 상태를 health check에 노출하고, 호스트가 ALS를 제공하거나 애플리케이션이 비트랜잭션 접근 경로로 전환할 때까지 트랜잭션 의존 handler로 트래픽을 라우팅하지 마세요.
...
- `PrismaService`와 그 컨텍스트 인식 설계는 유연하고 트랜잭션 인식적인 데이터 접근을 가능하게 합니다.
...
데이터베이스 스키마와 마이그레이션은 핵심 소스 코드의 일부로 다뤄야 합니다. 리포지토리를 가볍고 집중된 상태로 유지하고, 코드가 트랜잭션을 인식하도록 항상 서비스의 모델 메서드를 직접 호출하세요.
 이 원칙을 지키면 FluoBlog가 커져도 데이터 접근 흐름을 안정적으로 유지할 수 있습니다.
