...
To select rollback from a return value, explicitly set `shouldRollback` in the existing final Fluo boundary. See the [Prisma README](../../packages/prisma/README.md#choosing-rollback-from-a-result) for `TransactionBoundaryOptions<Result<T>>` using a consumer-defined Result. Without the option, even failure-looking values return normally and may commit. A root predicate selecting failure returns the same value after native rollback and cleanup succeed.

A nested predicate failure returns its original value while marking the shared owner sticky rollback-only. If the root also rejects its own result, it returns that root value; otherwise, after rollback, `TransactionRollbackOnlyError` carries the first nested failure in `readonly result: unknown`. Unsupported fallback/legacy targets reject before callbacks with `TransactionRollbackCapabilityError`. Native errors are not hidden by domain results and differ from `AfterCommitError` after committed work. Ordinary caught nested exceptions do not mark rollback-only and retain existing commit/hooks. Rollback discards all hooks, and native callback retries use fresh owners. External raw transactions and Redis `MULTI/EXEC` are unsupported, with no additional savepoint or durability guarantees. The [shared-owner contract](../../docs/architecture/transactions.md#result-based-rollback) owns the detailed semantics.
...
To maintain a high-performance system, you need to monitor transaction health in real time. `PrismaService.createPlatformStatusSnapshot()` reports lifecycle, readiness, transaction capability, transaction-context availability, and active transaction state. `details.activeTransactionBoundaries` counts open outer service/manual `transaction(...)` boundaries that shutdown waits to drain before disconnecting, while `details.activeRequestTransactions` separately counts request transaction activity. It does not publish built-in duration, commit-to-rollback, or lock-contention metrics. Add application or database instrumentation for those signals when you need them.
