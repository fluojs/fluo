...
On this supported route path, eviction runs only after the non-GET handler succeeds, `response.send(...)` settles successfully, and the response confirms commit. A send failure or request abort during disconnect or shutdown cancels eviction and preserves the previous cached read. There is no timer-based commit inference, so an adapter or outer pipeline that never invokes `response.send(...)` must complete the standard response lifecycle or invalidate explicitly. Keeping `@CacheEvict(...)` at the controller boundary makes the interceptor responsible for response-aware timing and contains a store deletion failure after the successful handler. Service-layer invalidation stays explicit and testable through `CacheService`: `await cache.del(...)` has no HTTP response-commit boundary and propagates a store deletion failure unless the caller catches or retries it.
