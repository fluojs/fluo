...
For read/modify/write on one cache value, `CacheService.update(...)` uses an optional store capability. It atomically applies a pure reducer; it does not coalesce origin database work. For inventory reservation, successful cache arithmetic does not mean a database order and reservation commit together. Keep authoritative reservations in database conditional updates and transactions, separate from cache-only mutation.

### 17.5.3 Partial Cache Updates: Granularity and Performance
To change some fields of a cached object, construct and return a new object in an `update` reducer. The store replaces the key's whole value; this is not a Redis hash-field update. Splitting data into smaller keys such as `user:1:profile` and `user:1:settings` lets you invalidate only the changed part, but does not combine those keys into one atomic transaction.
...
On this supported route path, eviction runs only after the non-GET handler succeeds and a framework response writer confirms commit. A writer failure or request abort during disconnect or shutdown cancels eviction and preserves the previous cached read. Adapter paths that commit without invoking `response.send(...)` retain a bounded five-second fallback, but it evicts only when `response.committed` already confirms commit at the deadline. An unconfirmed response is cancelled instead, so elapsed time alone cannot evict before a later failed commit. Keeping `@CacheEvict(...)` at the controller boundary makes the interceptor responsible for response-aware timing and contains a store deletion failure after the successful handler. Service-layer invalidation stays explicit and testable through `CacheService`: `await cache.del(...)` has no HTTP response-commit boundary and propagates a store deletion failure unless the caller catches or retries it.
...
`CacheService.update<T>(key, reducer, options?)` adds to the existing `get`, `set`, `remember`, `del`, `reset`, and `close` APIs. The [cache-manager README](../../packages/cache-manager/README.md#atomic-updates) owns inputs, TTL, failures, and ownership; [FluoBlog's standalone consumer](../01-fluoblog/ch20-caching.md#updating-one-cache-value-without-a-key-queue) shows actual module composition for increments without an application key queue.

The reducer decides an explicit set/delete from a detached value or `undefined` for a missing/expired entry, and can rerun on conflicts, so it must not perform I/O. The default maximum is 16 attempts. Memory provides per-key FIFO within one store instance shared by facades, not coordination across separate stores. Redis requires RedisModule DI with cache-side `redis.atomicUpdates: true`, Redis >=6.2 single-primary, and a nonempty application prefix; do not assume Cluster support. A custom store without the capability returns `unsupported`, with no non-atomic fallback.

Omitting TTL on a live entry preserves its absolute expiry and uses the module default only on creation. Update rejects invalid TTL with `RangeError` and applies neither jitter nor existing `CacheObservation` events. This does not change the existing set/remember invalid-TTL no-op, jitter, or observation contracts. Deletion/reset/close cancel late reducers; reset/close await queued updates, reducers, and isolated connection cleanup. A reducer ignoring cancellation can hold drain indefinitely, and nested same-key updates or awaiting reset/close inside the reducer deadlock. Cancellation after Redis EXEC dispatch does not undo commit. Follow the README's operational conditions for reserved metadata, namespace-wide opt-in, the lack of a global SCAN reset snapshot, and failover durability limits.
...
Deletion tied to a database change and the preceding section's HTTP response commit are different boundaries. Registering `afterCommit(async () => { await cache.del(key); })` inside an active Fluo transaction for Prisma, Drizzle, or Mongoose lets you await deletion after the successful final outer native commit. Here, `cache` and `key` denote the application's injected service and actual read key in a consumer fragment. This does not change `@CacheEvict`'s HTTP semantics or automatically discover query-specific keys.

Passing `{ requireAfterCommit: true }` as the database boundary's final argument requires native commit observation before invoking the callback. Rollback and failed commit do not run hooks; hook failures are reported with `AfterCommitError.committed: true`, identifying an already-committed database, and every hook outcome. Do not retry the entire database write. Choose cache recovery, TTL, and durable redelivery policies separately. Invalidating `remember()` in the same service remains different from refills by loaders in other processes.

Redis has no Fluo-owned commit tracking and does not support this API. A database hook calling Redis does not provide DB+Redis atomicity, an outbox, or exactly-once behavior across crashes and networks; future `MULTI/EXEC` requires separate review. Follow [Transaction Context](../../docs/architecture/transactions.md) for shared semantics and the [current FluoBlog caching chapter](../01-fluoblog/ch20-caching.md) for a concrete consumer application.
...
"Write-Through" caching is an application strategy that updates both the database and cache on the write path. It does not mean Fluo guarantees atomicity across the stores or always-fresh reads. Even with post-commit hooks ordering the work, cache failures and concurrent refill policies remain. "Write-Back" is a separate strategy that writes to the cache first and applies changes to the database in the background; the application must accept the risk of data loss when the cache is lost. Neither strategy is an automatically provided delivery guarantee.
