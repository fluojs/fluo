...
스코프는 클라이언트 애플리케이션이 요청한 권한입니다. 예를 들어, 모바일 앱은 사용자가 콘텐츠를 생성할 수 있도록 `posts:write` 스코프를 요청할 수 있습니다. Issuer 또는 Authorization Server가 요청된 scope 중 무엇을 부여할지 결정해 token에 기록하고, token 검증 후 strategy가 검증된 grant를 Principal의 `scopes` field로 정규화합니다.
...
```typescript
// token 검증 후 custom AuthStrategy 내부
return {
  subject: payload.sub,
  claims: payload,
  roles: payload.roles || [],
...
`@UseAuth('jwt')`와 함께 지원되는 `@RequireScopes('posts:write')` 데코레이터로 라우트의 필요 scope를 선언하세요. 그러면 검증된 strategy가 principal을 반환한 뒤 `AuthGuard`가 route requirement를 이미 부여된 scope와 비교합니다. `@RequireScopes(...)`는 scope를 부여하거나 제거하지 않고 token 내용을 제한하지도 않으며, token 발급은 계속 issuer 또는 Authorization Server의 책임입니다. 발급과 route enforcement를 분리하면 탈취된 token이나 손상된 client에 대해 "최소 권한의 원칙(Principle of Least Privilege)"을 유지하고, 특정 기능이 필요할 때만 사용자가 권한을 부여하는 "점진적 동의(Incremental Consent)"를 지원할 수 있습니다.
