...
처음에는 서비스 객체를 그대로 반환해도 된다고 판단하기 쉽습니다. 하지만 내부 필드가 생기는 순간 그 가정은 위험해집니다. 게시글 레코드에는 draft 정보, 내부 id, 작성자 메모, 구현 세부사항 같은 값이 들어 있을 수 있고, 모든 필드가 공개 API에 속하는 것은 아닙니다. 그래서 응답 DTO가 중요합니다. 클라이언트가 실제로 무엇을 보아야 하는지 명확한 계약으로 정하기 때문입니다.
...
흔한 실수 중 하나는 입력과 출력 모두에 하나의 DTO 클래스를 사용하는 것입니다. 이를 종종 "God DTO"라고 부릅니다. 처음에는 간단해 보이지만 금방 복잡해집니다. 예를 들어, 입력에는 비밀번호가 필요할 수 있지만 출력에는 절대로 포함되어서는 안 됩니다. 요청 DTO와 응답 DTO를 처음부터 분리해 두면, 민감한 데이터를 실수로 노출하거나 읽기 전용이어야 하는 필드를 입력으로 받는 일을 막을 수 있습니다.
...
`@Expose({ excludeExtraneous: true })`는 노출 허용 중심의 기본값을 만듭니다. 즉 안전한 기본값은 제외이며, 앱 밖으로 나가야 하는 필드만 명시적으로 허용합니다. 숨겨야 할 필드를 모두 기억하는 방식보다 이 기본값이 더 안전하고, 내부 필드가 나중에 추가되어도 공개 API로 자동 노출될 가능성을 줄여 줍니다.

## 7.3 Serializing Controller Results Automatically

직렬화 패키지는 `serialize(value)`로 값을 직접 가공할 수 있습니다. HTTP 핸들러에서는 인터셉터를 쓰는 패턴이 더 자연스럽습니다. 응답 shaping이 여러 라우트에 반복될 수 있기 때문에, 핸들러 바깥의 공통 파이프라인에 두는 편이 책임이 더 깔끔하게 나뉩니다.
...
직렬화는 cross-cutting concern이며 여러 라우트가 모두 필요로 할 수 있습니다. 인터셉터는 핸들러 실행과 응답 쓰기 사이에 위치하므로 재사용 가능한 응답 shaping을 넣기에 자연스러운 장소입니다. 이 위치 덕분에 엔드포인트 전반에서 일관된 동작을 만들 수 있고, 컨트롤러마다 같은 변환 코드를 반복하지 않아도 됩니다.
...
이렇게 하면 인터셉터와 동일한 강력함과 일관성을 유지하면서도, 변환 시점과 방식을 직접 제어할 수 있습니다. 핵심은 `serialize()`가 여러분이 넘긴 DTO 인스턴스를 기준으로 응답 형태를 다듬는다는 점이므로, 먼저 공개 DTO를 채운 뒤 그 값을 직렬화해야 합니다.

## 7.4 Updating FluoBlog to Return Public Output

이제 posts 기능이 공개 API처럼 느껴지도록 바꿔 봅시다.

서비스는 여전히 더 풍부한 내부 레코드를 다룰 수 있습니다. 컨트롤러는 응답 지향 DTO를 반환하면 됩니다. 이렇게 역할을 나누면 저장 모델을 바꾸더라도 공개 API 계약을 더 안정적으로 유지할 수 있습니다.
...
이 구조는 FluoBlog에 더 나은 관심사 분리를 제공합니다. 내부 레코드 구조는 나중에 바뀔 수 있지만, 공개 응답 계약은 안정적으로 유지할 수 있습니다. 그래서 API 소비자는 내부 구현 변화보다 의도적으로 설계된 응답 형태에 의존하게 됩니다.

### Where `@Transform()` Helps

가끔 공개 응답에는 가벼운 마무리 변환이 필요합니다. summary 값을 trim하거나, 사용자 이름을 대문자로 보여 주거나, 표시 전용 문자열을 정규화해야 할 수 있습니다. 이런 작업은 도메인 로직이라기보다 전송 경계에서의 표현 문제이므로 DTO 근처에 두는 편이 자연스럽습니다.

`@Transform()`은 이런 동기식 경계 변환을 위해 존재합니다. 속성값을 가져와 여러분이 제공한 로직에 따라 다른 값을 반환할 수 있게 해 줍니다. 예를 들어 위 `PublicPostDto`에서 응답 경계를 넘기기 전에 필드를 trim하거나 포맷할 때 유용합니다.

### Advanced Transformations

현재 `@Transform()`은 필드 값을 받아 변환된 결과를 반환합니다. 여러 내부 필드를 하나의 공개 필드로 합쳐야 한다면, 먼저 DTO를 채우기 전에 그 값을 계산한 다음 DTO 필드 값에 대해 직렬화 변환을 적용하는 방식이 가장 안전합니다.
...
이 패턴은 응답 경계에서 가벼운 표시용 포맷팅을 할 때 유용합니다. 이를 통해 서비스 계층을 반복적인 화면 표시 전용 정리 로직으로부터 깔끔하게 유지할 수 있습니다.

## 7.5 Safe Serialization Details Worth Knowing

직렬화기는 애플리케이션이 커질수록 중요해지는 몇 가지 성질을 갖고 있습니다. 재귀적인 객체 순회를 처리하고, 무한 재귀를 피하기 위해 순환 참조를 안전하게 끊으며, 기반 클래스의 데코레이터 계약을 상속합니다. 또한 모든 것을 데코레이터 인스턴스로 가정하지 않고 일반 객체도 신중하게 다룹니다. 이 디테일은 다소 고급스럽게 들릴 수 있지만, 지금은 하나의 실용적인 결론만 기억하면 됩니다. 직렬화기는 단순 편의 함수가 아니라 신뢰할 수 있는 경계 도구로 설계되었습니다.

### What It Does Not Promise

직렬화가 모든 값을 자동으로 엄격한 JSON 원시값으로 바꾸어 준다는 뜻은 아닙니다. `Date`나 `bigint` 같은 값은 클라이언트 계약에 따라 별도 정규화가 필요할 수 있습니다. 이 점은 전송 설계가 여전히 고민이 필요한 문제라는 사실을 상기시켜 줍니다. 데코레이터는 도움을 주지만, 명확한 API 설계를 대신해 주지는 않습니다.

### Performance Considerations

직렬화는 약간의 오버헤드를 추가하지만, fluo에서는 고도로 최적화되어 있습니다. fluo는 무거운 리플렉션(reflection) 라이브러리를 피하기 때문에, 직렬화 과정이 많은 기존 프레임워크보다 빠릅니다. 대부분의 초기 및 중급 애플리케이션에서 성능 영향은 직렬화가 제공하는 보안 및 명확성 이점에 비해 작습니다.
...
좋은 패턴은 서비스나 매퍼가 공개 DTO 생성을 인지하도록 하는 것입니다. 약한 패턴은 내부 객체를 아무 생각 없이 반환하고 민감한 필드가 안 새길 바라기만 하는 것입니다. 응답 경계를 명시적으로 설계하면 이런 기대에 의존하지 않고도 API 노출 범위를 통제할 수 있습니다.
...
초기 프로젝트 단계에서도 의미 있는 진전입니다. 성공 응답이 정돈되면 오류 처리와 API 문서화도 훨씬 더 분명해집니다.

### A Note on Maintenance

애플리케이션이 성장할수록 이러한 관심사 분리는 더 큰 가치를 냅니다. 데이터베이스 컬럼 이름을 바꾸거나 내부 전용 추적 필드를 추가해야 할 때도 공개 API는 깨지지 않습니다. 서비스의 매핑을 업데이트하거나 새 필드에 `@Exclude()`를 사용하면 됩니다. 이러한 디커플링(decoupling)은 모든 클라이언트에게 연동 코드를 다시 짜라고 요구하지 않으면서 백엔드 시스템을 오래 확장할 수 있게 하는 핵심입니다.
