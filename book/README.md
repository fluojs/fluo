...
The testing chapter covers the canonical Node.js `>=24.0.0 <27` `@fluojs/testing` path: `@fluojs/testing/vitest` decorator transforms, `createTestingModule({ rootModule })` slice tests with isolated module/provider overrides, and `createTestApp({ rootModule })` request-pipeline tests that forward runtime bootstrap options and close apps reliably. Node versions below 24 and Node 27+ are outside that published package range.
