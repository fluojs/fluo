...
The testing chapter covers the canonical Node.js `>=20.19.3 <21 || >=22.2.0 <27` `@fluojs/testing` path: `@fluojs/testing/vitest` decorator transforms, `createTestingModule({ rootModule })` slice tests with isolated module/provider overrides, and `createTestApp({ rootModule })` request-pipeline tests that forward runtime bootstrap options and close apps reliably. Node 21, Node 22 before 22.2.0, and unverified Node 27+ are outside that published package range.
