...
이 책의 원본 언어는 한국어다. 한국어 72장 전체를 완성하고 통합 검토한 뒤 영어로 번역한다. 한국어 집필 단계에서 영문 본문을 병행 작성하지 않는다. 확정 목차와 파일 식별자는 [series.json](./series.json)에 있다. 1권은 24장, 2권은 28장, 3권은 20장이다.
...
8. 담당 패키지 README·소스·테스트의 클릭 가능한 근거 링크.
...
한국어 세 권 전체를 먼저 기술·서사 검토한다. 그 결과를 기준으로 영어 .md 파일을 만들며, 장·절 순서, 예제, 명령, 제약, 실패 조건을 누락하지 않는다. 번역은 축약본이나 새 집필이 아니다. 코드 블록 내부 주석도 먼저 동일하게 유지해 실행 예제의 차이를 만들지 않는다. 용어는 멱등성/idempotency, 대사/reconciliation, 보상/compensation, 가시성/visibility, 정리/disposal를 일관되게 사용한다.
