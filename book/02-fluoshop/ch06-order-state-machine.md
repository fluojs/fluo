...
When the second request's conditional update runs after the first has changed version 0 to 1, the second receives an affected row count of 0. That request ends with an exception and creates no audit record. Conversely, if the update succeeds but insertion of the audit row fails, the entire transaction rolls back. This implementation does not set `shouldRollback`, so catching the error and returning `{ ok: false }` could commit the preceding update. This chapter propagates failures as exceptions within the database atomicity boundary.

Expressing expected rejection as a consumer Result requires a separate, explicit predicate in the final Fluo boundary under the [Result-based rollback contract](../../docs/architecture/transactions.md#result-based-rollback). Receiving the same root failure after successful rollback differs from a successful commit. A root cannot ignore sticky rollback-only state set by an opted-in nested failure, and native errors still propagate. This feature does not automatically change order transition rules or the exception-based HTTP contract below.
