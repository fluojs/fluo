...
An alternative for expected review rejection returned as a Result is `TransactionBoundaryOptions<T>.shouldRollback` in the [Mongoose README](../../packages/mongoose/README.md#choosing-rollback-from-a-result). It uses only the existing Fluo boundary position and adds no native options argument. If the root predicate rejects, the same root value returns after native rollback and session cleanup succeed. An opted-in nested failure returns its original value but marks the owner sticky rollback-only; unless the root also rejects its own result, the boundary throws `TransactionRollbackOnlyError` with the first nested failure.

Unsupported fallback/legacy targets reject before callbacks with `TransactionRollbackCapabilityError`. Native errors are not hidden by domain values and differ from post-commit `AfterCommitError` and `AfterCommitCleanupError`. Every native callback retry uses a fresh owner, so previous failure values, rollback-only state, and hooks do not cross attempts. Ordinary caught nested exceptions retain existing commit/hooks, but opted-in rollback discards all hooks. External raw transactions, Redis `MULTI/EXEC`, savepoints, and additional durability guarantees are unsupported. The [shared-owner contract](../../docs/architecture/transactions.md#result-based-rollback) owns the detailed semantics. This chapter's `ReviewAlreadyExists` and HTTP error mapping retain the exception-based choice; the database verification below does not claim to execute this Result alternative.
