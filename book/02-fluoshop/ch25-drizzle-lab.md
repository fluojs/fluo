...
We chose `transaction()` to make the boundary explicit for this comparison. Service decorators also select the wrapper explicitly: `@Transaction((self) => self.prisma, prismaOptions)` or `@Transaction((self) => self.db, drizzleOptions)`. Do not mix the two wrappers through legacy target discovery. In particular, the fallback to direct execution with the default `strictTransactions: false` is not atomicity. Be sure to set it to `true` in the registration below.
...
If the comparison expands to price-card invalidation, both wrappers provide `afterCommit(callback: () => void | Promise<void>): void`. Register cache deletion on the same wrapper after successfully writing the price and history, while still inside the active callback. Do not register it in the `conflict` branch. Hooks are awaited one at a time in registration order after the successful final outer native commit, so an inner `change()` returning is not evidence that an outer transaction has committed. The two wrappers do not share queues, nor do they observe transactions opened directly by raw clients.

Both packages use `transaction(fn, nativeOptions?, boundary?)` for manual calls and `requestTransaction(fn, signal?, nativeOptions?, boundary?)` for request calls. Their canonical decorator signature is `@Transaction(accessor, nativeOptions?, boundary?)`: select the Prisma or Drizzle wrapper first and add `{ requireAfterCommit: true }` only in the final boundary position. No-argument or options-only discovery is legacy single-target compatibility. Omitted native options and fail-open contracts remain unchanged. Opt-in rejects with `AfterCommitCapabilityError` before invoking the callback when native commit observation is unavailable. Even without opt-in, registration on an unsupported boundary, outside a boundary, or in a closed scope is rejected.

Nested calls use the same queue, discarded on rollback or failed commit. A caught nested exception without a savepoint leaves the queue subject to the final outer outcome. Hooks run outside the closed transaction's ALS context, so a fresh `current()` read does not reuse the old handle and a new transaction has an independent queue. Shutdown also waits for running hooks but does not allow late registration. If any hooks fail, after all hooks finish `AfterCommitError` reports `committed: true`, FIFO `results` for every outcome, and `errors` for every failure. Do not roll back or rerun the already-committed price change because of that error.

`AfterCommitCallback`, `TransactionBoundaryOptions`, `AfterCommitCapabilityError`, and `AfterCommitError` are root exports of each package. Package READMEs own the detailed APIs, and [Transaction Context](../../docs/architecture/transactions.md) owns shared semantics. This is an execution-order contract for an in-process owning boundary, not an outbox, crash recovery, or network exactly-once delivery. Deleting a Redis cache from a hook provides neither DB+Redis atomicity nor prevention of refills by loaders in other processes. The existing `PriceStore` experiment compares only database changes; do not read it as passing these hook checks too.
...
- [Prisma after-commit verification target](../../packages/prisma/src/after-commit.test.ts), [Drizzle after-commit verification target](../../packages/drizzle/src/after-commit.test.ts), [shared behavioral matrix verification target](../../tooling/governance/after-commit-contract.test.ts)
