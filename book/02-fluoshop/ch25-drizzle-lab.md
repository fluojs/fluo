...
Both packages use `transaction(fn, nativeOptions?, boundary?)` for manual calls and `requestTransaction(fn, signal?, nativeOptions?, boundary?)` for request calls. Add `{ requireAfterCommit: true }` in the final boundary position without moving existing options or signals. Distinguish Prisma's `@Transaction(input?, boundary?)` from Drizzle's `@Transaction(accessorOrOptions?, nativeOptions?, boundary?)`. Omitted native options and fail-open contracts remain unchanged. Opt-in rejects with `AfterCommitCapabilityError` before invoking the callback when native commit observation is unavailable. Even without opt-in, registration on an unsupported boundary, outside a boundary, or in a closed scope is rejected.
