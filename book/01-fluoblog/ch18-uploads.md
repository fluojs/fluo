...
Transport limits must apply before the controller. The following is a **partial configuration** to merge into the one options object of `FastifyHttpApplicationAdapter.create({ ... })` in the existing Node24 `src/main.ts`. If the same keys already exist, replace their values with those below; do not create another adapter. Retain the existing `port: blogConfig.PORT`, host, `createConsoleApplicationLogger()`, `shutdownRegistration: createNodeShutdownSignalRegistration()`, and the dynamic `AppModule` import after `ensureMetadataSymbol()`. Do not replace it with a new bootstrap that hardcodes port 3000. In the static factory, `multipart` is also a field in that same options object.
