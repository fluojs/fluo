...
- **Safety Defaults**: TCP closes newline-delimited frames above 1 MiB, `port: 0` routes outbound `send()`/`emit()` through the OS-assigned listener, and new `send()`/`emit()` calls are rejected after `close()` starts. gRPC removes each `AbortSignal` abort listener after a unary call settles and, for streaming calls, only once their reader has started and then ends, errors, or returns early; terminal events before reader consumption are outside this cleanup guarantee.
