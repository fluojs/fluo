...
- A Node.js `>=24.0.0 <27` application runtime, matching the `@fluojs/queue` package-owned support contract.
...
`@fluojs/queue` declares `engines.node >=24.0.0 <27` as its package-owned support contract. Upgrade deployments on Node.js versions below 24 and Node.js 27+ before adopting this major Queue release.
