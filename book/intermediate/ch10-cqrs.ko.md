...
이렇게 하면 entrypoint가 간결하게 유지됩니다. 이전 fluo 패키지들과 마찬가지로 lifecycle과 discovery는 각 feature에서 bus를 수동 조립하는 대신 module registration을 통해 이뤄집니다. Command와 query ownership은 class 이름만이 아니라 provider boundary에서도 검사됩니다. 따라서 같은 handler class를 가리키는 singleton provider alias가 둘 있어도 중복 command 또는 query handler로 간주됩니다. Event handler는 다릅니다. 일치하는 각 singleton provider는 같은 handler class alias를 포함해 discovery 순서대로 fan-out됩니다.
...
CQRS event bus에는 lifecycle contract도 있습니다. `publishAll(...)`은 다음 event로 넘어가기 전에 각 event의 CQRS pipeline을 기다리므로 입력 순서를 보존합니다. 애플리케이션 shutdown 중에는 진행 중인 `publish(...)` pipeline과 `publishAll(...)` sequence가 drain된 뒤 CQRS event bus가 `stopped` 상태에 도달합니다. Shutdown이 시작된 뒤 새 CQRS publish는 lifecycle warning과 함께 무시되고, saga shutdown이 완료된 뒤 직접 saga dispatch도 무시되므로 stopped application이 local handler나 saga를 다시 실행하지 않습니다. 애플리케이션이 위임 event-bus 발행을 `waitForHandlers: false`로 설정했다면, 이 drain은 위임 발행 호출이 resolve되었음을 의미할 뿐이며 `@OnEvent(...)` subscriber는 event-bus boundary 뒤에서 계속 실행 중일 수 있습니다.
...
- CQRS shutdown은 진행 중인 publish pipeline을 drain하고 shutdown 시작 후 새 publish를 무시하지만, `waitForHandlers: false`에서는 위임된 `@OnEvent(...)` subscriber 완료가 이 보장 밖에 있습니다.
