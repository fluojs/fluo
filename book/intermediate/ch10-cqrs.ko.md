...
같은 command를 두 handler가 처리할 수 있다면 write model은 비결정적이 됩니다. business invariant에는 허용하기 어려운 구조입니다. FluoShop은 command를 사용할 때 한 가지 질문에 명확히 답해야 합니다. 누가 이 상태 변화를 실제로 일으킬 책임을 지는가. CQRS는 하나의 handler로 답합니다. 이 ownership 규칙은 provider registration을 기준으로 하므로, 같은 command handler class를 서로 다른 DI token으로 등록해도 중복 command handler로 처리됩니다.
...
- 일치하는 `@EventHandler(...)` provider는 fan-out됩니다. 중복 event handler는 중복 command/query handler와 달리 유효하며, 서로 다른 DI token에 등록된 같은 class의 event-handler provider도 독립적으로 실행됩니다.
...
- CQRS shutdown은 진행 중인 publish pipeline을 drain하고 shutdown 완료 후 새 dispatch/publication을 거부하지만, `waitForHandlers: false`에서는 위임된 `@OnEvent(...)` subscriber 완료가 이 보장 밖에 있습니다.
