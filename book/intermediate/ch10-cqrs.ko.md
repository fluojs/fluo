...
다음 command나 nested event가 같은 in-process pipeline을 이어갈 때는 `CqrsDispatchContext`를 변경하지 말고 그대로 전달하세요. 이 값은 frozen fieldless value이며 fluo가 cycle, depth, shutdown-drain state를 비공개로 유지합니다. 애플리케이션 코드는 이를 직접 생성, 복제, 검사, mutate하지 않아야 합니다.
...
- 일치하는 `@EventHandler(...)` provider와 saga는 singleton provider token 기준으로 fan-out됩니다. 같은 decorated class를 사용해도 서로 다른 token은 별도 route로 유지됩니다.
