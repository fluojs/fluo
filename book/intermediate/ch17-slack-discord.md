...
Slack registration is global by default: `SlackModule.forRoot(...)` and `SlackModule.forRootAsync(...)` export `SlackService`, `SlackChannel`, and `SLACK_CHANNEL` with `global: options.global ?? true`. Use the fluo option `global?: boolean`—not NestJS `isGlobal`—and set `global: false` only when the migrated module must keep Slack providers local to modules that explicitly import it. Inject `SlackService` for direct delivery and `SLACK_CHANNEL` only when registering a notifications channel. If FluoShop grows multiple Slack clients, compose app-owned modules/providers around separate transports instead of expecting a package-level multi-client registry.

Async registration supports the fluo injected factory shape only: `SlackModule.forRootAsync({ inject, useFactory, global? })`. It consumes `inject` and `useFactory`, not NestJS `imports`, `useClass`, or `useExisting`. Register each dependency in FluoShop's application module graph first, list its token in `inject`, and return the final Slack options from `useFactory`.
