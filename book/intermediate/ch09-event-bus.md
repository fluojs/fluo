...
Outcomes list matching effective local handlers in discovery order, with per-publication `index`, `moduleName`, `targetName`, and `methodName`, then outbound transport in channel order. Results omit raw errors and handler return values; bus logs for the new publication path also omit raw handler/transport errors while retaining existing safe target/status messages. App-owned logs remain the application's responsibility. Remote handlers/subscribers are not included, and adapter success with zero subscribers is still transport success. This observation proves neither durability nor required handler registration.
...
- Event-bus requires Node.js `>=24.0.0 <27`; `waitForHandlers: false` resolves after it schedules shutdown-tracked background work.
