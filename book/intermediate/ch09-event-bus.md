...
`publish(...)` completion is a dispatch completion boundary, not evidence that every listener succeeded. When a matching local listener throws or rejects, the event bus logs and isolates the failure, continues the other matching listeners, and does not reject `publish(...)` because of that listener failure alone. Inbound transport callbacks apply the same logging and isolation rule, so callback completion does not surface isolated listener failures. This listener-only isolation does not swallow timeout, cancellation, transport publication, bootstrap, or other unrelated publisher failures.
...
- Local and inbound transport listener failures are logged and isolated, so other matching listeners continue; publisher or inbound callback completion neither proves every listener succeeded nor surfaces those isolated failures. This listener-only rule does not swallow unrelated timeout, cancellation, transport publication, or bootstrap failures.
