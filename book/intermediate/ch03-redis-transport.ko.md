...
Subscriber를 어떻게 만드는지에 따라 소유권 경계가 달라집니다. `publisher.duplicate()`로 만든 subscriber는 `RedisModule`이 자동으로 추적하지 않습니다. Raw publisher와 subscriber를 만든 애플리케이션이 직접 연결하고 transport에 전달하며 shutdown 경로에서 `closeNotificationTransport()`를 호출해야 합니다. Cleanup loop는 종료 순서를 지키면서 모든 정리 단계를 시도하고, 하나라도 실패하면 `AggregateError`를 던집니다. Transport를 닫아도 subscriber 구독만 해제되므로 애플리케이션이 두 raw client도 닫아야 합니다. Command client와 subscriber 모두에 fluo의 Redis lifecycle option을 적용하고 싶다면 named client를 등록하고 `getRedisClientToken(name)`으로 주입하세요:
...
fluo는 모니터링에 필요한 훅을 제공할 수 있지만, 경고 체계와 런북은 팀이 직접 준비해야 합니다. Redis는 많은 브로커보다 가볍습니다. 그렇다고 유지보수가 필요 없다는 뜻은 아닙니다.

## 3.6 Pub/Sub과 Streams 중 선택하기

Pub/Sub과 Streams 중 무엇을 쓸지는 더 화려해 보이는 기능을 고르는 문제가 아닙니다. 이벤트가 구독자 부재와 프로세스 실패를 견뎌야 하는지 여부를 묻는 문제입니다.
...
더 깊은 교훈은 아키텍처에 있습니다. Redis가 TCP를 모든 곳에서 대체하는 것은 아닙니다. 비즈니스가 지연된 완료, 재생, 느슨한 결합으로 이득을 보는 연결만 Redis가 맡는 것입니다. 분산 시스템은 각 연결이 자신의 실패 예산에 맞는 트랜스포트를 사용할 때 더 좋아집니다.

## 3.9 다음 파트 미리 보기

다음 파트에서는 RabbitMQ와 Kafka 같은 브로커를 살펴보며 더 무거운 메시징 요구 사항을 다룹니다. 그 트랜스포트들은 여기서 소개한 개념 위에 쌓입니다. 그 시점이 되면 FluoShop에는 이미 동기 요청 경로 하나와 durable event 경로 하나가 존재하게 됩니다. 그 대비가 있어야 더 무거운 브로커가 언제 정당화되고 언제 불필요한 복잡성인지 판단하기 쉬워집니다.
