...
Subscriber를 어떻게 만드는지에 따라 소유권 경계가 달라집니다. `publisher.duplicate()`로 만든 subscriber는 `RedisModule`이 자동으로 추적하지 않습니다. Raw publisher와 subscriber를 만든 애플리케이션이 직접 연결하고 transport에 전달하며 shutdown 경로에서 `closeNotificationTransport()`를 호출해야 합니다. Transport를 닫아도 subscriber 구독만 해제되므로 애플리케이션이 두 raw client도 닫아야 합니다. Command client와 subscriber 모두에 fluo의 Redis lifecycle option을 적용하고 싶다면 named client를 등록하고 `getRedisClientToken(name)`으로 주입하세요:
