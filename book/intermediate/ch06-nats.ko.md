...
완료 경계와 caller-owned collaborator shutdown을 포함한 패키지 수준 NATS transport 계약은 [`packages/microservices/README.ko.md`](../../packages/microservices/README.ko.md)에 정식으로 기록되어 있습니다.
...
`MicroservicesModule`, `MICROSERVICE`, `Microservice` 타입은 root `@fluojs/microservices`에 유지하고, 위 adapter import는 `@fluojs/microservices/nats`를 사용합니다. `MICROSERVICE`는 raw `NatsMicroserviceTransport`가 아니라 programmatic lifecycle facade로 resolve됩니다.

- `await microservice.send(...)`는 NATS request-reply가 상관관계가 유지된 응답을 반환할 때 settle하며, 오류, abort, timeout, shutdown 시 reject합니다.
