...
fluo의 Cloudflare 어댑터는 Worker `env` 객체를 `fetch` 진입점 경계로 전달하지만, package configuration에 직접 읽거나 병합하지는 않습니다. 설정 소유권은 애플리케이션에 두고, 필요한 Worker binding을 명시적 provider 또는 bootstrap 시점의 `@fluojs/config`로 매핑하세요. 이렇게 하면 서비스 코드가 Cloudflare 전역 객체를 직접 읽지 않아도 됩니다.
...
- Worker `env` 객체의 변수와 바인딩은 application boundary에서 매핑한 뒤 `ConfigService` 같은 provider를 통해 읽으세요.
