...
// bootstrap helper가 routing 및 middleware option을 적용하고 서버 소켓 없이
// dispatcher를 binding하며, 같은 Isolate 안에서 동일한 애플리케이션을 재사용합니다.
...
이 구조는 adapter가 소유하는 transport option과 bootstrap이 소유하는 routing 및 middleware option을 분리하면서, 의존성 주입 컨테이너 부트스트랩을 요청마다 반복하지 않고 같은 Isolate 안에서 재사용할 수 있게 합니다. 엣지에서는 이런 초기화 경계가 응답 시간과 비용에 직접 영향을 줍니다.
...
Worker host는 exported `fetch` handler에 대해 `worker.close()`를 호출하지 않으며 그 위치에 shutdown callback도 제공하지 않습니다. NestJS shutdown hook을 마이그레이션할 때는 application-owned explicit close trigger를 선택하고 그 trigger에서 `await worker.close()`를 호출하세요. 성공한 lazy-entrypoint close는 terminal이 아니라 재시작 가능합니다. 다음 `worker.fetch(...)`는 isolate 안에서 새 application을 bootstrap하여 bootstrap lifecycle hook을 다시 실행하고 application singleton provider를 다시 생성합니다.
