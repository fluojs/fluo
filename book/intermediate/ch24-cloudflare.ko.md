...
또 하나의 핵심은 실행 컨텍스트입니다. Workers에서는 `ctx.waitUntil()`을 사용해 응답 이후의 비동기 작업을 런타임에 등록할 수 있습니다. fluo 어댑터는 `Response` 객체가 반환된 뒤에도 이어질 수 있는 streaming response를 포함해 요청 lifecycle tracking에 이 경계를 사용하며, 분석 데이터 전송이나 웹훅 트리거처럼 요청 응답과 분리된 작업에서 중요합니다.
