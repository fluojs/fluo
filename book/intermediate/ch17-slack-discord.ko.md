...
Slack registration은 기본적으로 global입니다. `SlackModule.forRoot(...)`와 `SlackModule.forRootAsync(...)`는 `global: options.global ?? true`로 `SlackService`, `SlackChannel`, `SLACK_CHANNEL`을 export합니다. fluo 옵션은 NestJS `isGlobal`이 아니라 `global?: boolean`이며, migrated module이 Slack provider를 명시적으로 import한 module 안에만 유지해야 할 때만 `global: false`를 설정합니다. 직접 전달에는 `SlackService`를 주입하고 notifications 채널 등록에만 `SLACK_CHANNEL`을 사용하세요. FluoShop에 여러 Slack client가 필요해지면 package-level multi-client registry를 기대하지 말고 별도 transport를 감싸는 app-owned module/provider를 조합하세요.
