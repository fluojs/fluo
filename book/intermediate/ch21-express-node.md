...
The important point here is that binding Decorators such as `@FromBody()`, `@FromPath()`, and `@FromQuery()` work through the same contract whether Fastify or Express handles the request. fluo's internal Dispatcher handles translation between the adapter's native request format and the standard fluo context. DTO validation still follows the `@fluojs/validation` contract: request binding code materializes incoming payloads with `DefaultValidator.materialize(...)` before business logic sees a typed DTO, rather than installing a Nest-style global `ValidationPipe`.
