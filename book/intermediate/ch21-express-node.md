...
- Node.js 20.19.3 or newer for both `@fluojs/platform-express` and `@fluojs/platform-nodejs`.
...
The Express adapter targets Node.js 20.19.3 or newer and its package manifest declares `engines.node >=20.19.3` so listener-level RFC `QUERY` reaches Express and fluo dispatch.
...
`@fluojs/platform-nodejs` targets Node.js 20.19.3 or newer. Its package manifest declares `engines.node >=20.19.3` so listener-level RFC `QUERY` reaches fluo dispatch; use one of the fetch-style runtime adapters instead when the host is Bun, Deno, or Cloudflare Workers.
