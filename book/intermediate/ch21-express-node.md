...
- Node.js `>=24.0.0 <27` for both `@fluojs/platform-express` and `@fluojs/platform-nodejs`.
...
The Express adapter targets Node.js `>=24.0.0 <27` and its package manifest declares that exact `engines.node` range so listener-level RFC `QUERY` reaches Express and fluo dispatch. Node versions below 24 and Node 27+ are excluded.
...
`@fluojs/platform-nodejs` targets Node.js `>=24.0.0 <27`. Its package manifest declares that exact `engines.node` range so listener-level RFC `QUERY` reaches fluo dispatch; use one of the fetch-style runtime adapters instead when the host is Bun, Deno, or Cloudflare Workers.
