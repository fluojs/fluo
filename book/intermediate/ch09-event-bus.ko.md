...
Redis transport를 import하기 전에 event-bus package와 optional `ioredis` peer를 함께 설치합니다.
...
이 경계는 중요합니다. 이벤트 버스 API는 그대로 유지되고, 뒤에 있는 transport만 바뀝니다. 예제는 transport 전용 publisher와 subscriber를 서로 다른 client로 생성합니다. Redis는 구독 연결을 Pub/Sub mode로 전환하므로 `subscribeClient`로 publish나 일반 command를 실행하지 않아야 합니다. 이 연속성은 앞선 장에서 본 fluo의 설계 철학과 맞닿아 있습니다.
...
`publish(...)` completion은 모든 listener가 성공했다는 증거가 아니라 dispatch completion boundary입니다. 일치하는 local listener가 throw하거나 reject하면 event bus는 해당 실패를 log하고 격리하며, 다른 matching listener를 계속 실행하고 그 listener 실패만으로 `publish(...)`를 reject하지 않습니다. Inbound transport callback에도 같은 logging 및 isolation 규칙이 적용되므로 callback completion은 격리된 listener 실패를 외부로 드러내지 않습니다. 이 listener-only isolation은 timeout, cancellation, transport publication, bootstrap 또는 그 밖의 무관한 publisher 실패를 삼키지 않습니다.
...
Event handler가 숨겨진 worker가 되어서는 안 됩니다. 빠른 local reaction은 `@OnEvent(...)` 안에서 처리해도 괜찮지만, 느리거나 retry 가능한 작업에는 더 강한 운영 경계가 필요합니다. FluoShop은 invoice generation, marketplace catalog sync, 기타 failure-prone follow-up task를 event reaction에서 `@fluojs/queue`로 넘깁니다. Event는 여전히 그 작업이 왜 필요한지 설명하고, queue는 retry, backoff, workload isolation, dead-letter evidence를 소유합니다. 이 handoff는 애플리케이션이 소유한 unique claim store를 사용하고, Queue가 job을 받은 뒤에만 해당 claim을 enqueued로 표시하며, enqueue가 실패하면 claim을 해제해 다음 duplicate delivery가 다시 시도할 수 있게 합니다. 이 예제에서 `this.reactions`는 FluoShop 애플리케이션 코드이며, `@fluojs/event-bus`나 `@fluojs/queue` API가 아닙니다.
...
- Local 및 inbound transport listener 실패는 log되고 격리되므로 다른 matching listener는 계속 실행됩니다. Publisher 또는 inbound callback completion은 모든 listener가 성공했음을 증명하지도, 격리된 실패를 외부로 드러내지도 않습니다. 이 listener-only 규칙은 무관한 timeout, cancellation, transport publication 또는 bootstrap 실패를 삼키지 않습니다.
