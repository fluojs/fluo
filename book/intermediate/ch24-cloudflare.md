...
The Worker host does not invoke `worker.close()` for the exported `fetch` handler: it provides no shutdown callback there. When migrating NestJS shutdown hooks, choose an application-owned explicit close trigger and call `await worker.close()` from it. A successful lazy-entrypoint close is restartable, not terminal. The next `worker.fetch(...)` bootstraps a fresh application in the isolate, rerunning bootstrap lifecycle hooks and reconstructing application singleton providers.
