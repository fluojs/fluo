...
- Use `CloudflareWorkerApplicationHost.create({ fromEnv })` when first-fetch bindings must become bootstrap configuration: its factory runs once per isolate, and every application generation uses that first environment's cached module and options. Otherwise read and narrow request-varying bindings from `context.request.cloudflare?.env` at the application request boundary, then pass only application-shaped values into provider methods.
