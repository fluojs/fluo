...
- `@fluojs/platform-express`와 `@fluojs/platform-nodejs` 모두 Node.js `>=20.19.3 <21 || >=22.2.0 <27`.
...
Express adapter는 Node.js `>=20.19.3 <21 || >=22.2.0 <27`을 대상으로 하며 listener-level RFC `QUERY`가 Express와 fluo dispatch에 도달하도록 package manifest는 이 정확한 `engines.node` 범위를 선언합니다. Node 21, Node 22.2.0 미만, 검증되지 않은 Node 27 이상은 제외됩니다.
...
`@fluojs/platform-nodejs`는 Node.js `>=20.19.3 <21 || >=22.2.0 <27`을 대상으로 합니다. Listener-level RFC `QUERY`가 fluo dispatch에 도달하도록 package manifest는 이 정확한 `engines.node` 범위를 선언합니다. Host가 Bun, Deno, Cloudflare Workers라면 대신 fetch-style runtime adapter 중 하나를 사용하세요.
...
- Startup 뒤 해석된 bind target에는 `getListenTarget()`을 사용하고, platform-specific 기능이 필요한 infrastructure boundary에서만 `getServer()` 또는 `getRealtimeCapability()`를 사용하세요.
