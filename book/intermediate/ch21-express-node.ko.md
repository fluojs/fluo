...
- `@fluojs/platform-express`와 `@fluojs/platform-nodejs` 모두 Node.js `>=24.0.0 <27`.
...
Express adapter는 Node.js `>=24.0.0 <27`을 대상으로 하며 listener-level RFC `QUERY`가 Express와 fluo dispatch에 도달하도록 package manifest는 이 정확한 `engines.node` 범위를 선언합니다. Node 24 미만과 Node 27 이상은 제외됩니다.
...
`@fluojs/platform-nodejs`는 Node.js `>=24.0.0 <27`을 대상으로 합니다. Listener-level RFC `QUERY`가 fluo dispatch에 도달하도록 package manifest는 이 정확한 `engines.node` 범위를 선언합니다. Host가 Bun, Deno, Cloudflare Workers라면 대신 fetch-style runtime adapter 중 하나를 사용하세요.
