...
회귀 테스트는 세 스트리밍 모드에서 `end()` 완료, `error()` 전파, outbound 취소, `AbortSignal` listener cleanup을 검증합니다. `ServerStreamWriter.write()`는 `void`를 반환하며, `GrpcMicroserviceTransport`는 하부 gRPC `write()` boolean을 버리고 backpressure 또는 drain 계약을 노출하지 않습니다. `write()`를 전달 완료나 flow-control 승인으로 해석해서는 안 됩니다.
...
`ServerStreamWriter`는 의도적으로 generic이 아니므로 payload typing은 protobuf 생성 타입이나 애플리케이션 응답 타입이 담당합니다. 이 모델은 HTTP 폴링을 반복하거나 큐 위에 스트림을 흉내 내는 것보다 자연스럽고 지연 시간이 짧습니다.
