...
Gateway return value는 기본적으로 await된 뒤 무시됩니다. 위 예시처럼 handler에 전달되는 runtime socket으로 reply를 명시적으로 보내거나, `DenoWebSocketModule.forRoot({ replies: { mode: 'event-envelope' } })`를 설정해 serialize된 `{ event, data? }` handler-return reply를 opt-in하세요. Deno upgrade guard는 Web-standard `Request` 값을 받으며, `Deno.upgradeWebSocket(...)`이 socket을 accept하기 전에 `false`, structured `WebSocketUpgradeRejection`, 또는 throw된 HTTP exception으로 거절할 수 있습니다.
