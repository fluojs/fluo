...
RabbitMQ가 창고 작업 할당에 맞는 도구라면, Kafka는 비즈니스 히스토리를 오래 보존하는 쪽에 가깝습니다. 상품 기획자가 플래시 세일 동안 checkout이 얼마나 지연되었는지 묻는 순간, 주문 생명주기 이벤트가 로그로 남아 있는지는 운영 품질의 문제가 됩니다. 구체적인 요구사항은 이렇습니다:
...
FluoShop이 Kafka를 선택하는 이유가 여기에 있습니다. v1.4.0에서는 durable `order-timeline` stream을 추가합니다. Order Service, Payment Service, Fulfillment Service가 모두 이 stream에 milestone을 발행합니다. 그 뒤 별도의 consumer가 analytics와 operations용 projection을 만듭니다. 이 로그가 주문 실체에 대한 **단일 진실 공급원(Single Source of Truth)** 역할을 합니다. 개별 서비스가 각자의 DB를 가지고 있더라도, Kafka 로그는 서비스 경계를 넘어 실제로 어떤 일이 일어났는지 맞춰 보는 공식 감사 추적(audit trail)입니다.
...
기본값은 범용성을 우선합니다. FluoShop에서는 도메인과 운영 의도를 드러내는 이름이 더 낫습니다.

- `fluoshop.timeline.events`
- `fluoshop.domain.messages`
- `fluoshop.responses.<instance>`

응답 topic은 보통 클라이언트 인스턴스별로 분리합니다. 그래야 fluo가 동시 request-reply 흐름에서 응답 충돌을 피할 수 있습니다. 기본적으로 트랜스포트는 UUID 접미사가 붙은 `responseTopic`(예: `fluo.microservices.responses.uuid`)을 생성해 각 인스턴스에 응답 전용 수신 지점을 부여합니다.
...
Kafka는 이벤트 스트림으로 가장 널리 쓰입니다. 그럼에도 fluo는 durable broker routing이 필요한 경우를 위해 Kafka 위에서도 `send()`를 제공합니다. 이 방식은 TCP보다 느리고 비용도 큽니다. 그래도 broker-mediated decoupling이 필요하고 추가 지연을 받아들일 수 있다면 타당한 선택이 됩니다. FluoShop에서는 "주문 감사(Order Audit)" 요청에 이 경로를 사용합니다. 특정 답변이 필요하지만, 대상 서비스가 잠시 재시작 중이어도 요청 자체는 내구성 있게 남아 나중에 처리되어야 하기 때문입니다.
...
핵심은 Kafka가 특별한 핸들러 코드를 요구한다는 뜻이 아닙니다. 실제로 핸들러 모델은 그대로입니다. 중요한 점은 topic이 충분히 긴 시간 동안 history를 보존해, 다른 팀이 나중에 새로운 projection을 만들 수 있다는 데 있습니다. 이 지점이 RabbitMQ와의 전략적 차이입니다.
...
Kafka의 운영상 강점에는 설계 책임이 함께 따라옵니다. ordering은 보통 하나의 partition 내부에서만 보장됩니다. replay는 강력하지만 잘못 설계된 이벤트를 더 넓게 증폭시킬 수도 있습니다. retention은 이벤트가 상태 재구성에 필요한 의미를 충분히 담고 있을 때만 가치가 있습니다.
...
Kafka가 반드시 플랫폼 전체의 중심축이어야 하는 것은 아닙니다. FluoShop은 의도적으로 mixed transport 구성을 유지합니다.
...
이런 분업은 각 transport를 가장 적합한 역할에 배치합니다. 운영 측면에서 팀은 다음을 관찰해야 합니다.
...
이 신호들은 Kafka가 의도한 역할에 맞게 쓰이고 있는지 보여 줍니다. request-reply timeout이 지배적이라면 logged event에 더 적합한 transport에 동기 행위를 억지로 올리고 있을 수 있습니다. replay 비용이 너무 크다면 snapshot 없이 raw history에 과도하게 의존하는 projection 구조일 가능성이 있습니다.
...
RabbitMQ가 작업 할당의 경계를 보여 주었다면, Kafka는 비즈니스 히스토리를 보존하고 재사용하는 경계를 보여 줍니다. 그래서 두 transport는 같은 시스템 안에 함께 존재할 가치가 있습니다. FluoShop에서 v1.3.0에서 v1.4.0으로 넘어가는 변화는 속도보다 **장기적인 책임(long-term accountability)**에 가깝습니다.
