...
- **Safety Defaults**: TCP는 1 MiB를 넘는 newline-delimited frame을 닫고, `port: 0`은 outbound `send()`/`emit()`을 OS가 할당한 listener로 라우팅합니다. `close()`가 시작된 뒤에는 새 `send()`/`emit()` 호출을 거부합니다. gRPC는 unary call이 settle한 뒤 각 `AbortSignal` abort listener를 제거하고, streaming call은 reader가 시작된 뒤 end/error/early return으로 종료될 때만 제거합니다. Reader 소비 전 terminal event는 이 cleanup 보장에 포함되지 않습니다.
