...
일반적인 비즈니스 원자성은 서비스에 두세요. controller 또는 request orchestration boundary에서 전체 request가 하나의 transaction을 공유해야 하면 명시적 `DrizzleDatabase.requestTransaction(...)`을 호출하고 adapter가 제공하는 request `AbortSignal`을 전달하세요. Controller-level `@Transaction()`은 controller가 명시적 `DrizzleDatabase` 대상을 소유하는 경우의 호환성 경로로만 유지됩니다.
...
요청 전체 boundary에는 `requestTransaction(...)`을 명시적으로 호출하세요.
