...
Keep normal business atomicity on services. For a controller or request orchestration boundary where the whole request must share one transaction, call `DrizzleDatabase.requestTransaction(...)` explicitly and pass the request `AbortSignal` when the adapter exposes one. Controller-level `@Transaction()` is only a compatibility path for a controller that owns an explicit `DrizzleDatabase` target.
...
Use `requestTransaction(...)` explicitly for a request-wide boundary:
...
To express expected failure as a return value, opt into `shouldRollback` in the final Fluo boundary, as in the [typed Drizzle example](../../packages/drizzle/README.md#choosing-rollback-from-a-result). Native option positions remain unchanged, and there is no global Result shape. A root failure returns the same value after native rollback and cleanup succeed. A nested failure returns its original value while marking the owner sticky rollback-only; unless the root also rejects its own result, the boundary ends with `TransactionRollbackOnlyError` carrying the first nested failure.

Unsupported boundaries reject before callbacks with `TransactionRollbackCapabilityError`, and native errors propagate. Ordinary caught nested exceptions retain existing commit/hooks, but opted-in rollback discards all hooks and native callback retries use fresh owners. The [shared-owner contract](../../docs/architecture/transactions.md#result-based-rollback) defines the distinction from post-commit errors and limits concerning external raw transactions, Redis `MULTI/EXEC`, savepoints, and durability.
