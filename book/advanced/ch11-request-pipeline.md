...
This diagram shows Guaranteed Cleanup, a core principle of fluo architecture. Each layer is independent, but the Dispatcher ties them together as one flow. Whether the request takes a success path, an expected error path, or an unexpected panic path, the request-scope release stage is designed to run every time a request-scoped container was created. Fast-path requests whose graphs do not require request scope keep using the root container and do not dispose it, while per-dispatch controller resolution still preserves transient provider identity.
...
The current hot-path work is prepared in the dispatcher and binding-plan paths. `createDispatcher(...)` compiles handler execution plans for every `HandlerDescriptor`, stores them in the dispatcher-local `handlerExecutionPlans` `WeakMap`, records fast-path eligibility on each handler, and keeps a dispatcher-local `fastPathRuntimeCache` for method handles used by native fast routes. The active container still resolves the controller for every dispatch, so DI owns singleton reuse and transient controllers or dependencies receive fresh-per-resolution identity instead of becoming dispatcher-cached instances. DTO binding has its own plan cache in `packages/http/src/adapters/dto-binding-plan.ts`, where `getCompiledDtoBindingPlan(...)` stores field readers, bound property keys, converter presence, and validation filtering in a `WeakMap` keyed by DTO constructor. Content negotiation is also precomputed once during dispatcher creation through `resolveContentNegotiation(...)`, which deduplicates formatters, chooses the default formatter, and keeps normalized media types for request-time `Accept` matching. These optimizations keep request-scope-free routes on the root-container fast path while still promoting to isolated request scopes when the pipeline needs request-scoped providers.

## 11.10 Resource Cleanup, DI Scope Disposal

When request handling ends, a promoted request-scoped container must be disposed. This runs `onDispose` hooks for non-singleton objects created during the request, meaning request-scoped providers, and releases memory to prevent leaks. If the request graph did not require request scope and no promotion occurred, the cleanup path leaves the root container alive.
...
This process runs inside a `finally` block, so it always checks cleanup regardless of whether the request succeeds or fails. `packages/http/src/dispatch/dispatcher.test.ts` verifies both sides of the request-scope policy: request-scope-free routes skip request-scope creation, while request-scoped controllers, active middleware, observers, custom binders, DTO converters, and manual container resolution use isolated scopes that are disposed after the dispatch. `packages/http/src/dispatch/dispatcher-fast-path-scope.test.ts` separately verifies that this optimization does not reuse transient controller instances across requests.
