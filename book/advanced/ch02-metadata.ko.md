...
이러한 정책 결정은 `path:packages/core/src/metadata/class-di.ts`의 `mergeClassDiMetadata` 함수 내에 하드코딩되어 있습니다. 이 함수는 들어오는 메타데이터의 각 필드를 순회하며, 스코프와 같은 "단일 값" 필드는 덮어쓰고, 토큰 배열과 같은 "집합 값" 필드는 중복을 제거하면서 합칩니다. 이처럼 명확한 병합 규칙은 상속된 코드를 재사용하면서도 특정 부분만 유연하게 변경하고 싶어 하는 요구를 충족합니다.
