...
정규화 단계는 중요한 안정화 경계이기도 합니다. `path:packages/prisma/src/module.ts:63-76`의 `normalizePrismaModuleOptions()`는 caller가 넘긴 client를 보존하면서 `strictTransactions` 기본값을 명시적인 내부 값으로 바꿉니다. 이런 기본값 고정이 동적 모듈 생명주기의 초기에 일어나면, 뒤의 provider factory들은 partial options 대신 완성된 내부 형태만 다루면 됩니다.

`RedisModule.forRoot()`는 약간 다른 변형을 보여 줍니다. `RedisModuleOptions`는 caller-facing shape입니다. `path:packages/redis/src/module.ts:31-58`은 Fluo 전용 `name`, `global`, `lifecycle`, `sentinelName` field를 일반 `ioredis` option과 분리하고, registration name을 trim한 뒤 lifecycle setting을 검증하고, 빈 name 및 global named registration을 거부한 다음, `sentinelName`을 생성자 `name`으로 매핑합니다. 그 결과인 `RedisClientOptions`는 module input의 다른 이름이 아니라 constructor-facing shape입니다. 그 다음 `path:packages/redis/src/module.ts:81-140`은 raw Redis client, 고수준 `RedisService`, 정규화된 lifecycle timeout control을 받은 lifecycle service를 구성하는 provider 집합을 만듭니다. Provider는 client option을 먼저 spread하고 마지막에 `lazyConnect: true`를 할당하므로, caller가 module input에 `lazyConnect: false`를 강제로 cast해도 애플리케이션 bootstrap이 연결 시점을 소유합니다. 마지막으로 `path:packages/redis/src/module.ts:168-179`은 이 provider set을 제조된 module type에 bind하면서, 기본 등록은 global로 만들고 named registration은 scoped로 유지합니다. 여기서도 module factory의 본질은 provider assembly와 metadata binding의 orchestration 계층입니다. lifecycle service를 함께 등록하기 때문에, 애플리케이션 종료 시 Redis client 연결 해제도 Fluo의 표준 lifecycle hook 안에서 처리됩니다. 즉 동적 모듈은 정적인 객체 등록에 그치지 않고 런타임 lifecycle에도 참여합니다.
...
첫 발췌는 client, facade, lifecycle service가 정규화된 lifecycle timeout option과 함께 같은 provider set으로 묶이는 지점을 보여 줍니다. 두 번째 발췌는 그 provider set이 module metadata에 붙으면서 public token surface가 정규화된 registration name에서 선택되는 과정을 보여 줍니다. 기본 등록은 `REDIS_CLIENT`와 `RedisService`를 export하고, named registration은 helper에서 파생한 named client/service token만 export합니다.

`path:packages/redis/src/module.ts:107-140`의 named branch를 보면 같은 provider factory가 이름이 붙은 client token, service token, lifecycle token을 함께 만들어 냅니다. 이를 동적 모듈 안에 등록하면, 단일 애플리케이션에서 여러 Redis instance가 쓰이더라도 `RedisModule`의 각 instance가 특정 token surface에 바인딩된 service를 만들 수 있습니다. 이 instance 격리가 programmatic module construction의 핵심 이점입니다.
...
- `PrismaModule`은 `path:packages/prisma/src/module.ts:41-139`에 option normalization과 runtime Provider assembly를 둡니다.
- `QueueModule`은 `path:packages/queue/src/module.ts:9-42`에 option normalization과 internal Provider assembly를 둡니다.
- `RedisModule`은 `path:packages/redis/src/module.ts:31-141`에 Redis option normalization과 client/service Provider assembly를 둡니다.
...
이 memoization은 cosmetic이 아닙니다. 이것은 시스템의 정확성을 위한 핵심 기능입니다. 명시적으로 memoize된 promise나 컨테이너가 resolution promise를 공유하는 singleton provider 중 하나가 없다면, options token을 소비하는 downstream consumer마다 별도의 비동기 구성 로드(파일 읽기나 API 호출 등)가 중복해서 일어날 수 있습니다. 이런 공유 전략이 있으면 해당 module registration의 resolution은 애플리케이션 컨테이너마다 정확히 한 번만 일어납니다.

`PrismaModule.forRootAsync()`도 `path:packages/prisma/src/module.ts:171-208`에서 볼 수 있듯이 정규화된 Prisma 옵션에 대해 같은 중앙화 원칙을 사용합니다. 하나의 singleton async options provider를 등록하고, 공개 client/options/service provider를 그 내부 값에서 파생하므로 bootstrap이 이를 resolve한 뒤에는 패키지의 나머지 부분이 해당 옵션을 동기적으로 소비할 수 있습니다.
...
forRootAsync(options):
  1. async options의 공유 경계를 선택합니다. 로컬 cachedResult promise, singleton
     options provider, 또는 컨테이너 밖에서도 명시적 메모이제이션이 필요할 때 둘 다 사용할 수 있습니다.
  2. 그 공유 경계를 통해 사용자의 useFactory를 호출하는 factory function을 정의합니다.
  3. 해당 팩토리와 주입된 의존성을 사용하여 options provider를 등록합니다.
...
`path:packages/email/src/module.ts:74-95`와 `path:packages/prisma/src/module.ts:100-139`를 비교해 보면 이 반복 패턴이 잘 보입니다. 프로바이더 하나가 정규화된 옵션을 실체화(materialize)하고, 다른 프로바이더들이 그 하나의 소스로부터 파생값과 서비스를 팬아웃합니다.

이 반복 패턴은 위의 Prisma와 Email 발췌가 대표합니다. `path:packages/email/src/module.ts:74-95`와 `path:packages/prisma/src/module.ts:100-139`은 같은 구조를 공유하므로, 여기서는 두 파일을 모두 풀어 붙이지 않고 citation으로 남깁니다. 중요한 공통점은 public facade provider가 하나의 내부 options token에서 파생되며, 소비자에게는 문서화된 facade token만 export된다는 점입니다.
...
또한 공유 패턴은 여러 module이 동일한 "설정 공유" module을 import하는 복잡한 graph에서도 비용 큰 비동기 로직이 한 번만 수행되도록 합니다. 명시적 변형에서는 `cachedResult` promise가 동기화 지점 역할을 하고, singleton-provider 변형에서는 DI 컨테이너가 저장한 resolution promise가 모든 dependent에 같은 동기화를 제공합니다. 두 방식 모두 여러 consumer를 하나의 결정론적 초기화 sequence로 모으며, 시작 시점의 race condition을 피해야 하는 운영 환경에서 중요한 성질입니다.
...
`RedisModule`은 좋은 사례 연구입니다. `path:packages/redis/src/module.ts:168-179`은 기본 등록을 global로 만들고 `REDIS_CLIENT`와 `RedisService` token을 export합니다. 반면 `RedisModule.forRoot({ name, ... })`는 사용자가 제공한 `name`에서 `getRedisClientToken(name)`과 `getRedisServiceToken(name)`으로 파생한 특화 token을 내보내는 non-global module을 만듭니다. 여기서 dynamic module은 provider를 만드는 데 그치지 않고, 안정적이고 주소 지정 가능한(addressable) public token surface를 설계합니다.

명명된 Redis 등록은 같은 provider factory를 쓰되, export token을 `name` option에서 파생합니다. 정규화된 lifecycle option도 같은 factory로 전달되므로, 문서화된 Redis `connect()` 및 `quit()` timeout 계약이 기본 클라이언트와 명명된 클라이언트에 동일하게 적용됩니다.
...
이 발췌는 dynamic module이 단순히 provider를 계산하는 데서 끝나지 않는다는 점을 보여 줍니다. 호출자가 넘긴 `name` option이 raw client token과 facade service token을 바꾸고, 그 둘만 export 표면에 올라갑니다. 호출자의 `lifecycle` option은 provider graph 내부에 남아 Fluo가 소유한 Redis connect 및 quit timeout guardrail을 설정하지만, public export token이 되지는 않습니다.
...
이 named registration pattern은 primary/secondary database나 local cache/global session store처럼 동일 인프라의 여러 instance와 통신해야 하는 backend에서 중요합니다. `path:packages/redis/src/tokens.ts:25-65`를 보면 Fluo가 `getRedisClientToken(name)`을 통해 stable named Symbol token을 파생하고, `getRedisServiceToken(name)`이 대응되는 facade-service token을 제공하는 방식을 볼 수 있습니다. 동적 모듈은 `REDIS_CLIENT_PRIMARY` 같은 임의의 constant를 노출하는 대신, 자신의 내부 client/service provider를 이 helper-derived name에 매핑합니다.
...
넷째, 비동기 옵션 팩토리는 애플리케이션 컨테이너마다 한 번만 resolve되게 하십시오. `path:packages/email/src/module.ts:117-136`은 명시적인 로컬 `cachedResult` 패턴을 보여 주고, `path:packages/prisma/src/module.ts:187-200`은 async normalized-options provider를 singleton으로 등록해 downstream provider가 컨테이너를 통해 그 resolved value를 공유하게 합니다. 동일한 once-per-container 계약을 보존한다면 명시적 메모이제이션과 singleton provider 공유는 모두 허용되는 방식입니다. 이런 패턴이 없으면 비동기 `useFactory` 작업이 예기치 않게 반복될 수 있습니다. 이는 동일한 모듈 내에 해당 옵션 토큰에 의존하는 여러 프로바이더가 있을 때 특히 중요합니다.

이 규칙은 위 Email async 발췌가 명시적 memoization 변형을 대표합니다. Prisma의 `path:packages/prisma/src/module.ts:187-200`은 singleton-provider 변형을 대표하므로, 중복 코드 대신 citation으로 같은 목표를 보강합니다.
