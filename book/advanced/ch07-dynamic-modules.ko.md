...
넷째, 비동기 옵션 팩토리는 애플리케이션 컨테이너마다 한 번만 resolve되게 하십시오. `path:packages/email/src/module.ts:117-136`은 명시적인 로컬 `cachedResult` 패턴을 보여 주고, `path:packages/prisma/src/module.ts:187-200`은 async normalized-options provider를 singleton으로 등록해 downstream provider가 그 resolved value를 공유하게 합니다. 이런 패턴이 없으면 비동기 `useFactory` 작업이 예기치 않게 반복될 수 있습니다. 이는 동일한 모듈 내에 해당 옵션 토큰에 의존하는 여러 프로바이더가 있을 때 특히 중요합니다.
