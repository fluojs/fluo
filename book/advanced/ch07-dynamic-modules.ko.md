...
`RedisModule.forRoot()`는 약간 다른 변형을 보여 줍니다. `path:packages/redis/src/module.ts:31-53`은 caller option에서 module-only `name`, `global`, `lifecycle` field를 `ioredis` client option과 분리해 정규화합니다. 그 다음 `path:packages/redis/src/module.ts:56-116`은 raw Redis client, 고수준 `RedisService`, 정규화된 lifecycle timeout control을 받은 lifecycle service를 구성하는 provider 집합을 만듭니다. 마지막으로 `path:packages/redis/src/module.ts:143-153`은 이 provider set을 제조된 module type에 bind하면서, 기본 등록은 global로 만들고 named registration은 scoped로 유지합니다. 여기서도 module factory의 본질은 provider assembly와 metadata binding의 orchestration 계층입니다. lifecycle service를 함께 등록하기 때문에, 애플리케이션 종료 시 Redis client 연결 해제도 Fluo의 표준 lifecycle hook 안에서 처리됩니다. 즉 동적 모듈은 정적인 객체 등록에 그치지 않고 런타임 lifecycle에도 참여합니다.
