...
- `fluo dev --studio`를 Node dev-runner live sidecar workflow로, `fluo inspect`를 static/report fallback workflow로 사용하는 방식을 구분합니다.
...
`packages/studio/README.md`에 정의된 inspection 및 Studio 흐름은 네 가지 주요 계층으로 구성됩니다.
...
Mermaid 렌더링은 `renderMermaid(snapshot)` 계약을 통해 `@fluojs/studio`에 위임됩니다. 이 출력이 필요하면 CLI Studio/viewer 명령을 실행하는 Node.js `>=24.0.0 <27` 프로젝트에 Studio를 개발 전용 의존성으로 설치합니다. Studio는 이 artifact의 runtime-neutral consumer-side declaration을 소유하고 `@fluojs/runtime`에 의존하지 않습니다. Studio의 Node.js 지원 범위는 자체 package manifest가 선언합니다.
