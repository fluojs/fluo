...
`IntersectionType(A, B)`는 두 DTO contract를 결합합니다. Mapped-type helper는 `@fluojs/validation/mapped-types` subpath에서만 제공되며 root import alias는 없습니다.
...
| DTO composition | `@fluojs/validation/mapped-types`의 `PickType`, `OmitType`, `PartialType`, `IntersectionType` |
