...
```ts
import { OmitType, PartialType, PickType } from '@fluojs/validation/mapped-types';
import { CreatePostDto } from './create-post.dto';
...
`IntersectionType(A, B)` combines two DTO contracts. The mapped-type helpers are available only from the `@fluojs/validation/mapped-types` subpath; root imports have no aliases.
...
| DTO composition | `PickType`, `OmitType`, `PartialType`, `IntersectionType` from `@fluojs/validation/mapped-types` |
...
- Keep examples and docs aligned with [Testing](/docs/guides/testing) and the runnable examples under [Examples](/docs/examples).
...
- [Package Selection](/docs/guides/package-selection)
- [Package Reference](/docs/packages)
- [RealWorld API Example](/docs/examples/realworld-api)
