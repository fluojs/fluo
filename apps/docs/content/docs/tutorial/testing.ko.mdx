...
`../src/main`이 아니라 **`../src/app`**을 import하세요. `main.ts`는 Fastify 리스너를 엽니다. 반면 `createTestApp({ rootModule: AppModule })`은 실제 모듈 그래프를 컴파일하고 수신 포트 없이 HTTP 파이프라인을 실행합니다. 이 테스트에는 실행 중인 서버가 필요하지 않습니다.
