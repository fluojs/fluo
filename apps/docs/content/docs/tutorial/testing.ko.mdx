...
예제의 기존 `vitest.config.ts`는 `fluoDecoratorsPlugin({ sourceMaps: true, transformBoundary: 'test' })`를 사용하고 `@fluojs/core/metadata-preload`를 preload하며 source와 test 디렉터리를 모두 포함합니다. 이 설정을 유지하며 기존 방식의 데코레이터 플래그를 켜지 않습니다.
...
각 테스트는 자신의 앱을 만들고 요청 완료를 기다린 뒤 assertion이 실패해도 `withCleanup`에 등록한 정리를 실행합니다. 다른 테스트의 POST나 고정된 대기 시간에 의존하지 않습니다. 마지막 테스트는 두 애플리케이션을 의도적으로 생성해 메모리를 공유하지 않는다는 것을 확인합니다.
