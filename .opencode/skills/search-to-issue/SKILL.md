...
이 스킬은 **fluo 저장소의 실제 패키지 구조, 공개 surface 문서, behavioral contract 정책, GitHub label 체계**에 맞춘 repo-local 감사 스킬이다.
...
3. behavioral contract 판단은 `docs/operations/behavioral-contract-policy.md`를 최우선 지침으로 삼는다.
4. GitHub label은 실제 저장소에 존재하는 라벨만 사용한다.
5. 기본 issue 묶음 단위는 **패키지당 1개**다. 다만 같은 root cause와 같은 fix가 여러 패키지에 진짜로 공통이면 예외적으로 cross-package issue를 허용할 수 있다.
...
범위 해석은 반드시 아래 순서를 따른다. **한 번 scope가 결정되면 거기서 멈춘다.**
...
1. 감사 전에 반드시 다음을 읽는다.
   ...
   - platform/runtime 계열이면 `docs/operations/platform-conformance-authoring-checklist.md`
   - public API/docs 계열이면 `docs/operations/public-export-tsdoc-baseline.md`
...
이슈 발굴 및 생성 시, 저장소의 `docs/operations/behavioral-contract-policy.md`를 최우선 지침으로 삼는다.
