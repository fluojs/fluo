...
5. **release intent record는 장기 canonical machine input**
   - root `CHANGELOG.md`는 사람용 narrative이고, `tooling/release/intents/*.json`은 릴리스 준비/리뷰용 machine input이다.
   - `1.0.0-beta.2` 이상 후보 릴리스는 release intent record가 필요하다. `1.0.0-beta.1` 이하는 legacy-compatible로 취급한다.
   - intent entry는 대상 package, disposition(`release` / `no-release` / `downstream-evaluate`), semver(`major` / `minor` / `patch` / `none`), summary, rationale, 필요 시 migrationNote를 포함해야 한다.
   - `downstream-evaluate`는 자동 downstream publish가 아니라 명시적 human review decision이다.
   - Changesets, Beachball, 또는 다른 release automation dependency는 현재 승인된 경로가 아니다.
...
5. `1.0.0-beta.2` 이상이면 대응되는 `tooling/release/intents/*.json` 기록이 있는지 확인한다.
6. 이미 같은 버전이 npm에 publish 되었는지 확인한다.
7. 열린 release PR 중 같은 패키지/버전이 있는지 확인한다.
...
6. `1.0.0-beta.2` 이상 후보라면 `tooling/release/intents/*.json`에 release intent record를 추가/갱신한다.
7. canonical verifier 실행:
   - `pnpm verify:release-readiness --target-package ... --target-version ... --dist-tag ...`
   - release intent file이 필요한 후보라면 `--release-intent-file <path>`도 함께 전달한다.
8. 필요시 관련 docs/gate 통과 확인
...
- `1.0.0-beta.2` 이상 package release notes는 `### @fluojs/<package>` package subsection을 포함해야 한다.
...
- `tooling/release/intents/*.json`은 package별 release/no-release/downstream-evaluate 판단의 source of truth로 취급한다.
...
- `1.0.0-beta.2` 이상 릴리스에서는 release intent record를 누락하지 않는다.
- release intent 없이 package version / CHANGELOG만 올린 상태를 릴리스 준비 완료로 판단하지 않는다.
...
- `1.0.0-beta.2` 이상에서 release intent record 없이 publish 준비를 완료했다고 판단하지 않는다.
