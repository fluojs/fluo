...
5. 패키지 릴리스 준비 PR은 root `CHANGELOG.md`, `tooling/release/intents/*.json`, 대상 `package.json` version, release-readiness evidence가 함께 정합해야 한다.
6. `1.0.0-beta.2` 이상 후보 릴리스에서 release intent record가 없으면 기본 verdict는 `block`이다.
7. Changesets/Beachball 기반 version PR, local publish path, multi-package publish widening을 도입하는 PR은 현재 release contract 위반으로 기본 verdict를 `block`으로 둔다.
...
- 담당 범위:
  ...
  - release PR이면 `docs/contracts/release-governance.md`, `tooling/release/intents/README.md`, root `CHANGELOG.md`
...
- `needs-human-check`
  ...
  - release-intent disposition이 downstream impact를 충분히 설명하지만 human release manager 판단이 필요한 경우
  - Changesets/Beachball 또는 release automation migration 제안이지만 아직 구현 PR이 아니라 정책/RFC 판단만 필요한 경우
...
- `1.0.0-beta.2` 이상 release PR에서 release intent record, package-scoped changelog notes, target package version, release-readiness preflight evidence 중 하나라도 빠졌으면 기본 verdict는 `block`이다.
- Changesets/Beachball, local `npm publish`, 또는 single-package CI-only boundary를 넓히는 구현 PR은 `docs/contracts/release-governance.md`가 승인 기준을 바꾸기 전까지 기본 verdict는 `block`이다.
