...
- Use provider-qualified model IDs in agent frontmatter. The current Ollama Cloud models are `ollama-cloud/glm-5.2` and `ollama-cloud/glm-5.1`.
- Select `ollama-cloud/glm-5.2` for central or high-stakes gates and implementation agents.
- Select `ollama-cloud/glm-5.1` for specialist package audits and reviews.
- Before pinning a model, verify it appears in the `ollama-cloud` provider catalog.
