...
argument-hint: "<문제 설명 | issue 목록 | run-id> [plan|register|execute|resume] [base-branch]"
---
...
```
/lane-supervisor <문제 설명 | issue 목록 | run-id> [plan|register|execute|resume] [base-branch]
```
...
```json
{
  ...
  "mode": "plan|register|execute|resume",
  "base_branch": "main",
  "source_mode": "search-to-issue|existing-issues",
  "merge_policy": "developer-final|supervisor-auto|supervisor-with-human-escalation",
...
`resume`는 ledger의 mode, source_mode, merge_policy, lane status, retry_count, PR/worktree mapping이 현재 repository/GitHub 상태와 충돌하지 않는지 먼저 확인한다. 모순되면 `needs-human-check`로 멈춘다.
...
명시 권한이 없으면 cleanup status를 `skipped — authority required`로 남긴다.

### 14. Main sync

root worktree가 clean이면 `git pull --ff-only origin <base-branch>`를 고려한다. dirty이면 sync를 중단하고 `root_main_sync.status = blocked-dirty`로 기록한다.

### 15. Continue / finish

각 lane에서 다음 unlocked issue를 선택한다. 더 이상 실행할 issue가 없으면 lane을 `done` 처리한다. 모든 lane이 `done`, `blocked`, 또는 `needs-human-check` terminal state이면 최종 보고를 작성한다.
...
merge policy: developer-final | supervisor-auto | supervisor-with-human-escalation
...
- retry_count 3회 이상에서 자동 fix-back을 계속하지 않는다.
