...
이 커맨드는 `/create-lane`이 생성한 `.sisyphus/lanes/<lane-id>.json`을 실행하는 **execution harness**다. source discovery나 lane planning을 다시 하지 않고, ledger에 고정된 lane head issue만 순서대로 `/issue-to-pr`, `/pr-to-merge`에 위임한다.
...
- `/execute-lane .sisyphus/lanes/lane-2026-06-01-runtime-a.json resume main`
...
1. **ledger 로드/검증** — lane id 또는 path를 해석하고 `.sisyphus/lanes/<lane-id>.json`의 schema, base branch, authority scope, lane 상태를 확인한다.
...
- ledger path가 `.sisyphus/lanes/` 아래이거나 명시된 lane ledger path다.
...
- `authority_scope`가 side effect별로 명확하다.
...
5. local/remote branch cleanup은 ledger authority와 command-owned 확인이 있을 때만 수행한다.
6. root worktree가 dirty이면 `git pull --ff-only origin <base-branch>`를 시도하지 않고 `root_main_sync.status = blocked-dirty`로 기록한다.
