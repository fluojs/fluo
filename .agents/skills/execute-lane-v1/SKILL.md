---
name: execute-lane-v1
description: "SUPERSEDED by execute-lane (v4). Retained for reference only; do not invoke for new lanes. Historical v1 DAG-based lane execution: independent issue DAGs, sentinel-enveloped preflight workers, exact-head review, resumable evidence. Known defect: the preflight prompt contract in issue-dag-prompts.mjs and the envelope verifier in issue-dag-worker-runtime.mjs disagree on the required response shape, so a compliant worker is always rejected."
---
