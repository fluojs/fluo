...
1. repository/PR policy에 맞는 merge method를 선택한다. method가 불명확하면 임의로 squash/merge/rebase를 고르지 말고 `needs-human-check`로 멈춘다.
2. 선택된 method로 `gh pr merge <pr> --<approved-method>`를 실행한다. remote branch 삭제 권한이 함께 승인된 경우에는 GitHub head branch 삭제 옵션도 함께 사용할 수 있다.
...
merge 확인 후, cleanup authority gate가 명시적으로 통과한 경우에만 아래 순서로 진행한다. cleanup은 merge의 부속 단계지만 별도 안전 gate이며, 실패해도 merge 자체를 되돌리려 하지 않는다.
