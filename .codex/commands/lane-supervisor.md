...
- `execute` — confirmed issue set을 lane에 배정하고 `/issue-to-pr` → `/pr-to-merge` → bounded fix-back → gated merge/cleanup/main sync를 **lane queue가 모두 소진되거나 진짜 terminal 상태가 될 때까지 반복** 조율한다.
...
7. **Merge/cleanup/root sync** — final child verdict가 `merge`이고 checks가 통과한 PR은 merge gate를 통과한 뒤 실제 merge를 수행한다. command가 만든 worktree/branch이고 PR merge 및 issue close 상태가 확인된 경우에만 cleanup한다. root sync는 root worktree가 clean이고 fast-forward-only가 가능한 경우에만 수행한다.
...
### 0. Supervisor loop invariant

`lane-supervisor`는 단발성 dispatch command가 아니라 **lane drain loop**다. `execute`, `auto`, `execute --full-auto`, `resume`에서 구현/리뷰를 시작했다면 아래 불변식을 반드시 지킨다.

1. **최종 보고 금지 조건** — 아래 중 하나라도 남아 있으면 최종 보고를 작성하지 않는다.
   - `queued`, `running`, `in_review`, `merged` 상태 lane
   - `block` verdict를 받았지만 `retry_count`와 blocker signature 정책상 fix-back 여지가 남은 PR
   - 선행 issue merge 이후 unlock된 후속 issue
   - evidence/check 상태가 pending, skipped, stale, in_progress인 PR
2. **진짜 terminal 상태**는 `done`, `blocked-terminal`, `needs-human-check-terminal`, `blocked-budget-exhausted`, `blocked-maintainer-decision`, `blocked-child-contract-error`, `blocked-ledger-conflict`뿐이다. 단순 `block`, `blocked`, `needs-human-check`, `in_review`는 terminal이 아니다.
3. **lane head 우선 원칙** — 각 lane은 항상 queue의 head issue만 실행한다. head가 merge되어 `completed_issues`에 들어가기 전에는 같은 lane의 다음 issue를 dispatch하지 않는다.
4. **block은 fix-back 입력** — `/pr-to-merge`가 `block`을 반환하면 그것은 최종 보고 사유가 아니라, 먼저 같은 PR/branch/worktree에 대한 fix-back 요청으로 전환해야 한다.
5. **loop completion 조건** — 모든 lane이 `done` 또는 진짜 terminal 상태이고, 실행 중인 worker/reviewer background task가 없으며, ledger가 현재 GitHub/repo 상태와 일치할 때만 최종 보고한다.
...
lane status는 신규 issue 구현 중에는 `queued -> running -> in_review`로 전이한다. PR URL이 없거나 verification summary가 불명확하면 즉시 terminal로 끝내지 말고, 먼저 같은 worker session에 1회 이상 “PR URL/branch/worktree/verification 또는 blocker를 반드시 보고하라”는 재지시를 수행한다. 재지시 후에도 결과가 없고 repo/GitHub read-only 확인에서 worktree/branch/PR이 없으면 `blocked-child-contract-error`로 표시한다.

#### Child completion barrier

`lane-supervisor`는 child implementer가 완료 보고를 반환하기 전에는 해당 worktree의 부분 변경을 직접 인계해 commit, push, PR 생성, fix-back 완료 처리로 진행하지 않는다. child가 running 상태이거나 최종 보고 없이 worktree에 미커밋 변경만 남긴 상태는 **incomplete child result**다.

허용되는 supervisor 동작은 아래로 제한한다.

1. 같은 child session에 상태/완료 보고를 요청한다.
2. read-only `git status`, `git diff --stat`, `git log`, `gh pr view/list`로 실제 상태만 확인한다.
3. child가 종료됐지만 보고가 누락된 경우 1회 재지시하거나 fallback executor를 새로 띄우되, 기존 미커밋 변경을 임의로 커밋하지 않는다.
4. 재지시 후에도 완료 보고가 없으면 lane을 `blocked-child-contract-error` 또는 `needs-human-check-terminal`로 기록한다.

완료 보고로 인정하려면 최소한 issue, branch, worktree, 변경 파일, commit hash 또는 명시적 blocker, 실행한 verifier와 결과, changeset 상태가 포함되어야 한다. `supervisor-auto` merge authority는 child completion barrier를 우회하지 않는다.
...
- `retry_count <= 2`: 같은 `/issue-to-pr` worker 컨텍스트에 좁은 blocker 해결만 재지시한다. 이 단계는 `execute`에서도 자동으로 수행한다.
- `retry_count >= 3`: interactive `execute`에서는 `needs-human-check-terminal`로 escalate한다. `auto`/`execute --full-auto`에서는 아래 full-auto 정책을 따른다.
- fix-back 이후에는 반드시 동일 PR에 대해 `/pr-to-merge`를 재실행한다. 재실행 전에는 다음 issue를 dispatch하거나 최종 보고하지 않는다.
- 같은 blocker signature가 반복되면 ledger에 `blocker_signatures`와 `same_failure_count`를 기록한다.
- blocker가 docs companion 누락, typecheck failure, failing test, skipped/pending checks처럼 구체적 수정 또는 증거 보강으로 해소 가능한 경우 `block`은 terminal이 아니다.
- maintainer-only decision, legal/security disclosure, release policy exception처럼 자동 수정으로 판단할 수 없는 경우에만 `needs-human-check-terminal` 또는 `blocked-maintainer-decision`으로 전환한다.
...
### 11-A. Merge execution contract

merge gate를 통과한 PR은 **verdict 기록으로 끝내지 않고** 승인된 권한 범위 안에서 실제 merge까지 진행한다. 단, `/pr-to-merge` 자체는 read-only gate이므로 merge 실행은 항상 `lane-supervisor`가 소유한다.

fluo repository의 PR merge method는 항상 **squash merge**다. repository settings가 merge commit 또는 rebase merge를 허용하더라도 `lane-supervisor`는 이를 선택지로 보지 않는다. supervisor merge authority가 있는 PR은 아래 gate를 모두 통과한 뒤 `gh pr merge <pr> --squash`로만 merge한다.

실행 전 확인:

1. ledger의 `PR`, `branch`, `worktree`, `base_branch`, `linked issue`가 현재 GitHub/repo 상태와 일치한다.
2. PR head branch가 ledger의 `branch`와 일치하고, worktree는 `<repo-root>/.worktrees/<branch>` 아래 command-owned path다.
3. PR이 open 상태이고 draft가 아니며 base branch가 ledger의 `base_branch`와 일치한다.
4. `gh pr checks <pr>` 또는 동등한 current checks가 모두 통과했고 stale/pending/skipped required check가 없다.
5. worktree에 uncommitted change나 unpushed remediation commit이 남아 있으면 merge하지 않고 `blocked`로 기록한다.

실행 순서:

1. `gh pr merge <pr> --squash`를 실행한다. remote branch 삭제 권한이 함께 승인된 경우에는 GitHub head branch 삭제 옵션도 함께 사용할 수 있다.
2. merge command가 repository policy나 branch protection 때문에 squash merge를 거부하면 다른 merge method로 재시도하지 말고 `needs-human-check`로 멈춘다.
3. `gh pr view <pr>`로 PR `MERGED` 상태, merge commit/sha, base branch 반영을 재확인한다.
4. linked issue가 `Closes #<issue>`로 자동 close되지 않았으면 즉시 cleanup하지 말고 issue close 상태를 재조회한다. 자동 close가 지연 중이면 짧게 재확인하고, 그래도 open이면 `cleanup: blocked-issue-open`으로 기록한다.
5. merge가 확인되면 lane head issue를 `completed_issues`에 넣고 lane status를 `merged`로 전이한 뒤 cleanup gate로 넘긴다.

금지:

- `/pr-to-merge`의 `merge` verdict만 보고 checks/current PR state 재확인 없이 merge하지 않는다.
- `--merge` 또는 `--rebase`로 PR을 merge하지 않는다.
- PR이 merge되지 않았거나 linked issue close 상태가 확인되지 않은 상태에서 branch/worktree cleanup을 시작하지 않는다.
...
merge 확인 후, `supervisor-auto` 또는 `supervisor-full-auto` merge authority로 merge한 command-owned lane worktree/branch는 cleanup authority가 함께 부여된 것으로 간주하고 아래 순서로 진행한다. `developer-final`처럼 사람이 merge를 수행한 경우에는 별도 cleanup approval이 필요하다. cleanup은 merge의 부속 단계지만 별도 안전 gate이며, 실패해도 merge 자체를 되돌리려 하지 않는다.

1. PR merge state가 `MERGED`인지 재확인한다.
2. linked issue close state가 `CLOSED`인지 확인한다.
3. ledger의 worktree path가 `<repo-root>/.worktrees/<branch>`이고 해당 worktree가 ledger branch를 checkout 중인지 확인한다.
4. worktree가 dirty이면 `git worktree remove --force`를 실행하지 않고 `cleanup: blocked-dirty-worktree`로 기록한다.
5. local branch가 다른 worktree에서 사용 중이면 삭제하지 않고 `cleanup: blocked-branch-in-use`로 기록한다.
6. GitHub merge 단계에서 remote head branch가 이미 삭제됐는지 확인한다. 남아 있고 remote cleanup authority가 있으면 remote branch를 삭제한다.
7. `git worktree remove <worktree>`로 local worktree를 제거한다. dirty worktree 강제 제거는 사용자가 별도 승인했거나 ledger가 generated-only 상태를 증명할 때만 허용한다.
8. local branch가 merge된 head branch이고 다른 worktree가 사용하지 않으면 local branch를 삭제한다.
9. `git worktree list`와 branch 조회로 제거 결과를 검증하고 ledger에 cleanup 결과를 기록한다.
...
각 lane에서 다음 unlocked issue를 선택한다. 더 이상 실행할 issue가 없으면 lane을 `done` 처리한다. interactive mode에서도 단순 `blocked`/`needs-human-check`가 아니라 `blocked-terminal`/`needs-human-check-terminal`로 명시된 lane만 종료 상태로 인정한다. full-auto mode에서는 unresolved `needs-human-check`를 먼저 remediation loop로 되돌리고, 해소 불가능한 경우 `blocked-*`로 최종 보고한다.

반복 순서:

1. running worker 결과 수집
2. PR mapping ledger 기록
3. `/pr-to-merge` 실행
4. `merge`면 merge gate와 merge execution contract를 수행한 뒤 issue close 확인, cleanup gate, lane head 완료 처리
5. `block`이면 bounded fix-back loop 수행 후 3으로 복귀
6. `needs-human-check`이면 mode별 remediation/escalation 처리
7. 선행 issue가 완료된 lane의 다음 issue unlock/dispatch
8. 모든 lane이 `done` 또는 진짜 terminal인지 재검사
9. 아직 남은 lane이 있으면 1로 복귀하고 최종 보고 금지
...
- merge gate를 통과한 PR을 verdict만 기록하고 실제 merge/cleanup/main sync 처리 없이 완료로 보고하지 않는다.
- `/pr-to-merge` `block` verdict를 최종 보고 사유로 즉시 사용하지 않는다. 먼저 bounded fix-back loop를 수행한다.
