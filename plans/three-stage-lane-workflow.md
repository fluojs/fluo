...
> **Deliverables**:
> - New command contracts for `.opencode-backup/commands/search-issue.md`, `.opencode-backup/commands/create-lane.md`, `.opencode-backup/commands/execute-lane.md`
> - Removal of `.opencode-backup/commands/lane-supervisor.md`
...
### Deliverables
- `.opencode-backup/commands/search-issue.md`: issue discovery/registration command contract.
- `.opencode-backup/commands/create-lane.md`: GitHub issue set to lane ledger command contract.
- `.opencode-backup/commands/execute-lane.md`: lane ledger execution/drain command contract.
- Delete `.opencode-backup/commands/lane-supervisor.md`; old invocations must fail instead of silently redirecting.
...
### Definition of Done
- `grep -RIn "Source choice\\|search-to-issue를 먼저 실행\\|source_mode: search-to-issue" .opencode-backup/commands .opencode-backup/agents` returns no live workflow contract references.
...
- Command-doc verifier gates check that `.opencode-backup/commands/create-lane.md` and `.opencode-backup/commands/execute-lane.md` document the canonical schema, status, cursor, root-sync, authority, and cleanup prerequisites.
...
- [ ] 1. Define Canonical Three-Stage Command Contracts
  ...
  **References**:
  - Pattern: `.opencode-backup/commands/search-issue.md` - reuse audit and issue creation responsibilities.
  - Historical source: run `git log --all --oneline -- .opencode-backup/commands/lane-supervisor.md`, then use `git show <commit>:.opencode-backup/commands/lane-supervisor.md` with the selected historical commit to inspect the deleted lane planning and execution loop.
  - Pattern: `.opencode-backup/commands/issue-to-pr.md` - preserve child completion barrier.
  - Pattern: `.opencode-backup/commands/pr-to-merge.md` - preserve read-only review gate.
  ...
  **QA Scenarios**:
  ...
  Scenario: Responsibility table is complete
    Tool: bash
    Steps: grep -RIn "search-issue.*create-lane.*execute-lane\\|Allowed\\|Forbidden\\|Inputs\\|Outputs" .opencode-backup/commands plans
  ...
  Scenario: Execution stage has no discovery responsibility
    Tool: bash
    Steps: grep -RIn "search-to-issue를 먼저 실행\\|Source choice\\|Suggested additions" .opencode-backup/commands/execute-lane.md
    Expected: command exits non-zero with no matches.
    Evidence: evidence/task-1-no-execute-discovery.txt
  ```

  **Commit**: YES | Message: `docs(commands): define three-stage lane workflow` | Files: `.opencode-backup/commands/*.md`

- [ ] 2. Define Lane Artifact and Ledger Schema

  **What to do**: Define the handoff artifacts:
  - `.opencode-backup/search-issue/<run-id>.json`
...
Include the exact root keys `version`, `run_id`, `lane_id`, `status`, `created_by`, `base_branch`, `source`, `merge_policy`, `pr_merge_method`, `authority_scope`, `retry_policy`, `execution`, `confirmed_issues`, `suggested_but_excluded`, `backlog_candidates`, `release_handoffs`, `completed_issues`, `issue_progress`, `lanes`, `dependency_graph`, and `root_main_sync`, with only optional `created_at`. The strict validator enforces exact-key equality, not an external provenance exemption.
  ...
  **Final strict v1 contract**:
  ...
  - `run_id` and `lane_id` use path-safe basenames without `+`. A `search-issue` source uses the source-only grammar `[A-Za-z0-9][A-Za-z0-9+._-]*`, preserves internal `+`, and requires exact `.opencode-backup/search-issue/<search_run_id>.json` provenance.
  ...
  **QA Scenarios**:
  ...
  Scenario: Canonical v1 lane schema is documented
    Tool: bash
    Steps: grep -RIn "status: ready\\|version: 1\\|current_issue\\|issue_progress\\|completed_issues" .opencode-backup/commands plans
  ...
  Scenario: Execution cannot change issue selection
    Tool: bash
    Steps: grep -RIn "confirmed_issues.*mutate\\|issue selection.*forbidden\\|never expands issue scope" .opencode-backup/commands/execute-lane.md
    Expected: output contains explicit prohibition.
    Evidence: evidence/task-2-no-scope-mutation.txt
  ```

  **Commit**: YES | Message: `docs(commands): define lane ledger schema` | Files: `.opencode-backup/commands/create-lane.md`, `.opencode-backup/commands/execute-lane.md`

- [ ] 3. Replace Search Command Contract

  **What to do**: Introduce `.opencode-backup/commands/search-issue.md`. Move the issue discovery and registration responsibilities into it, then add explicit artifact output.
  ...
  **References**:
  - Source: `.opencode-backup/commands/search-issue.md` - active issue discovery and registration contract.
  - Policy: `AGENTS.md` - side-effect gates for issue creation.

  **Acceptance Criteria**:
  - [ ] `.opencode-backup/commands/search-issue.md` exists.
  ...
  **QA Scenarios**:
  ...
  Scenario: Search stage outputs issue artifact
    Tool: bash
    Steps: grep -RIn ".opencode-backup/search-issue/.*selected_issues" .opencode-backup/commands/search-issue.md
  ...
  Scenario: Search stage forbids lane execution
    Tool: bash
    Steps: grep -RIn "lane planning.*금지\\|PR 생성.*금지\\|merge.*금지" .opencode-backup/commands/search-issue.md
    Expected: explicit prohibitions appear.
    Evidence: evidence/task-3-search-forbidden.txt
  ```

  **Commit**: YES | Message: `docs(commands): add search-issue stage` | Files: `.opencode-backup/commands/search-issue.md`

- [ ] 4. Add Create-Lane Command Contract

  **What to do**: Add `.opencode-backup/commands/create-lane.md`. It accepts issue numbers or a search artifact, validates GitHub issues read-only, groups lanes, sets merge policy, writes a ready lane ledger, and runs lane ledger validation.
  ...
  **References**:
  - Historical planning logic: run `git log --all --oneline -- .opencode-backup/commands/lane-supervisor.md`, then use `git show <commit>:.opencode-backup/commands/lane-supervisor.md` with the selected historical commit to inspect the deleted lane planning section.
  - Validator: `tooling/governance/verify-lane-ledger.mjs`.

  **Acceptance Criteria**:
  - [ ] `.opencode-backup/commands/create-lane.md` exists.
  ...
  **QA Scenarios**:
  ...
  Scenario: Create-lane has no implementation verbs
    Tool: bash
    Steps: grep -RIn "issue-to-pr\\|gh pr create\\|gh pr merge\\|git worktree add" .opencode-backup/commands/create-lane.md
  ...
  Scenario: Create-lane records authority upfront
    Tool: bash
    Steps: grep -RIn "merge_policy\\|authority_scope\\|cleanup_command_worktrees" .opencode-backup/commands/create-lane.md
    Expected: all authority fields appear.
    Evidence: evidence/task-4-authority.txt
  ```

  **Commit**: YES | Message: `docs(commands): add create-lane stage` | Files: `.opencode-backup/commands/create-lane.md`

- [ ] 5. Add Execute-Lane Command Contract

  **What to do**: Add `.opencode-backup/commands/execute-lane.md`. Move the old `lane-supervisor` drain loop into it: worker dispatch, PR collection, child completion barrier, review gate, fix-back loop, squash merge, issue close verification, cleanup, root main sync, and ledger validation.
  ...
  **References**:
  - Historical drain loop: run `git log --all --oneline -- .opencode-backup/commands/lane-supervisor.md`, then use `git show <commit>:.opencode-backup/commands/lane-supervisor.md` with the selected historical commit to inspect the deleted drain loop.
  - Implementation command: `.opencode-backup/commands/issue-to-pr.md`.
  - Review command: `.opencode-backup/commands/pr-to-merge.md`.
  - Release boundary: `.opencode-backup/commands/execute-lane.md` is the active execution contract; package publishing remains outside this workflow.

  **Acceptance Criteria**:
  - [ ] `.opencode-backup/commands/execute-lane.md` exists.
  ...
  **QA Scenarios**:
  ...
  Scenario: Execute-lane has drain loop but no discovery
    Tool: bash
    Steps: grep -RIn "Source choice\\|Suggested additions\\|search-to-issue를 먼저 실행" .opencode-backup/commands/execute-lane.md
    Expected: no matches.
    Evidence: evidence/task-5-no-discovery.txt

  Scenario: Execute-lane enforces squash and cleanup
    Tool: bash
    Steps: grep -RIn "gh pr merge .*--squash\\|cleanup\\|linked issue.*CLOSED" .opencode-backup/commands/execute-lane.md
    Expected: squash merge, cleanup, and issue close checks appear.
    Evidence: evidence/task-5-merge-cleanup.txt
  ```

  **Commit**: YES | Message: `docs(commands): add execute-lane stage` | Files: `.opencode-backup/commands/execute-lane.md`

- [ ] 6. Deprecate Lane-Supervisor and Update Caller References

  **What to do**: Delete `.opencode-backup/commands/lane-supervisor.md` entirely. Update `issue-to-pr`, `pr-to-merge`, and `fluo-issue-implementer` references from `lane-supervisor` to `execute-lane` or “lane execution harness”.
  ...
  **References**:
  - Stale refs: `.opencode-backup/commands/issue-to-pr.md`, `.opencode-backup/commands/pr-to-merge.md`, `.opencode-backup/agents/fluo-issue-implementer.md`.
  - Old command to remove: `.opencode-backup/commands/lane-supervisor.md`.

  **Acceptance Criteria**:
  - [ ] `.opencode-backup/commands/lane-supervisor.md` no longer exists.
  ...
  **QA Scenarios**:
  ...
  Scenario: No old source-choice examples remain
    Tool: bash
    Steps: grep -RIn "이 문제를 issue로 나누고\\|search-to-issue를 먼저 실행\\|Source choice" .opencode-backup/commands .opencode-backup/agents
  ...
  Scenario: Fix-back caller names are updated
    Tool: bash
    Steps: grep -RIn "execute-lane\\|lane execution harness" .opencode-backup/commands/issue-to-pr.md .opencode-backup/commands/pr-to-merge.md .opencode-backup/agents/fluo-issue-implementer.md
    Expected: all three files contain updated caller naming.
    Evidence: evidence/task-6-caller-refs.txt
  ```

  **Commit**: YES | Message: `docs(commands): remove lane-supervisor` | Files: `.opencode-backup/commands/lane-supervisor.md`, `.opencode-backup/commands/issue-to-pr.md`, `.opencode-backup/commands/pr-to-merge.md`, `.opencode-backup/agents/fluo-issue-implementer.md`
...
- [ ] 7. Genericize Lane Ledger Validator
  ...
  **QA Scenarios**:
  ...
  Scenario: Command-doc verifier gates are explicit
    Tool: bash
    Steps: grep -RIn "root_main_sync\\|authority\\|cleanup\\|current_issue\\|issue_progress" .opencode-backup/commands/create-lane.md .opencode-backup/commands/execute-lane.md
...
- [ ] 8. End-to-End Pipeline Documentation and Migration Verification
  ...
  **What to do**: Add an end-to-end command sequence to the command docs:
  ```text
  /search-issue
  /create-lane .opencode-backup/search-issue/<search_run_id>.json main
  ...
  **References**:
  ...
  - `.opencode-backup/MIGRATION.md` - active migration note documenting removal of `lane-supervisor` and the canonical replacement flow.
  ...
  **QA Scenarios**:
  ...
  Scenario: Canonical pipeline appears
    Tool: bash
    Steps: grep -RIn "/search-issue\\|/create-lane\\|/execute-lane" .opencode-backup/commands
  ...
  Scenario: No bypass path remains
    Tool: bash
    Steps: grep -RIn "/execute-lane .*2046\\|execute-lane.*issue list\\|lane-supervisor .*execute" .opencode-backup/commands
  ...
  **Commit**: YES | Message: `docs(commands): document lane pipeline migration` | Files: `.opencode-backup/commands/*.md`
...
- [ ] F2. Contract Reference Audit
  - Run `grep -RIn "lane-supervisor\\|search-to-issue\\|Source choice" .opencode-backup/commands .opencode-backup/agents tooling package.json` and classify every remaining match as migration artifact, validator compatibility, or defect.
- [ ] F3. Validator Audit
  ...
  - Run explicit command-doc verifier gates against `.opencode-backup/commands/create-lane.md` and `.opencode-backup/commands/execute-lane.md` for schema, statuses, cursors, root-sync authority and terminal prerequisites, and cleanup ordering.
...
- [ ] F4. Manual Pipeline QA
  - Use `tmux` to run a dry documentation walkthrough:
    ...
    tmux send-keys -t ulw-qa-lane-pipeline 'grep -RIn "/search-issue\\|/create-lane\\|/execute-lane" .opencode-backup/commands && pnpm verify:lane-ledger -- tooling/governance/fixtures/lane-ledger/valid-ready.json' C-m
